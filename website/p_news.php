...
$sql_cat= "SELECT Field_Name AS cat FROM  `setting`  WHERE  `Class` LIKE  '53'";
...
<?php }while($row_cat=mysql_fetch_assoc($query_cat)) ?>
    ...
    <strong>Category: </strong> <?php  echo $row_news['category'] .'  <strong>- Date </strong>  '. date("d-m-Y", strtotime( $row_news['ndate']))?> 
