...
if($inlineadd == ADD_INLINE)
	$templatefile = "student_evaluation_inline_add.htm";
else
	$templatefile = "student_evaluation_add.htm";
...
if(isset($_REQUEST['afteradd'])){
		header('Location: student_evaluation_add.php');
...
if(@$_POST["a"]=="added")
{
	...
	$avalues=array();
	$blobfields=array();
//	processing eid - start
	$inlineAddOption = true;
	$inlineAddOption = $inlineadd==ADD_INLINE;
	if($inlineAddOption)
	{
		$control_eid = $pageObject->getControl("eid", $id);
		$control_eid->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing eid - end
			...
			if($inlineadd==ADD_SIMPLE || $inlineadd==ADD_MASTER)
			{
				...
				if (count($keys))
				{
					...
					if($pageObject->pSet->hasEditPage() && $permis['edit'])
						$message .='&nbsp;<a href=\'student_evaluation_edit.php?'.$keylink.'\'>'."Edit".'</a>&nbsp;';
					if($pageObject->pSet->hasViewPage() && $permis['search'])
						$message .='&nbsp;<a href=\'student_evaluation_view.php?'.$keylink.'\'>'."View".'</a>&nbsp;';
				}
				$mesClass = "mes_ok";	
			}
		}
...
if (no_output_done() && $inlineadd==ADD_SIMPLE && $IsSaved)
{
	...
	// redirect
	header("Location: student_evaluation_".$pageObject->getPageType().".php");
