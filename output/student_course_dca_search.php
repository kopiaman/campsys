...
include("include/student_course_dca_variables.php");
...
$layout->blocks["top"][] = "search";$page_layouts["student_course_dca_search"] = $layout;
...
$mode = SEARCH_SIMPLE;
$templatefile = "student_course_dca_search.htm";
...
$params['shortTableName'] = 'student_course_dca';
...
if ($mode==SEARCH_SIMPLE)
{
	...
	if($pageObject->pSet->getLookupTable("ProgramID"))
		$pageObject->settingsMap["globalSettings"]['shortTNames'][$pageObject->pSet->getLookupTable("ProgramID")] = GetTableURL($pageObject->pSet->getLookupTable("ProgramID"));
	
	$pageObject->fillFieldToolTips("ProgramID");	
	
	$srchFields = $pageObject->searchClauseObj->getSearchCtrlParams("ProgramID");
	...
	{
		$firstFieldParams['fName'] = "ProgramID";
	...
	if(isEnableSection508())
		$xt->assign_section("ProgramID_label","<label for=\"".GetInputElementId("ProgramID", $id, PAGE_SEARCH)."\">","</label>");
	else 
		$xt->assign("ProgramID_label", true);
	
	$xt->assign("ProgramID_fieldblock", true);
	$xt->assignbyref("ProgramID_editcontrol", $ctrlBlockArr['searchcontrol']);
	$xt->assign("ProgramID_notbox", $ctrlBlockArr['notbox']);
	// create second control, if need it
	$xt->assignbyref("ProgramID_editcontrol1", $ctrlBlockArr['searchcontrol1']);
	// create search type select
	$xt->assign("searchtype_ProgramID", $ctrlBlockArr['searchtype']);
	$isFieldNeedSecCtrl = $searchControlBuilder->isNeedSecondCtrl("ProgramID");
	$ctrlInd = 0;
	if ($isFieldNeedSecCtrl)
	{
		$pageObject->controlsMap["search"]["searchBlocks"][] = array('fName'=>"ProgramID", 'recId'=>$id, 'ctrlsMap'=>array(0=>$ctrlInd, 1=>($ctrlInd+1)));
		$ctrlInd+=2;
	}
	else
	{
		$pageObject->controlsMap["search"]["searchBlocks"][] = array('fName'=>"ProgramID", 'recId'=>$id, 'ctrlsMap'=>array(0=>$ctrlInd));			
		$ctrlInd++;
	}
	// search fields data
