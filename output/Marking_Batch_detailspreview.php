...
$layout->blocks["bare"][] = "detailspreviewgrid";$page_layouts["Marking_Batch_detailspreview"] = $layout;
...
if($mastertable == "Marking_Student_list")
{
	$where = "";
...
if($rowcount) {
	...
	//	e1 - 
			$viewContainer->recId = $recordsCounter;
		    $value = $viewContainer->showDBValue("e1", $data, $keylink);
			$row["e1_value"] = $value;
	//	e2 - 
			$viewContainer->recId = $recordsCounter;
		    $value = $viewContainer->showDBValue("e2", $data, $keylink);
			$row["e2_value"] = $value;
	//	e3 - 
			$viewContainer->recId = $recordsCounter;
		    $value = $viewContainer->showDBValue("e3", $data, $keylink);
			$row["e3_value"] = $value;
	//	e4 - 
			$viewContainer->recId = $recordsCounter;
		    $value = $viewContainer->showDBValue("e4", $data, $keylink);
			$row["e4_value"] = $value;
	//	cgpa - Custom
			$viewContainer->recId = $recordsCounter;
		    $value = $viewContainer->showDBValue("cgpa", $data, $keylink);
			$row["cgpa_value"] = $value;
		$rowinfo[] = $row;
		$data = $cipherer->DecryptFetchedArray($rs);
	}
...
$xt->load_template("Marking_Batch_detailspreview.htm");
