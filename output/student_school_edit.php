...
include("include/student_school_variables.php");
...
$layout->blocks["top"][] = "details";$page_layouts["student_school_edit"] = $layout;
...
$templatefile = ($inlineedit == EDIT_INLINE) ? "student_school_inline_edit.htm" : "student_school_edit.htm";
...
//	processing subject - begin
	$condition = 1;

	if($condition)
	{
		$control_subject = $pageObject->getControl("subject", $id);
		$control_subject->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing subject - end
	...
	//	do event
		...
		{		
			...
			//	processing subject - begin
				...
				if($condition)
				{
					$control_subject->afterSuccessfulSave();
				}
	//	processing subject - end
			//	processing grade - begin
							$condition = 1;
...
if ($IsSaved && no_output_done() && $inlineedit == EDIT_SIMPLE)
{
	...
	// redirect
	header("Location: student_school_".$pageObject->getPageType().".php?".$keyGetQ);
...
if(!$data)
{
	...
	{
		header("Location: student_school_list.php?a=return");
		exit();
	}
	else
		$data = array();
...
if($inlineedit != EDIT_INLINE)
{
	...
	if(!$pageObject->isAppearOnTabs("subject"))
		$xt->assign("subject_fieldblock",true);
	else
		$xt->assign("subject_tabfieldblock",true);
	$xt->assign("subject_label",true);
	if(isEnableSection508())
		$xt->assign_section("subject_label","<label for=\"".GetInputElementId("subject", $id, PAGE_EDIT)."\">","</label>");
...
//	subject - 
	$value = $pageObject->showDBValue("subject", $data, $keylink);
	$showValues["subject"] = $value;
	$showFields[] = "subject";
		$showRawValues["subject"] = substr($data["subject"],0,100);
...
if($pageObject->isShowDetailTables && $inlineedit!=EDIT_INLINE && !isMobile())
{
	...
	for($d=0;$d<count($dpParams['ids']);$d++)
	{
		...
		$options['masterTable'] = "student_school";
		$options['firstTime'] = 1;
...
$xt->assign("viewlink_attrs","id=\"viewButton".$id."\" name=\"viewButton".$id."\" onclick=\"window.location.href='student_school_view.php?".$viewlink."'\"");
