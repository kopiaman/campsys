...
$layout->blocks["top"][] = "details";$page_layouts["w_gallery_add"] = $layout;
			...
			if($inlineadd==ADD_SIMPLE || $inlineadd==ADD_MASTER)
			{
				...
				if (count($keys))
				{
					...
					if($pageObject->pSet->hasEditPage() && $permis['edit'])
						$message .='&nbsp;<a href=\'w_gallery_edit.php?'.$keylink.'\'>'."Edit".'</a>&nbsp;';
					if($pageObject->pSet->hasViewPage() && $permis['search'])
						$message .='&nbsp;<a href=\'w_gallery_view.php?'.$keylink.'\'>'."View".'</a>&nbsp;';
				}
				$mesClass = "mes_ok";	
			}
		}
...
if($readavalues)
{
	$defvalues["category_name"]=@$avalues["category_name"];
}
...
if($inlineadd!=ADD_INLINE)
{
	...
	if(!$pageObject->isAppearOnTabs("category_name"))
		$xt->assign("category_name_fieldblock",true);
	else
		$xt->assign("category_name_tabfieldblock",true);
	$xt->assign("category_name_label",true);
	if(isEnableSection508())
		$xt->assign_section("category_name_label","<label for=\"".GetInputElementId("category_name", $id, PAGE_ADD)."\">","</label>");
...
function GetAddedDataLookupQuery($pageObject, $keys, $forLookup)
{
	...
	if(postvalue("table")=="w_gallery_sub" && postvalue("field")=="wcat") 
	{
		$lookupFieldName = "wcat";
		$lookupTable = "w_gallery";
		$linkFieldName = "wcat";
		$dispfield = "category_name";
		// Table which contain lookup field!
		$lookupPSet = new ProjectSettings("w_gallery_sub", PAGE_ADD);
		$nLookupType = $lookupPSet->getLookupType($lookupFieldName);
		if($nLookupType == LT_QUERY)
		{
			if($lookupPSet->getCustomDisplay($lookupFieldName))
				$pageObject->pSet->getSQLQuery()->AddCustomExpression($dispfield, $pageObject->pSet, "w_gallery_sub", "wcat");
			...
			$LookupSQL = "select ";
			$LookupSQL .= GetFullFieldName("wcat", $strTableName, true);
					if($linkFieldName != $dispfield)
				$LookupSQL .= ",category_name";
		}
	}
