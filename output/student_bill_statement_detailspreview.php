...
$layout->blocks["bare"][] = "detailspreviewgrid";$page_layouts["student_bill_statement_detailspreview"] = $layout;
...
if($mastertable == "student_bill_list2")
{
	$where = "";
...
if($rowcount) {
	...
	//	statement - Document Download
		    ...
		    $value = $viewContainer->showDBValue("statement", $data, $keylink);
			$row["statement_value"] = $value;
	//	dates - Short Date
			$viewContainer->recId = $recordsCounter;
		    $value = $viewContainer->showDBValue("dates", $data, $keylink);
			$row["dates_value"] = $value;
		$rowinfo[] = $row;
		$data = $cipherer->DecryptFetchedArray($rs);
	}
...
$xt->load_template("student_bill_statement_detailspreview.htm");
