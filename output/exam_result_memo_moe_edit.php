...
include("include/exam_result_memo_moe_variables.php");
...
$templatefile = ($inlineedit == EDIT_INLINE) ? "exam_result_memo_moe_inline_edit.htm" : "exam_result_memo_moe_edit.htm";
...
if ($IsSaved && no_output_done() && $inlineedit == EDIT_SIMPLE)
{
	...
	// redirect
	header("Location: exam_result_memo_moe_".$pageObject->getPageType().".php?".$keyGetQ);
...
if(!$data)
{
	...
	{
		header("Location: exam_result_memo_moe_list.php?a=return");
		exit();
	}
	else
		$data = array();
...
if($pageObject->isShowDetailTables && $inlineedit!=EDIT_INLINE && !isMobile())
{
	...
	for($d=0;$d<count($dpParams['ids']);$d++)
	{
		...
		$options['masterTable'] = "exam_result_memo_moe";
		$options['firstTime'] = 1;
...
$xt->assign("viewlink_attrs","id=\"viewButton".$id."\" name=\"viewButton".$id."\" onclick=\"window.location.href='exam_result_memo_moe_view.php?".$viewlink."'\"");
