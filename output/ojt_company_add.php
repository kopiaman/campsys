...
add_nocache_headers();
include("include/ojt_company_variables.php");
...
if($inlineadd == ADD_INLINE)
	$templatefile = "ojt_company_inline_add.htm";
else
	$templatefile = "ojt_company_add.htm";
...
if(isset($_REQUEST['afteradd'])){
		header('Location: ojt_company_add.php');
			...
			if($inlineadd==ADD_SIMPLE || $inlineadd==ADD_MASTER)
			{
				...
				if (count($keys))
				{
					...
					if($pageObject->pSet->hasEditPage() && $permis['edit'])
						$message .='&nbsp;<a href=\'ojt_company_edit.php?'.$keylink.'\'>'."Edit".'</a>&nbsp;';
					if($pageObject->pSet->hasViewPage() && $permis['search'])
						$message .='&nbsp;<a href=\'ojt_company_view.php?'.$keylink.'\'>'."View".'</a>&nbsp;';
				}
				$mesClass = "mes_ok";	
			}
		}
...
if (no_output_done() && $inlineadd==ADD_SIMPLE && $IsSaved)
{
	...
	// redirect
	header("Location: ojt_company_".$pageObject->getPageType().".php");
...
if($pageObject->isShowDetailTables && ($inlineadd==ADD_SIMPLE || $inlineadd==ADD_POPUP) && !isMobile())
{
	...
	for($d=0;$d<count($dpParams['ids']);$d++)
	{
		...
		$options['masterTable'] = "ojt_company";
		$options['firstTime'] = 1;
...
function GetAddedDataLookupQuery($pageObject, $keys, $forLookup)
{
	...
	if(postvalue("table")=="Report__Industry_Training" && postvalue("field")=="organization_name") 
	{
		$lookupFieldName = "organization_name";
		$lookupTable = "ojt_company";
		$linkFieldName = "id";
		$dispfield = "organization_name";
		// Table which contain lookup field!
		$lookupPSet = new ProjectSettings("Report-_Industry_Training", PAGE_ADD);
		$nLookupType = $lookupPSet->getLookupType($lookupFieldName);
		if($nLookupType == LT_QUERY)
		{
			if($lookupPSet->getCustomDisplay($lookupFieldName))
				$pageObject->pSet->getSQLQuery()->AddCustomExpression($dispfield, $pageObject->pSet, "Report-_Industry_Training", "organization_name");
