...
add_nocache_headers();
include("include/student_qualification_variables.php");
...
$layout->blocks["top"][] = "details";$page_layouts["student_qualification_add"] = $layout;
...
$layout->blocks["bare"][] = "1";$page_layouts["student_qualification_add_School_Education_Record1"] = $layout;
...
$layout->skins["1"] = "fields";
$layout->blocks["bare"][] = "1";$page_layouts["student_qualification_add_Highest_Education_Record1"] = $layout;
...
if($inlineadd == ADD_INLINE)
	$templatefile = "student_qualification_inline_add.htm";
else
	$templatefile = "student_qualification_add.htm";
...
if(isset($_REQUEST['afteradd'])){
		header('Location: student_qualification_add.php');
...
if(@$_POST["a"]=="added")
{
	...
	$avalues=array();
	$blobfields=array();
//	processing Education_Type - start
	$inlineAddOption = true;
	if($inlineAddOption)
	{
		$control_Education_Type = $pageObject->getControl("Education_Type", $id);
		$control_Education_Type->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing Education_Type - end
...
//	processing Education_Type - start
			...
			if($inlineAddOption)
			{
				$control_Education_Type->afterSuccessfulSave();
			}
//	processing Education_Type - end
//	processing Institution - start
			$inlineAddOption = true;
			...
			if($inlineadd==ADD_SIMPLE || $inlineadd==ADD_MASTER)
			{
				...
				if (count($keys))
				{
					...
					if($pageObject->pSet->hasEditPage() && $permis['edit'])
						$message .='&nbsp;<a href=\'student_qualification_edit.php?'.$keylink.'\'>'."Edit".'</a>&nbsp;';
					if($pageObject->pSet->hasViewPage() && $permis['search'])
						$message .='&nbsp;<a href=\'student_qualification_view.php?'.$keylink.'\'>'."View".'</a>&nbsp;';
				}
				$mesClass = "mes_ok";	
			}
		}
...
if (no_output_done() && $inlineadd==ADD_SIMPLE && $IsSaved)
{
	...
	// redirect
	header("Location: student_qualification_".$pageObject->getPageType().".php");
...
if($readavalues)
{
	...
	$defvalues["YearGrad"]=@$avalues["YearGrad"];
	$defvalues["Education_Type"]=@$avalues["Education_Type"];
	$defvalues["Major"]=@$avalues["Major"];
	$defvalues["Grade_CGPA"]=@$avalues["Grade_CGPA"];
}
...
if($inlineadd!=ADD_INLINE)
{
	...
	if(!$pageObject->isAppearOnTabs("YearGrad"))
		$xt->assign("YearGrad_fieldblock",true);
	else
		$xt->assign("YearGrad_tabfieldblock",true);
	$xt->assign("YearGrad_label",true);
	if(isEnableSection508())
		$xt->assign_section("YearGrad_label","<label for=\"".GetInputElementId("YearGrad", $id, PAGE_ADD)."\">","</label>");
	
	if(!$pageObject->isAppearOnTabs("Education_Type"))
		$xt->assign("Education_Type_fieldblock",true);
	else
		$xt->assign("Education_Type_tabfieldblock",true);
	$xt->assign("Education_Type_label",true);
	if(isEnableSection508())
		$xt->assign_section("Education_Type_label","<label for=\"".GetInputElementId("Education_Type", $id, PAGE_ADD)."\">","</label>");
	
	if(!$pageObject->isAppearOnTabs("Major"))
		$xt->assign("Major_fieldblock",true);
	else
		$xt->assign("Major_tabfieldblock",true);
	$xt->assign("Major_label",true);
	if(isEnableSection508())
		$xt->assign_section("Major_label","<label for=\"".GetInputElementId("Major", $id, PAGE_ADD)."\">","</label>");
	
	if(!$pageObject->isAppearOnTabs("Grade_CGPA"))
		$xt->assign("Grade_CGPA_fieldblock",true);
	else
		$xt->assign("Grade_CGPA_tabfieldblock",true);
	$xt->assign("Grade_CGPA_label",true);
	if(isEnableSection508())
		$xt->assign_section("Grade_CGPA_label","<label for=\"".GetInputElementId("Grade_CGPA", $id, PAGE_ADD)."\">","</label>");
...
//	Education_Type
	...
	if($display)
	{	
		$value = $pageObject->showDBValue("Education_Type", $data, $keylink);
		$showValues["Education_Type"] = $value;
		$showFields[] = "Education_Type";
	}	
//	Major
	$display = false;
	if($inlineadd==ADD_MASTER)
...
//	Grade_CGPA
	...
	{	
		...
		$showRawValues["Education_Type"] = substr($data["Education_Type"],0,100);
...
if($pageObject->isShowDetailTables && ($inlineadd==ADD_SIMPLE || $inlineadd==ADD_POPUP) && !isMobile())
{
	...
	for($d=0;$d<count($dpParams['ids']);$d++)
	{
		...
		$options['masterTable'] = "student_qualification";
		$options['firstTime'] = 1;
