...
$layout->blocks["top"][] = "search";$page_layouts["timetable_attendance_search"] = $layout;
...
$mode = SEARCH_SIMPLE;
$templatefile = "timetable_attendance_search.htm";
...
$params['shortTableName'] = 'timetable_attendance';
...
if ($mode==SEARCH_SIMPLE)
{
	...
	if($pageObject->pSet->getLookupTable("teach_hour"))
		$pageObject->settingsMap["globalSettings"]['shortTNames'][$pageObject->pSet->getLookupTable("teach_hour")] = GetTableURL($pageObject->pSet->getLookupTable("teach_hour"));
	
	$pageObject->fillFieldToolTips("teach_hour");	
	
	$srchFields = $pageObject->searchClauseObj->getSearchCtrlParams("teach_hour");
	$firstFieldParams = array();
	if (count($srchFields))
	{
		$firstFieldParams = $srchFields[0];
	...
	{
		$firstFieldParams['fName'] = "teach_hour";
		$firstFieldParams['eType'] = '';
		$firstFieldParams['value1'] = '';
		$firstFieldParams['opt'] = '';
		$firstFieldParams['value2'] = '';
	...
	if(isEnableSection508())
		$xt->assign_section("teach_hour_label","<label for=\"".GetInputElementId("teach_hour", $id, PAGE_SEARCH)."\">","</label>");
	else 
		$xt->assign("teach_hour_label", true);
	
	$xt->assign("teach_hour_fieldblock", true);
	$xt->assignbyref("teach_hour_editcontrol", $ctrlBlockArr['searchcontrol']);
	$xt->assign("teach_hour_notbox", $ctrlBlockArr['notbox']);
	// create second control, if need it
	$xt->assignbyref("teach_hour_editcontrol1", $ctrlBlockArr['searchcontrol1']);
	// create search type select
	$xt->assign("searchtype_teach_hour", $ctrlBlockArr['searchtype']);
	$isFieldNeedSecCtrl = $searchControlBuilder->isNeedSecondCtrl("teach_hour");
	$ctrlInd = 0;
	if ($isFieldNeedSecCtrl)
	{
		$pageObject->controlsMap["search"]["searchBlocks"][] = array('fName'=>"teach_hour", 'recId'=>$id, 'ctrlsMap'=>array(0=>$ctrlInd, 1=>($ctrlInd+1)));
		$ctrlInd+=2;
	}
	else
	{
		$pageObject->controlsMap["search"]["searchBlocks"][] = array('fName'=>"teach_hour", 'recId'=>$id, 'ctrlsMap'=>array(0=>$ctrlInd));			
		$ctrlInd++;
	}
	// search fields data
