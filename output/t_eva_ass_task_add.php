...
if($inlineadd == ADD_INLINE)
	$templatefile = "t_eva_ass_task_inline_add.htm";
else
	$templatefile = "t_eva_ass_task_add.htm";
...
if(isset($_REQUEST['afteradd'])){
		header('Location: t_eva_ass_task_add.php');
...
if(@$_POST["a"]=="added")
{
	...
	$avalues=array();
	$blobfields=array();
//	processing sub_topic - start
	$inlineAddOption = true;
	$inlineAddOption = $inlineadd==ADD_INLINE;
	if($inlineAddOption)
	{
		$control_sub_topic = $pageObject->getControl("sub_topic", $id);
		$control_sub_topic->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
...
//	processing main_topic - start
	$inlineAddOption = true;
	$inlineAddOption = $inlineadd==ADD_INLINE;
	if($inlineAddOption)
	{
		$control_main_topic = $pageObject->getControl("main_topic", $id);
		$control_main_topic->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing main_topic - end
//	processing taskno - start
	$inlineAddOption = true;
	$inlineAddOption = $inlineadd==ADD_INLINE;
	if($inlineAddOption)
	{
		$control_taskno = $pageObject->getControl("taskno", $id);
		$control_taskno->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
			...
			if($inlineadd==ADD_SIMPLE || $inlineadd==ADD_MASTER)
			{
				...
				if (count($keys))
				{
					...
					if($pageObject->pSet->hasEditPage() && $permis['edit'])
						$message .='&nbsp;<a href=\'t_eva_ass_task_edit.php?'.$keylink.'\'>'."Edit".'</a>&nbsp;';
					if($pageObject->pSet->hasViewPage() && $permis['search'])
						$message .='&nbsp;<a href=\'t_eva_ass_task_view.php?'.$keylink.'\'>'."View".'</a>&nbsp;';
				}
				$mesClass = "mes_ok";	
			}
		}
...
if (no_output_done() && $inlineadd==ADD_SIMPLE && $IsSaved)
{
	...
	// redirect
	header("Location: t_eva_ass_task_".$pageObject->getPageType().".php");
