...
$layout->blocks["top"][] = "details";$page_layouts["student_qualification_edit"] = $layout;
...
$templatefile = ($inlineedit == EDIT_INLINE) ? "student_qualification_inline_edit.htm" : "student_qualification_edit.htm";
...
//	processing Education_Type - begin
	$condition = 1;

	if($condition)
	{
		$control_Education_Type = $pageObject->getControl("Education_Type", $id);
		$control_Education_Type->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);
...
if ($IsSaved && no_output_done() && $inlineedit == EDIT_SIMPLE)
{
	...
	// redirect
	header("Location: student_qualification_".$pageObject->getPageType().".php?".$keyGetQ);
...
if(!$data)
{
	...
	{
		header("Location: student_qualification_list.php?a=return");
		exit();
	}
	else
		$data = array();
...
if($readevalues)
{
	...
	$data["Education_Type"] = $evalues["Education_Type"];
...
if($inlineedit != EDIT_INLINE)
{
	...
	if(isEnableSection508())
		$xt->assign_section("YearGrad_label","<label for=\"".GetInputElementId("YearGrad", $id, PAGE_EDIT)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("Education_Type_label","<label for=\"".GetInputElementId("Education_Type", $id, PAGE_EDIT)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("Major_label","<label for=\"".GetInputElementId("Major", $id, PAGE_EDIT)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("Grade_CGPA_label","<label for=\"".GetInputElementId("Grade_CGPA", $id, PAGE_EDIT)."\">","</label>");
...
foreach($pageObject->editFields as $fName)
{
	...
	if($inlineedit == EDIT_INLINE)
	{
		...
		$controls["controls"]['mode'] = "inline_edit";
			...
			if($pageObject->pSet->isUseRTE("YearGrad") && $fName == "YearGrad")
	{
		$_SESSION[$strTableName."_"."YearGrad"."_rte"] = date("Y");
...
$xt->assign("viewlink_attrs","id=\"viewButton".$id."\" name=\"viewButton".$id."\" onclick=\"window.location.href='student_qualification_view.php?".$viewlink."'\"");
