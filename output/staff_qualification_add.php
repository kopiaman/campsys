			...
			if($inlineadd==ADD_SIMPLE || $inlineadd==ADD_MASTER)
			{
				...
				if (count($keys))
				{
					...
					if($pageObject->pSet->hasEditPage() && $permis['edit'])
						$message .='&nbsp;<a href=\'staff_qualification_edit.php?'.$keylink.'\'>'."Edit".'</a>&nbsp;';
					if($pageObject->pSet->hasViewPage() && $permis['search'])
						$message .='&nbsp;<a href=\'staff_qualification_view.php?'.$keylink.'\'>'."View".'</a>&nbsp;';
				}
				$mesClass = "mes_ok";	
			}
		}
...
if($readavalues)
{
	...
	$defvalues["EducationType"]=@$avalues["EducationType"];
...
if($inlineadd!=ADD_INLINE)
{
	...
	if(!$pageObject->isAppearOnTabs("EducationType"))
		$xt->assign("EducationType_fieldblock",true);
	else
		$xt->assign("EducationType_tabfieldblock",true);
	$xt->assign("EducationType_label",true);
	if(isEnableSection508())
		$xt->assign_section("EducationType_label","<label for=\"".GetInputElementId("EducationType", $id, PAGE_ADD)."\">","</label>");
