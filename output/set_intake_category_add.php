...
add_nocache_headers();
include("include/set_intake_category_variables.php");
...
$layout->blocks["top"][] = "details";$page_layouts["set_intake_category_add"] = $layout;
...
if($inlineadd == ADD_INLINE)
	$templatefile = "set_intake_category_inline_add.htm";
else
	$templatefile = "set_intake_category_add.htm";
...
if(isset($_REQUEST['afteradd'])){
		header('Location: set_intake_category_add.php');
...
//	processing Intake_Code - start
	$inlineAddOption = true;
	$inlineAddOption = $inlineadd!=ADD_INLINE;
	if($inlineAddOption)
	{
		$control_Intake_Code = $pageObject->getControl("Intake_Code", $id);
		$control_Intake_Code->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing Intake_Code - end
...
//	processing Intake_Code - start
			...
			if($inlineAddOption)
			{
				$control_Intake_Code->afterSuccessfulSave();
			}
//	processing Intake_Code - end
			...
			if($inlineadd==ADD_SIMPLE || $inlineadd==ADD_MASTER)
			{
				...
				if (count($keys))
				{
					...
					if($pageObject->pSet->hasEditPage() && $permis['edit'])
						$message .='&nbsp;<a href=\'set_intake_category_edit.php?'.$keylink.'\'>'."Edit".'</a>&nbsp;';
					if($pageObject->pSet->hasViewPage() && $permis['search'])
						$message .='&nbsp;<a href=\'set_intake_category_view.php?'.$keylink.'\'>'."View".'</a>&nbsp;';
				}
				$mesClass = "mes_ok";	
			}
		}
...
if (no_output_done() && $inlineadd==ADD_SIMPLE && $IsSaved)
{
	...
	// redirect
	header("Location: set_intake_category_".$pageObject->getPageType().".php");
...
if($readavalues)
{
	...
	$defvalues["Intake_Code"]=@$avalues["Intake_Code"];
}
...
if($inlineadd!=ADD_INLINE)
{
	...
	if(!$pageObject->isAppearOnTabs("Intake_Code"))
		$xt->assign("Intake_Code_fieldblock",true);
	else
		$xt->assign("Intake_Code_tabfieldblock",true);
	$xt->assign("Intake_Code_label",true);
	if(isEnableSection508())
		$xt->assign_section("Intake_Code_label","<label for=\"".GetInputElementId("Intake_Code", $id, PAGE_ADD)."\">","</label>");
...
//	Intake_Code
	...
	if($display)
	{	
		$value = $pageObject->showDBValue("Intake_Code", $data, $keylink);
		$showValues["Intake_Code"] = $value;
		$showFields[] = "Intake_Code";
	}	
		$showRawValues["id"] = substr($data["id"],0,100);
		$showRawValues["Intake_Category"] = substr($data["Intake_Category"],0,100);
		$showRawValues["Intake_Code"] = substr($data["Intake_Code"],0,100);
...
if($pageObject->isShowDetailTables && ($inlineadd==ADD_SIMPLE || $inlineadd==ADD_POPUP) && !isMobile())
{
	...
	for($d=0;$d<count($dpParams['ids']);$d++)
	{
		...
		$options['masterTable'] = "set_intake_category";
		$options['firstTime'] = 1;
...
function GetAddedDataLookupQuery($pageObject, $keys, $forLookup)
{
	...
	if(postvalue("table")=="student_info" && postvalue("field")=="Intake_Category") 
	{
		$lookupFieldName = "Intake_Category";
		$lookupTable = "set_intake_category";
		$linkFieldName = "Intake_Code";
		$dispfield = "Intake_Category";
		...
		if($nLookupType == LT_QUERY)
		{
			if($lookupPSet->getCustomDisplay($lookupFieldName))
				$pageObject->pSet->getSQLQuery()->AddCustomExpression($dispfield, $pageObject->pSet, "student_info", "Intake_Category");
			...
			$LookupSQL = "select ";
			$LookupSQL .= GetFullFieldName("Intake_Code", $strTableName, true);
					if($linkFieldName != $dispfield)
				$LookupSQL .= ",Intake_Category";
		}
	}
	if(postvalue("table")=="Report_Sponsor" && postvalue("field")=="Intake_Category") 
	{
		$lookupFieldName = "Intake_Category";
		$lookupTable = "set_intake_category";
		$linkFieldName = "Intake_Code";
		...
		{
			if($lookupPSet->getCustomDisplay($lookupFieldName))
				$pageObject->pSet->getSQLQuery()->AddCustomExpression($dispfield, $pageObject->pSet, "Report_Sponsor", "Intake_Category");
			$lookupQueryObj = $pageObject->pSet->getSQLQuery()->CloneObject();
			if(!$noBlobReplace)
				$lookupQueryObj->ReplaceFieldsWithDummies($pageObject->pSet->getBinaryFieldsIndices());
		}else{
	...
	if(postvalue("table")=="Report_Student_Class" && postvalue("field")=="Intake_Category") 
	{
		$lookupFieldName = "Intake_Category";
		$lookupTable = "set_intake_category";
		$linkFieldName = "Intake_Code";
		...
		{
			if($lookupPSet->getCustomDisplay($lookupFieldName))
				$pageObject->pSet->getSQLQuery()->AddCustomExpression($dispfield, $pageObject->pSet, "Report_Student_Class", "Intake_Category");
			$lookupQueryObj = $pageObject->pSet->getSQLQuery()->CloneObject();
			if(!$noBlobReplace)
				$lookupQueryObj->ReplaceFieldsWithDummies($pageObject->pSet->getBinaryFieldsIndices());
		}else{
	...
	if(postvalue("table")=="Evaluation_Batch_List" && postvalue("field")=="Intake_Category") 
	{
		$lookupFieldName = "Intake_Category";
		$lookupTable = "set_intake_category";
		$linkFieldName = "Intake_Code";
		...
		{
			if($lookupPSet->getCustomDisplay($lookupFieldName))
				$pageObject->pSet->getSQLQuery()->AddCustomExpression($dispfield, $pageObject->pSet, "Evaluation_Batch_List", "Intake_Category");
			$lookupQueryObj = $pageObject->pSet->getSQLQuery()->CloneObject();
			if(!$noBlobReplace)
				$lookupQueryObj->ReplaceFieldsWithDummies($pageObject->pSet->getBinaryFieldsIndices());
		}else{
	...
	if(postvalue("table")=="student_bill_list" && postvalue("field")=="Intake_Category") 
	{
		$lookupFieldName = "Intake_Category";
		$lookupTable = "set_intake_category";
		$linkFieldName = "Intake_Code";
		...
		{
			if($lookupPSet->getCustomDisplay($lookupFieldName))
				$pageObject->pSet->getSQLQuery()->AddCustomExpression($dispfield, $pageObject->pSet, "student_bill_list", "Intake_Category");
			$lookupQueryObj = $pageObject->pSet->getSQLQuery()->CloneObject();
			if(!$noBlobReplace)
				$lookupQueryObj->ReplaceFieldsWithDummies($pageObject->pSet->getBinaryFieldsIndices());
		}else{
	...
	if(postvalue("table")=="Marking_Student_list" && postvalue("field")=="Intake_Category") 
	{
		$lookupFieldName = "Intake_Category";
		$lookupTable = "set_intake_category";
		$linkFieldName = "Intake_Code";
		...
		{
			if($lookupPSet->getCustomDisplay($lookupFieldName))
				$pageObject->pSet->getSQLQuery()->AddCustomExpression($dispfield, $pageObject->pSet, "Marking_Student_list", "Intake_Category");
			$lookupQueryObj = $pageObject->pSet->getSQLQuery()->CloneObject();
			if(!$noBlobReplace)
				$lookupQueryObj->ReplaceFieldsWithDummies($pageObject->pSet->getBinaryFieldsIndices());
		}else{
	...
	if(postvalue("table")=="Report_Attendance_Student_List" && postvalue("field")=="Intake_Category") 
	{
		$lookupFieldName = "Intake_Category";
		$lookupTable = "set_intake_category";
		$linkFieldName = "Intake_Code";
		...
		{
			if($lookupPSet->getCustomDisplay($lookupFieldName))
				$pageObject->pSet->getSQLQuery()->AddCustomExpression($dispfield, $pageObject->pSet, "Report_Attendance_Student_List", "Intake_Category");
			$lookupQueryObj = $pageObject->pSet->getSQLQuery()->CloneObject();
			if(!$noBlobReplace)
				$lookupQueryObj->ReplaceFieldsWithDummies($pageObject->pSet->getBinaryFieldsIndices());
		}else{
	...
	if(postvalue("table")=="Report_Student_Course_List" && postvalue("field")=="Intake_Category") 
	{
		$lookupFieldName = "Intake_Category";
		$lookupTable = "set_intake_category";
		$linkFieldName = "Intake_Code";
		...
		{
			if($lookupPSet->getCustomDisplay($lookupFieldName))
				$pageObject->pSet->getSQLQuery()->AddCustomExpression($dispfield, $pageObject->pSet, "Report_Student_Course_List", "Intake_Category");
			$lookupQueryObj = $pageObject->pSet->getSQLQuery()->CloneObject();
			if(!$noBlobReplace)
				$lookupQueryObj->ReplaceFieldsWithDummies($pageObject->pSet->getBinaryFieldsIndices());
		}else{
	...
	if(postvalue("table")=="student_course_DCA_List_student2" && postvalue("field")=="Intake_Category") 
	{
		$lookupFieldName = "Intake_Category";
		$lookupTable = "set_intake_category";
		$linkFieldName = "Intake_Code";
		...
		{
			if($lookupPSet->getCustomDisplay($lookupFieldName))
				$pageObject->pSet->getSQLQuery()->AddCustomExpression($dispfield, $pageObject->pSet, "student_course_DCA_List_student2", "Intake_Category");
			$lookupQueryObj = $pageObject->pSet->getSQLQuery()->CloneObject();
			if(!$noBlobReplace)
				$lookupQueryObj->ReplaceFieldsWithDummies($pageObject->pSet->getBinaryFieldsIndices());
		}else{
	...
	if(postvalue("table")=="student_bill_list2" && postvalue("field")=="Intake_Category") 
	{
		$lookupFieldName = "Intake_Category";
		$lookupTable = "set_intake_category";
		$linkFieldName = "Intake_Code";
		...
		{
			if($lookupPSet->getCustomDisplay($lookupFieldName))
				$pageObject->pSet->getSQLQuery()->AddCustomExpression($dispfield, $pageObject->pSet, "student_bill_list2", "Intake_Category");
			$lookupQueryObj = $pageObject->pSet->getSQLQuery()->CloneObject();
			if(!$noBlobReplace)
				$lookupQueryObj->ReplaceFieldsWithDummies($pageObject->pSet->getBinaryFieldsIndices());
		}else{
