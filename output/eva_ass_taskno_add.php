...
if($inlineadd == ADD_INLINE)
	$templatefile = "eva_ass_taskno_inline_add.htm";
else
	$templatefile = "eva_ass_taskno_add.htm";
...
if(isset($_REQUEST['afteradd'])){
		header('Location: eva_ass_taskno_add.php');
...
//	insert masterkey value if exists and if not specified
	if(@$_SESSION[$sessionPrefix."_mastertable"]=="eva_ass_topic")
	{
		if(postvalue("masterkey1"))
			$_SESSION[$sessionPrefix."_masterkey1"] = postvalue("masterkey1");
		
		if($avalues["sub_topic"]==""){
			$avalues["sub_topic"] = prepare_for_db("sub_topic",$_SESSION[$sessionPrefix."_masterkey1"]);
		}
			...
			if($inlineadd==ADD_SIMPLE || $inlineadd==ADD_MASTER)
			{
				...
				if (count($keys))
				{
					...
					if($pageObject->pSet->hasEditPage() && $permis['edit'])
						$message .='&nbsp;<a href=\'eva_ass_taskno_edit.php?'.$keylink.'\'>'."Edit".'</a>&nbsp;';
					if($pageObject->pSet->hasViewPage() && $permis['search'])
						$message .='&nbsp;<a href=\'eva_ass_taskno_view.php?'.$keylink.'\'>'."View".'</a>&nbsp;';
				}
				$mesClass = "mes_ok";	
			}
		}
...
if (no_output_done() && $inlineadd==ADD_SIMPLE && $IsSaved)
{
	...
	// redirect
	header("Location: eva_ass_taskno_".$pageObject->getPageType().".php");
...
if(@$_SESSION[$sessionPrefix."_mastertable"]=="eva_ass_topic")
{
	...
	if(postvalue("mainMPageType")<>"add")
		$defvalues["sub_topic"] = @$_SESSION[$sessionPrefix."_masterkey1"];	
...
if($pageObject->isShowDetailTables && ($inlineadd==ADD_SIMPLE || $inlineadd==ADD_POPUP) && !isMobile())
{
	...
	for($d=0;$d<count($dpParams['ids']);$d++)
	{
		...
		$options['masterTable'] = "eva_ass_taskno";
		$options['firstTime'] = 1;
