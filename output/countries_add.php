...
add_nocache_headers();
include("include/countries_variables.php");
...
$layout->blocks["top"][] = "details";$page_layouts["countries_add"] = $layout;
...
if($inlineadd == ADD_INLINE)
	$templatefile = "countries_inline_add.htm";
else
	$templatefile = "countries_add.htm";
...
if(isset($_REQUEST['afteradd'])){
		header('Location: countries_add.php');
...
if(@$_POST["a"]=="added")
{
	...
	$avalues=array();
	$blobfields=array();
//	processing ccode - start
	$inlineAddOption = true;
	if($inlineAddOption)
	{
		$control_ccode = $pageObject->getControl("ccode", $id);
		$control_ccode->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing ccode - end
...
//	processing ccode - start
			...
			if($inlineAddOption)
			{
				$control_ccode->afterSuccessfulSave();
			}
//	processing ccode - end
//	processing country - start
			$inlineAddOption = true;
			...
			if($inlineadd==ADD_SIMPLE || $inlineadd==ADD_MASTER)
			{
				...
				if (count($keys))
				{
					...
					if($pageObject->pSet->hasEditPage() && $permis['edit'])
						$message .='&nbsp;<a href=\'countries_edit.php?'.$keylink.'\'>'."Edit".'</a>&nbsp;';
					if($pageObject->pSet->hasViewPage() && $permis['search'])
						$message .='&nbsp;<a href=\'countries_view.php?'.$keylink.'\'>'."View".'</a>&nbsp;';
				}
				$mesClass = "mes_ok";	
			}
		}
...
if (no_output_done() && $inlineadd==ADD_SIMPLE && $IsSaved)
{
	...
	// redirect
	header("Location: countries_".$pageObject->getPageType().".php");
...
if(array_key_exists("copyid1",$_REQUEST) || array_key_exists("editid1",$_REQUEST))
{
	...
	{
		$copykeys["ccode"]=postvalue("copyid1");
	}
	else
	{
		$copykeys["ccode"]=postvalue("editid1");
	}
...
if($inlineadd!=ADD_INLINE)
{
	...
	if(!$pageObject->isAppearOnTabs("ccode"))
		$xt->assign("ccode_fieldblock",true);
	else
		$xt->assign("ccode_tabfieldblock",true);
	$xt->assign("ccode_label",true);
	if(isEnableSection508())
		$xt->assign_section("ccode_label","<label for=\"".GetInputElementId("ccode", $id, PAGE_ADD)."\">","</label>");
...
if(@$_POST["a"]=="added" && ($inlineadd == ADD_INLINE || $inlineadd == ADD_MASTER || $inlineadd==ADD_POPUP)) 
{
	...
	$keylink="";
	$keylink.="&key1=".htmlspecialchars(rawurlencode(@$data["ccode"]));
...
//	ccode
	...
	if($display)
	{	
		$value = $pageObject->showDBValue("ccode", $data, $keylink);
		$showValues["ccode"] = $value;
		$showFields[] = "ccode";
	}	
//	country
	$display = false;
	...
	{	
		...
		$showRawValues["ccode"] = substr($data["ccode"],0,100);
