...
$layout->blocks["top"][] = "search";$page_layouts["ojt_company_search"] = $layout;
...
$mode = SEARCH_SIMPLE;
$templatefile = "ojt_company_search.htm";
...
$params['shortTableName'] = 'ojt_company';
...
if ($mode==SEARCH_SIMPLE)
{
	...
	if($pageObject->pSet->getLookupTable("tel"))
		$pageObject->settingsMap["globalSettings"]['shortTNames'][$pageObject->pSet->getLookupTable("tel")] = GetTableURL($pageObject->pSet->getLookupTable("tel"));
	
	$pageObject->fillFieldToolTips("tel");	
	
	$srchFields = $pageObject->searchClauseObj->getSearchCtrlParams("tel");
	$firstFieldParams = array();
	if (count($srchFields))
	{
		$firstFieldParams = $srchFields[0];
	...
	if(isEnableSection508())
		$xt->assign_section("tel_label","<label for=\"".GetInputElementId("tel", $id, PAGE_SEARCH)."\">","</label>");
	else 
		$xt->assign("tel_label", true);
	
	$xt->assign("tel_fieldblock", true);
	$xt->assignbyref("tel_editcontrol", $ctrlBlockArr['searchcontrol']);
	$xt->assign("tel_notbox", $ctrlBlockArr['notbox']);
	// create second control, if need it
	$xt->assignbyref("tel_editcontrol1", $ctrlBlockArr['searchcontrol1']);
	// create search type select
	$xt->assign("searchtype_tel", $ctrlBlockArr['searchtype']);
	$isFieldNeedSecCtrl = $searchControlBuilder->isNeedSecondCtrl("tel");
	$ctrlInd = 0;
	if ($isFieldNeedSecCtrl)
	{
		$pageObject->controlsMap["search"]["searchBlocks"][] = array('fName'=>"tel", 'recId'=>$id, 'ctrlsMap'=>array(0=>$ctrlInd, 1=>($ctrlInd+1)));
		$ctrlInd+=2;
	}
	else
	{
		$pageObject->controlsMap["search"]["searchBlocks"][] = array('fName'=>"tel", 'recId'=>$id, 'ctrlsMap'=>array(0=>$ctrlInd));			
		$ctrlInd++;
	}
	// search fields data
