...
$layout->blocks["top"][] = "details";$page_layouts["w_faq_add"] = $layout;
...
if($inlineadd == ADD_INLINE)
	$templatefile = "w_faq_inline_add.htm";
else
	$templatefile = "w_faq_add.htm";
...
if(@$_POST["a"]=="added")
{
	...
	$avalues=array();
	$blobfields=array();
//	processing question - start
	$inlineAddOption = true;
	$inlineAddOption = $inlineadd!=ADD_INLINE;
	if($inlineAddOption)
	{
		$control_question = $pageObject->getControl("question", $id);
		$control_question->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing question - end
...
//	processing fstatus - start
	$inlineAddOption = true;
	$inlineAddOption = $inlineadd!=ADD_INLINE;
	if($inlineAddOption)
	{
		$control_fstatus = $pageObject->getControl("fstatus", $id);
		$control_fstatus->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
...
//	processing answer - start
	...
	if($inlineAddOption)
	{
		...
		$control_answer->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
...
//	processing question - start
			...
			if($inlineAddOption)
			{
				$control_question->afterSuccessfulSave();
			}
//	processing question - end
//	processing position - start
			$inlineAddOption = true;
			...
			if($inlineadd==ADD_SIMPLE || $inlineadd==ADD_MASTER)
			{
				...
				if (count($keys))
				{
					...
					if($pageObject->pSet->hasEditPage() && $permis['edit'])
						$message .='&nbsp;<a href=\'w_faq_edit.php?'.$keylink.'\'>'."Edit".'</a>&nbsp;';
					if($pageObject->pSet->hasViewPage() && $permis['search'])
						$message .='&nbsp;<a href=\'w_faq_view.php?'.$keylink.'\'>'."View".'</a>&nbsp;';
				}
				$mesClass = "mes_ok";	
			}
		}
...
if (no_output_done() && $inlineadd==ADD_SIMPLE && $IsSaved)
{
	...
	// redirect
	header("Location: w_faq_".$pageObject->getPageType().".php");
...
if(array_key_exists("copyid1",$_REQUEST) || array_key_exists("editid1",$_REQUEST))
{
	...
	{
		$copykeys["faqid"]=postvalue("copyid1");
	}
	else
	{
		$copykeys["faqid"]=postvalue("editid1");
	}
...
if($readavalues)
{
	$defvalues["question"]=@$avalues["question"];
	$defvalues["answer"]=@$avalues["answer"];
	$defvalues["position"]=@$avalues["position"];
	$defvalues["fstatus"]=@$avalues["fstatus"];
...
if($inlineadd!=ADD_INLINE)
{
	...
	if(!$pageObject->isAppearOnTabs("question"))
		$xt->assign("question_fieldblock",true);
	else
		$xt->assign("question_tabfieldblock",true);
	$xt->assign("question_label",true);
	if(isEnableSection508())
		$xt->assign_section("question_label","<label for=\"".GetInputElementId("question", $id, PAGE_ADD)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("answer_label","<label for=\"".GetInputElementId("answer", $id, PAGE_ADD)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("fstatus_label","<label for=\"".GetInputElementId("fstatus", $id, PAGE_ADD)."\">","</label>");
...
if(@$_POST["a"]=="added" && ($inlineadd == ADD_INLINE || $inlineadd == ADD_MASTER || $inlineadd==ADD_POPUP)) 
{
	...
	$keylink="";
	$keylink.="&key1=".htmlspecialchars(rawurlencode(@$data["faqid"]));
...
//	question
	...
	if($display)
	{	
		$value = $pageObject->showDBValue("question", $data, $keylink);
		$showValues["question"] = $value;
		$showFields[] = "question";
	}	
//	answer
	$display = false;
...
//	category
	...
	{	
		...
		$showRawValues["faqid"] = substr($data["faqid"],0,100);
		$showRawValues["question"] = substr($data["question"],0,100);
