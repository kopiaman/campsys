...
include("include/Report_Absent_variables.php");
...
$layout->blocks["top"][] = "export";$page_layouts["Report_Absent_export"] = $layout;
...
$xt->display("Report_Absent_export.htm");

function ExportToExcel_old($cipherer)
{
	global $cCharset;
	header("Content-Type: application/vnd.ms-excel");
	header("Content-Disposition: attachment;Filename=Report_Absent.xls");
...
function ExportToWord($cipherer)
{
	...
	header("Content-Disposition: attachment;Filename=Report_Absent.doc");
...
function ExportToXML($cipherer)
{
	...
	header("Content-Disposition: attachment;Filename=Report_Absent.xml");
	...
	while((!$nPageSize || $i<$nPageSize) && $row)
	{
		$values = array();
			...
			$values["reason"] = $pageObject->showDBValue("reason", $row);
			$values["ABS"] = $pageObject->showDBValue("ABS", $row);
			$values["total_hour"] = $pageObject->showDBValue("total_hour", $row);
...
function ExportToCSV($cipherer)
{
	...
	header("Content-Disposition: attachment;Filename=Report_Absent.csv");
...
// write data rows
	...
	while((!$nPageSize || $iNumberOfRows < $nPageSize) && $row)
	{
		$values = array();
			...
			$values["reason"] = $pageObject->getViewControl("reason")->showDBValue($row, "");
			$values["ABS"] = $row["ABS"];
			$values["total_hour"] = $pageObject->getViewControl("total_hour")->showDBValue($row, "");
		...
		if ($eventRes)
		{
			...
			if($outstr!="")
				$outstr.=",";
			$outstr.='"'.str_replace('"', '""', $values["ABS"]).'"';
			if($outstr!="")
				$outstr.=",";
			$outstr.='"'.str_replace('"', '""', $values["total_hour"]).'"';
			if($outstr!="")
				$outstr.=",";
...
// write header
	...
	if($_REQUEST["type"]=="excel")
	{
		...
		echo '<td style="width: 100" x:str>'.PrepareForExcel("ABS").'</td>';	
		echo '<td style="width: 100" x:str>'.PrepareForExcel("Total Hour").'</td>';	
...
// write data rows
	...
	while((!$nPageSize || $iNumberOfRows<$nPageSize) && $row)
	{
		...
		$values = array();
					...
					$values["courseID"] = $pageObject->getViewControl("courseID")->showDBValue($row, "");
					$values["reason"] = $pageObject->getViewControl("reason")->showDBValue($row, "");
					$values["ABS"] = $pageObject->getViewControl("ABS")->showDBValue($row, "");
					$values["total_hour"] = $pageObject->getViewControl("total_hour")->showDBValue($row, "");
					$values["StudentID"] = $pageObject->getViewControl("StudentID")->showDBValue($row, "");
