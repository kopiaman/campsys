...
include("include/Chart_Exam_Internal_variables.php");
...
$layout->blocks["top"][] = "search";$page_layouts["Chart_Exam_Internal_search"] = $layout;
...
$mode = SEARCH_SIMPLE;
$templatefile = "Chart_Exam_Internal_search.htm";
...
$params['shortTableName'] = 'Chart_Exam_Internal';
...
if ($mode==SEARCH_SIMPLE)
{
	...
	if($pageObject->pSet->getLookupTable("program_course"))
		$pageObject->settingsMap["globalSettings"]['shortTNames'][$pageObject->pSet->getLookupTable("program_course")] = GetTableURL($pageObject->pSet->getLookupTable("program_course"));
	
	$pageObject->fillFieldToolTips("program_course");	
	
	$srchFields = $pageObject->searchClauseObj->getSearchCtrlParams("program_course");
	$firstFieldParams = array();
	if (count($srchFields))
	{
		$firstFieldParams = $srchFields[0];
	...
	{
		$firstFieldParams['fName'] = "program_course";
		$firstFieldParams['eType'] = '';
		$firstFieldParams['value1'] = '';
		$firstFieldParams['opt'] = '';
		$firstFieldParams['value2'] = '';
	...
	if(isEnableSection508())
		$xt->assign_section("program_course_label","<label for=\"".GetInputElementId("program_course", $id, PAGE_SEARCH)."\">","</label>");
	else 
		$xt->assign("program_course_label", true);
	
	$xt->assign("program_course_fieldblock", true);
	$xt->assignbyref("program_course_editcontrol", $ctrlBlockArr['searchcontrol']);
	$xt->assign("program_course_notbox", $ctrlBlockArr['notbox']);
	// create second control, if need it
	$xt->assignbyref("program_course_editcontrol1", $ctrlBlockArr['searchcontrol1']);
	// create search type select
	$xt->assign("searchtype_program_course", $ctrlBlockArr['searchtype']);
	$isFieldNeedSecCtrl = $searchControlBuilder->isNeedSecondCtrl("program_course");
	$ctrlInd = 0;
	if ($isFieldNeedSecCtrl)
	{
		$pageObject->controlsMap["search"]["searchBlocks"][] = array('fName'=>"program_course", 'recId'=>$id, 'ctrlsMap'=>array(0=>$ctrlInd, 1=>($ctrlInd+1)));
		$ctrlInd+=2;
	}
	else
	{
		$pageObject->controlsMap["search"]["searchBlocks"][] = array('fName'=>"program_course", 'recId'=>$id, 'ctrlsMap'=>array(0=>$ctrlInd));			
		$ctrlInd++;
	}
	// search fields data
