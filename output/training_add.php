...
$layout->blocks["top"][] = "details";$page_layouts["training_add"] = $layout;
...
if(@$_POST["a"]=="added")
{
	...
	$avalues=array();
	$blobfields=array();
//	processing TrainingName - start
	$inlineAddOption = true;
	if($inlineAddOption)
	{
		$control_TrainingName = $pageObject->getControl("TrainingName", $id);
		$control_TrainingName->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing TrainingName - end
...
//	processing TrainingName - start
			...
			if($inlineAddOption)
			{
				$control_TrainingName->afterSuccessfulSave();
			}
//	processing TrainingName - end
...
if(array_key_exists("copyid1",$_REQUEST) || array_key_exists("editid1",$_REQUEST))
{
	...
	{
		$copykeys["TrainingID"]=postvalue("copyid1");
	}
	else
	{
		$copykeys["TrainingID"]=postvalue("editid1");
	}
...
if($readavalues)
{
	$defvalues["TrainingName"]=@$avalues["TrainingName"];
}
...
if($inlineadd!=ADD_INLINE)
{
	...
	if(!$pageObject->isAppearOnTabs("TrainingName"))
		$xt->assign("TrainingName_fieldblock",true);
	else
		$xt->assign("TrainingName_tabfieldblock",true);
	$xt->assign("TrainingName_label",true);
	if(isEnableSection508())
		$xt->assign_section("TrainingName_label","<label for=\"".GetInputElementId("TrainingName", $id, PAGE_ADD)."\">","</label>");
...
if(@$_POST["a"]=="added" && ($inlineadd == ADD_INLINE || $inlineadd == ADD_MASTER || $inlineadd==ADD_POPUP)) 
{
	...
	$keylink="";
	$keylink.="&key1=".htmlspecialchars(rawurlencode(@$data["TrainingID"]));
...
//	TrainingName
	...
	if($display)
	{	
		$value = $pageObject->showDBValue("TrainingName", $data, $keylink);
		$showValues["TrainingName"] = $value;
		$showFields[] = "TrainingName";
	}	
		$showRawValues["TrainingID"] = substr($data["TrainingID"],0,100);
		$showRawValues["TrainingName"] = substr($data["TrainingName"],0,100);
...
function GetAddedDataLookupQuery($pageObject, $keys, $forLookup)
{
	...
	if(postvalue("table")=="staff_compulsory_training" && postvalue("field")=="TrainingID") 
	{
		$lookupFieldName = "TrainingID";
		$lookupTable = "training";
		$linkFieldName = "TrainingID";
		$dispfield = "TrainingName";
		...
		if($nLookupType == LT_QUERY)
		{
			if($lookupPSet->getCustomDisplay($lookupFieldName))
				$pageObject->pSet->getSQLQuery()->AddCustomExpression($dispfield, $pageObject->pSet, "staff_compulsory_training", "TrainingID");
			...
			$LookupSQL = "select ";
			$LookupSQL .= GetFullFieldName("TrainingID", $strTableName, true);
					if($linkFieldName != $dispfield)
				$LookupSQL .= ",TrainingName";
		}
	}
	if(postvalue("table")=="Report_Compulsory_Training" && postvalue("field")=="TrainingID") 
	{
		$lookupFieldName = "TrainingID";
		$lookupTable = "training";
		$linkFieldName = "TrainingID";
		...
		{
			if($lookupPSet->getCustomDisplay($lookupFieldName))
				$pageObject->pSet->getSQLQuery()->AddCustomExpression($dispfield, $pageObject->pSet, "Report_Compulsory_Training", "TrainingID");
			$lookupQueryObj = $pageObject->pSet->getSQLQuery()->CloneObject();
			if(!$noBlobReplace)
				$lookupQueryObj->ReplaceFieldsWithDummies($pageObject->pSet->getBinaryFieldsIndices());
		}else{
	...
	if(postvalue("table")=="Report_Staff_Training" && postvalue("field")=="TrainingID") 
	{
		$lookupFieldName = "TrainingID";
		$lookupTable = "training";
		$linkFieldName = "TrainingID";
		...
		{
			if($lookupPSet->getCustomDisplay($lookupFieldName))
				$pageObject->pSet->getSQLQuery()->AddCustomExpression($dispfield, $pageObject->pSet, "Report_Staff_Training", "TrainingID");
			$lookupQueryObj = $pageObject->pSet->getSQLQuery()->CloneObject();
			if(!$noBlobReplace)
				$lookupQueryObj->ReplaceFieldsWithDummies($pageObject->pSet->getBinaryFieldsIndices());
		}else{
