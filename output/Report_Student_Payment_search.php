...
include("include/Report_Student_Payment_variables.php");
...
$layout->blocks["top"][] = "search";$page_layouts["Report_Student_Payment_search"] = $layout;
...
$mode = SEARCH_SIMPLE;
$templatefile = "Report_Student_Payment_search.htm";
...
$params['shortTableName'] = 'Report_Student_Payment';
...
if ($mode==SEARCH_SIMPLE)
{
	...
	if($pageObject->pSet->getLookupTable("pay_date"))
		$pageObject->settingsMap["globalSettings"]['shortTNames'][$pageObject->pSet->getLookupTable("pay_date")] = GetTableURL($pageObject->pSet->getLookupTable("pay_date"));
	
	$pageObject->fillFieldToolTips("pay_date");	
	
	$srchFields = $pageObject->searchClauseObj->getSearchCtrlParams("pay_date");
	$firstFieldParams = array();
	if (count($srchFields))
	{
		$firstFieldParams = $srchFields[0];
	...
	{
		$firstFieldParams['fName'] = "pay_date";
		$firstFieldParams['eType'] = '';
		$firstFieldParams['value1'] = '';
		$firstFieldParams['opt'] = '';
		$firstFieldParams['value2'] = '';
	...
	if(isEnableSection508())
		$xt->assign_section("pay_date_label","<label for=\"".GetInputElementId("pay_date", $id, PAGE_SEARCH)."\">","</label>");
	else 
		$xt->assign("pay_date_label", true);
	
	$xt->assign("pay_date_fieldblock", true);
	$xt->assignbyref("pay_date_editcontrol", $ctrlBlockArr['searchcontrol']);
	$xt->assign("pay_date_notbox", $ctrlBlockArr['notbox']);
	// create second control, if need it
	$xt->assignbyref("pay_date_editcontrol1", $ctrlBlockArr['searchcontrol1']);
	// create search type select
	$xt->assign("searchtype_pay_date", $ctrlBlockArr['searchtype']);
	$isFieldNeedSecCtrl = $searchControlBuilder->isNeedSecondCtrl("pay_date");
	$ctrlInd = 0;
	if ($isFieldNeedSecCtrl)
	{
		$pageObject->controlsMap["search"]["searchBlocks"][] = array('fName'=>"pay_date", 'recId'=>$id, 'ctrlsMap'=>array(0=>$ctrlInd, 1=>($ctrlInd+1)));
		$ctrlInd+=2;
	}
	else
	{
		$pageObject->controlsMap["search"]["searchBlocks"][] = array('fName'=>"pay_date", 'recId'=>$id, 'ctrlsMap'=>array(0=>$ctrlInd));			
		$ctrlInd++;
	}
	// search fields data
	...
	if($pageObject->pSet->getLookupTable("terms"))
		$pageObject->settingsMap["globalSettings"]['shortTNames'][$pageObject->pSet->getLookupTable("terms")] = GetTableURL($pageObject->pSet->getLookupTable("terms"));
	...
	if(isEnableSection508())
		$xt->assign_section("terms_label","<label for=\"".GetInputElementId("terms", $id, PAGE_SEARCH)."\">","</label>");
	else 
	...
	if($pageObject->pSet->getLookupTable("ref_no"))
		$pageObject->settingsMap["globalSettings"]['shortTNames'][$pageObject->pSet->getLookupTable("ref_no")] = GetTableURL($pageObject->pSet->getLookupTable("ref_no"));
	
	$pageObject->fillFieldToolTips("ref_no");	
	
	$srchFields = $pageObject->searchClauseObj->getSearchCtrlParams("ref_no");
	$firstFieldParams = array();
	if (count($srchFields))
	{
		$firstFieldParams = $srchFields[0];
	...
	if(isEnableSection508())
		$xt->assign_section("ref_no_label","<label for=\"".GetInputElementId("ref_no", $id, PAGE_SEARCH)."\">","</label>");
	else 
		$xt->assign("ref_no_label", true);
	
	$xt->assign("ref_no_fieldblock", true);
	$xt->assignbyref("ref_no_editcontrol", $ctrlBlockArr['searchcontrol']);
	$xt->assign("ref_no_notbox", $ctrlBlockArr['notbox']);
	// create second control, if need it
	$xt->assignbyref("ref_no_editcontrol1", $ctrlBlockArr['searchcontrol1']);
	// create search type select
	$xt->assign("searchtype_ref_no", $ctrlBlockArr['searchtype']);
	$isFieldNeedSecCtrl = $searchControlBuilder->isNeedSecondCtrl("ref_no");
	$ctrlInd = 0;
	if ($isFieldNeedSecCtrl)
	{
		$pageObject->controlsMap["search"]["searchBlocks"][] = array('fName'=>"ref_no", 'recId'=>$id, 'ctrlsMap'=>array(0=>$ctrlInd, 1=>($ctrlInd+1)));
		$ctrlInd+=2;
	}
	else
	{
		$pageObject->controlsMap["search"]["searchBlocks"][] = array('fName'=>"ref_no", 'recId'=>$id, 'ctrlsMap'=>array(0=>$ctrlInd));			
		$ctrlInd++;
	}
