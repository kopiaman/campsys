...
include("include/support_variables.php");
...
$layout->blocks["top"][] = "details";$page_layouts["support_edit"] = $layout;
...
//	processing topic - begin
	$condition = $inlineedit!=EDIT_INLINE;//(!$inlineedit) edit simple mode

	if($condition)
	{
		$control_topic = $pageObject->getControl("topic", $id);
		$control_topic->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing topic - end
...
//	processing inquiry - begin
	$condition = $inlineedit!=EDIT_INLINE;//(!$inlineedit) edit simple mode

	if($condition)
	{
		$control_inquiry = $pageObject->getControl("inquiry", $id);
		$control_inquiry->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing inquiry - end
...
//	processing answer - begin
	$condition = $inlineedit!=EDIT_INLINE;//(!$inlineedit) edit simple mode

	if($condition)
	{
		$control_answer = $pageObject->getControl("answer", $id);
		$control_answer->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing answer - end
...
//	processing stitle - begin
	$condition = $inlineedit!=EDIT_INLINE;//(!$inlineedit) edit simple mode

	if($condition)
	{
		$control_stitle = $pageObject->getControl("stitle", $id);
		$control_stitle->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing stitle - end
...
//	processing attachAns - begin
	$condition = $inlineedit!=EDIT_INLINE;//(!$inlineedit) edit simple mode

	if($condition)
	{
		$control_attachAns = $pageObject->getControl("attachAns", $id);
		$control_attachAns->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing attachAns - end
	...
	//	do event
		...
		{		
			...
			//	processing topic - begin
				...
				if($condition)
				{
					$control_topic->afterSuccessfulSave();
				}
	//	processing topic - end
			//	processing staff - begin
							$condition = $inlineedit!=EDIT_INLINE;//(!$inlineedit) edit simple mode
	...
	//	processing staff - end
			//	processing inquiry - begin
				...
				if($condition)
				{
					$control_inquiry->afterSuccessfulSave();
				}
	//	processing inquiry - end
			//	processing sstatus - begin
							$condition = $inlineedit!=EDIT_INLINE;//(!$inlineedit) edit simple mode
	...
	//	processing sstatus - end
			//	processing answer - begin
				...
				if($condition)
				{
					$control_answer->afterSuccessfulSave();
				}
	//	processing answer - end
			//	processing sdate - begin
							$condition = $inlineedit!=EDIT_INLINE;//(!$inlineedit) edit simple mode
	...
	//	processing sdate - end
			//	processing stitle - begin
				...
				if($condition)
				{
					$control_stitle->afterSuccessfulSave();
				}
	//	processing stitle - end
			//	processing attach - begin
							$condition = $inlineedit!=EDIT_INLINE;//(!$inlineedit) edit simple mode
	...
	//	processing attach - end
			//	processing attachAns - begin
				...
				if($condition)
				{
					$control_attachAns->afterSuccessfulSave();
				}
	//	processing attachAns - end
...
if ($IsSaved && no_output_done() && $inlineedit == EDIT_SIMPLE)
{
	...
	// redirect
	header("Location: support_".$pageObject->getPageType().".php?".$keyGetQ);
...
if(!$data)
{
	...
	{
		header("Location: support_list.php?a=return");
		exit();
	}
	else
		$data = array();
...
if($inlineedit != EDIT_INLINE)
{
	...
	if(!$pageObject->isAppearOnTabs("topic"))
		$xt->assign("topic_fieldblock",true);
	else
		$xt->assign("topic_tabfieldblock",true);
	$xt->assign("topic_label",true);
	if(isEnableSection508())
		$xt->assign_section("topic_label","<label for=\"".GetInputElementId("topic", $id, PAGE_EDIT)."\">","</label>");
		
	if(!$pageObject->isAppearOnTabs("staff"))
		$xt->assign("staff_fieldblock",true);
	else
		$xt->assign("staff_tabfieldblock",true);
	$xt->assign("staff_label",true);
	if(isEnableSection508())
		$xt->assign_section("staff_label","<label for=\"".GetInputElementId("staff", $id, PAGE_EDIT)."\">","</label>");
		
	if(!$pageObject->isAppearOnTabs("inquiry"))
		$xt->assign("inquiry_fieldblock",true);
	else
		$xt->assign("inquiry_tabfieldblock",true);
	$xt->assign("inquiry_label",true);
	if(isEnableSection508())
		$xt->assign_section("inquiry_label","<label for=\"".GetInputElementId("inquiry", $id, PAGE_EDIT)."\">","</label>");
	...
	if(!$pageObject->isAppearOnTabs("answer"))
		$xt->assign("answer_fieldblock",true);
	else
		$xt->assign("answer_tabfieldblock",true);
	$xt->assign("answer_label",true);
	if(isEnableSection508())
		$xt->assign_section("answer_label","<label for=\"".GetInputElementId("answer", $id, PAGE_EDIT)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("sdate_label","<label for=\"".GetInputElementId("sdate", $id, PAGE_EDIT)."\">","</label>");
		
	if(!$pageObject->isAppearOnTabs("stitle"))
		$xt->assign("stitle_fieldblock",true);
	else
		$xt->assign("stitle_tabfieldblock",true);
	$xt->assign("stitle_label",true);
	if(isEnableSection508())
		$xt->assign_section("stitle_label","<label for=\"".GetInputElementId("stitle", $id, PAGE_EDIT)."\">","</label>");
	...
	if(!$pageObject->isAppearOnTabs("attachAns"))
		$xt->assign("attachAns_fieldblock",true);
	else
		$xt->assign("attachAns_tabfieldblock",true);
	$xt->assign("attachAns_label",true);
	if(isEnableSection508())
		$xt->assign_section("attachAns_label","<label for=\"".GetInputElementId("attachAns", $id, PAGE_EDIT)."\">","</label>");
...
$xt->assign("viewlink_attrs","id=\"viewButton".$id."\" name=\"viewButton".$id."\" onclick=\"window.location.href='support_view.php?".$viewlink."'\"");
