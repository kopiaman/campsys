...
$templatefile = ($inlineedit == EDIT_INLINE) ? "Report_Student_Course_inline_edit.htm" : "Report_Student_Course_edit.htm";
...
//	processing e1 - begin
	...
	if($condition)
	{
		$control_e1 = $pageObject->getControl("e1", $id);
		$control_e1->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);
...
//	processing e2 - begin
	...
	if($condition)
	{
		$control_e2 = $pageObject->getControl("e2", $id);
		$control_e2->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);
...
//	processing e3 - begin
	...
	if($condition)
	{
		$control_e3 = $pageObject->getControl("e3", $id);
		$control_e3->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);
...
//	processing e4 - begin
	...
	if($condition)
	{
		$control_e4 = $pageObject->getControl("e4", $id);
		$control_e4->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);
...
//	processing e4 - end
	...
	//	processing CourseID - end
			//	processing e1 - begin
				...
				if($condition)
				{
					$control_e1->afterSuccessfulSave();
				}
	//	processing e1 - end
			//	processing e2 - begin
							$condition = $inlineedit==EDIT_INLINE;//($inlineedit) inline mode
				...
				if($condition)
				{
					$control_e2->afterSuccessfulSave();
				}
	//	processing e2 - end
			//	processing e3 - begin
							$condition = $inlineedit==EDIT_INLINE;//($inlineedit) inline mode
				...
				if($condition)
				{
					$control_e3->afterSuccessfulSave();
				}
	//	processing e3 - end
			//	processing e4 - begin
							$condition = $inlineedit==EDIT_INLINE;//($inlineedit) inline mode
				...
				if($condition)
				{
					$control_e4->afterSuccessfulSave();
				}
	//	processing e4 - end
...
if ($IsSaved && no_output_done() && $inlineedit == EDIT_SIMPLE)
{
	...
	// redirect
	header("Location: Report_Student_Course_".$pageObject->getPageType().".php?".$keyGetQ);
...
if(!$data)
{
	...
	{
		header("Location: Report_Student_Course_list.php?a=return");
		exit();
	}
	else
		$data = array();
...
//	e1 - 
	$value = $pageObject->showDBValue("e1", $data, $keylink);
	$showValues["e1"] = $value;
	$showFields[] = "e1";
		$showRawValues["e1"] = substr($data["e1"],0,100);

//	e2 - 
	$value = $pageObject->showDBValue("e2", $data, $keylink);
	$showValues["e2"] = $value;
	$showFields[] = "e2";
		$showRawValues["e2"] = substr($data["e2"],0,100);

//	e3 - 
	$value = $pageObject->showDBValue("e3", $data, $keylink);
	$showValues["e3"] = $value;
	$showFields[] = "e3";
		$showRawValues["e3"] = substr($data["e3"],0,100);

//	e4 - 
	$value = $pageObject->showDBValue("e4", $data, $keylink);
	$showValues["e4"] = $value;
	$showFields[] = "e4";
		$showRawValues["e4"] = substr($data["e4"],0,100);
...
//	Eligible - 
	$value = $pageObject->showDBValue("Eligible", $data, $keylink);
	$showValues["Eligible"] = $value;
	$showFields[] = "Eligible";
		$showRawValues["Eligible"] = substr($data["Eligible"],0,100);
/////////////////////////////////////////////////////////////
//	start inline output
/////////////////////////////////////////////////////////////
...
if($pageObject->isShowDetailTables && $inlineedit!=EDIT_INLINE && !isMobile())
{
	...
	for($d=0;$d<count($dpParams['ids']);$d++)
	{
		...
		$options['masterTable'] = "Report_Student_Course";
		$options['firstTime'] = 1;
...
$xt->assign("viewlink_attrs","id=\"viewButton".$id."\" name=\"viewButton".$id."\" onclick=\"window.location.href='Report_Student_Course_view.php?".$viewlink."'\"");
