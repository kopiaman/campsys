...
$templatefile = ($inlineedit == EDIT_INLINE) ? "student_bill_statement_inline_edit.htm" : "student_bill_statement_edit.htm";
...
//	processing statement - begin
	$condition = $inlineedit==EDIT_INLINE;//($inlineedit) inline mode

	if($condition)
	{
		$control_statement = $pageObject->getControl("statement", $id);
		$control_statement->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing statement - end
	...
	//	do event
		...
		{		
			...
			//	processing statement - begin
				...
				if($condition)
				{
					$control_statement->afterSuccessfulSave();
				}
	//	processing statement - end
			//	processing dates - begin
							$condition = $inlineedit==EDIT_INLINE;//($inlineedit) inline mode
...
if ($IsSaved && no_output_done() && $inlineedit == EDIT_SIMPLE)
{
	...
	// redirect
	header("Location: student_bill_statement_".$pageObject->getPageType().".php?".$keyGetQ);
...
if($readevalues)
{
	$data["statement"] = $evalues["statement"];
...
//	statement - Document Download
	...
	$showValues["statement"] = $value;
	$showFields[] = "statement";
		$showRawValues["statement"] = substr($data["statement"],0,100);
...
$xt->assign("viewlink_attrs","id=\"viewButton".$id."\" name=\"viewButton".$id."\" onclick=\"window.location.href='student_bill_statement_view.php?".$viewlink."'\"");
