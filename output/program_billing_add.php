...
if($inlineadd == ADD_INLINE)
	$templatefile = "program_billing_inline_add.htm";
else
	$templatefile = "program_billing_add.htm";
...
if(isset($_REQUEST['afteradd'])){
		header('Location: program_billing_add.php');
...
if(@$_POST["a"]=="added")
{
	...
	$avalues=array();
	$blobfields=array();
//	processing item - start
	$inlineAddOption = true;
	$inlineAddOption = $inlineadd==ADD_INLINE;
	if($inlineAddOption)
	{
		$control_item = $pageObject->getControl("item", $id);
		$control_item->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing item - end
...
//	insert masterkey value if exists and if not specified
	if(@$_SESSION[$sessionPrefix."_mastertable"]=="Program_Batch_Bill")
	{
			...
			if($inlineadd==ADD_SIMPLE || $inlineadd==ADD_MASTER)
			{
				...
				if (count($keys))
				{
					...
					if($pageObject->pSet->hasEditPage() && $permis['edit'])
						$message .='&nbsp;<a href=\'program_billing_edit.php?'.$keylink.'\'>'."Edit".'</a>&nbsp;';
					if($pageObject->pSet->hasViewPage() && $permis['search'])
						$message .='&nbsp;<a href=\'program_billing_view.php?'.$keylink.'\'>'."View".'</a>&nbsp;';
				}
				$mesClass = "mes_ok";	
			}
		}
...
if (no_output_done() && $inlineadd==ADD_SIMPLE && $IsSaved)
{
	...
	// redirect
	header("Location: program_billing_".$pageObject->getPageType().".php");
...
if(array_key_exists("copyid1",$_REQUEST) || array_key_exists("editid1",$_REQUEST))
{
	...
	{
		$copykeys["proid"]=postvalue("copyid1");
	}
	else
	{
		$copykeys["proid"]=postvalue("editid1");
	}
