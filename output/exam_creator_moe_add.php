...
add_nocache_headers();
include("include/exam_creator_moe_variables.php");
...
$layout->blocks["top"][] = "details";$page_layouts["exam_creator_moe_add"] = $layout;
...
if($inlineadd == ADD_INLINE)
	$templatefile = "exam_creator_moe_inline_add.htm";
else
	$templatefile = "exam_creator_moe_add.htm";
...
if(isset($_REQUEST['afteradd'])){
		header('Location: exam_creator_moe_add.php');
...
if(@$_POST["a"]=="added")
{
	...
	$avalues=array();
	$blobfields=array();
//	processing types - start
	$inlineAddOption = true;
	$inlineAddOption = $inlineadd!=ADD_INLINE;
	if($inlineAddOption)
	{
		$control_types = $pageObject->getControl("types", $id);
		$control_types->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing types - end
//	processing date_memo - start
	$inlineAddOption = true;
	$inlineAddOption = $inlineadd!=ADD_INLINE;
	if($inlineAddOption)
	{
		$control_date_memo = $pageObject->getControl("date_memo", $id);
		$control_date_memo->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing date_memo - end
//	processing ref - start
	$inlineAddOption = true;
	$inlineAddOption = $inlineadd!=ADD_INLINE;
	if($inlineAddOption)
	{
		$control_ref = $pageObject->getControl("ref", $id);
		$control_ref->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing ref - end
//	processing exam_date - start
	$inlineAddOption = true;
	$inlineAddOption = $inlineadd!=ADD_INLINE;
	if($inlineAddOption)
	{
		$control_exam_date = $pageObject->getControl("exam_date", $id);
		$control_exam_date->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing exam_date - end
//	processing stime - start
	$inlineAddOption = true;
	$inlineAddOption = $inlineadd!=ADD_INLINE;
	if($inlineAddOption)
	{
		$control_stime = $pageObject->getControl("stime", $id);
		$control_stime->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing stime - end
//	processing etime - start
	$inlineAddOption = true;
	$inlineAddOption = $inlineadd!=ADD_INLINE;
	if($inlineAddOption)
	{
		$control_etime = $pageObject->getControl("etime", $id);
		$control_etime->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing etime - end
//	processing exam_venue - start
	$inlineAddOption = true;
	$inlineAddOption = $inlineadd!=ADD_INLINE;
	if($inlineAddOption)
	{
		$control_exam_venue = $pageObject->getControl("exam_venue", $id);
		$control_exam_venue->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing exam_venue - end
//	processing program - start
	...
	if($inlineAddOption)
	{
		...
		$control_program->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
...
//	processing batch - start
	...
	if($inlineAddOption)
	{
		...
		$control_batch->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
...
//	processing course - start
	$inlineAddOption = true;
	$inlineAddOption = $inlineadd!=ADD_INLINE;
	if($inlineAddOption)
	{
		$control_course = $pageObject->getControl("course", $id);
		$control_course->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing course - end
...
//	processing types - start
			...
			if($inlineAddOption)
			{
				$control_types->afterSuccessfulSave();
			}
//	processing types - end
//	processing date_memo - start
			$inlineAddOption = true;
			...
			if($inlineAddOption)
			{
				$control_date_memo->afterSuccessfulSave();
			}
//	processing date_memo - end
//	processing ref - start
			$inlineAddOption = true;
			...
			if($inlineAddOption)
			{
				$control_ref->afterSuccessfulSave();
			}
//	processing ref - end
//	processing exam_date - start
			$inlineAddOption = true;
			...
			if($inlineAddOption)
			{
				$control_exam_date->afterSuccessfulSave();
			}
//	processing exam_date - end
//	processing stime - start
			$inlineAddOption = true;
			...
			if($inlineAddOption)
			{
				$control_stime->afterSuccessfulSave();
			}
//	processing stime - end
//	processing etime - start
			$inlineAddOption = true;
			...
			if($inlineAddOption)
			{
				$control_etime->afterSuccessfulSave();
			}
//	processing etime - end
//	processing exam_venue - start
			$inlineAddOption = true;
			...
			if($inlineAddOption)
			{
				$control_exam_venue->afterSuccessfulSave();
			}
//	processing exam_venue - end
//	processing program - start
			$inlineAddOption = true;
...
//	processing course - start
			...
			if($inlineAddOption)
			{
				$control_course->afterSuccessfulSave();
			}
//	processing course - end
			...
			if($inlineadd==ADD_SIMPLE || $inlineadd==ADD_MASTER)
			{
				...
				if (count($keys))
				{
					...
					if($pageObject->pSet->hasEditPage() && $permis['edit'])
						$message .='&nbsp;<a href=\'exam_creator_moe_edit.php?'.$keylink.'\'>'."Edit".'</a>&nbsp;';
					if($pageObject->pSet->hasViewPage() && $permis['search'])
						$message .='&nbsp;<a href=\'exam_creator_moe_view.php?'.$keylink.'\'>'."View".'</a>&nbsp;';
				}
				$mesClass = "mes_ok";	
			}
		}
...
if (no_output_done() && $inlineadd==ADD_SIMPLE && $IsSaved)
{
	...
	// redirect
	header("Location: exam_creator_moe_".$pageObject->getPageType().".php");
...
{
	$defvalues["types"] = "MOE";
	$defvalues["ref"] = "MATA/KUA/ED-1/MEMO-39/2013";
}
...
if($readavalues)
{
	$defvalues["date_memo"]=@$avalues["date_memo"];
	$defvalues["ref"]=@$avalues["ref"];
	$defvalues["exam_date"]=@$avalues["exam_date"];
	$defvalues["exam_venue"]=@$avalues["exam_venue"];
	$defvalues["stime"]=@$avalues["stime"];
	$defvalues["etime"]=@$avalues["etime"];
	$defvalues["program"]=@$avalues["program"];
	$defvalues["batch"]=@$avalues["batch"];
	$defvalues["class"]=@$avalues["class"];
	$defvalues["course"]=@$avalues["course"];
	$defvalues["types"]=@$avalues["types"];
}
...
if($inlineadd!=ADD_INLINE)
{
	...
	if(!$pageObject->isAppearOnTabs("date_memo"))
		$xt->assign("date_memo_fieldblock",true);
	else
		$xt->assign("date_memo_tabfieldblock",true);
	$xt->assign("date_memo_label",true);
	if(isEnableSection508())
		$xt->assign_section("date_memo_label","<label for=\"".GetInputElementId("date_memo", $id, PAGE_ADD)."\">","</label>");
	
	if(!$pageObject->isAppearOnTabs("ref"))
		$xt->assign("ref_fieldblock",true);
	else
		$xt->assign("ref_tabfieldblock",true);
	$xt->assign("ref_label",true);
	if(isEnableSection508())
		$xt->assign_section("ref_label","<label for=\"".GetInputElementId("ref", $id, PAGE_ADD)."\">","</label>");
	
	if(!$pageObject->isAppearOnTabs("exam_date"))
		$xt->assign("exam_date_fieldblock",true);
	else
		$xt->assign("exam_date_tabfieldblock",true);
	$xt->assign("exam_date_label",true);
	if(isEnableSection508())
		$xt->assign_section("exam_date_label","<label for=\"".GetInputElementId("exam_date", $id, PAGE_ADD)."\">","</label>");
	
	if(!$pageObject->isAppearOnTabs("exam_venue"))
		$xt->assign("exam_venue_fieldblock",true);
	else
		$xt->assign("exam_venue_tabfieldblock",true);
	$xt->assign("exam_venue_label",true);
	if(isEnableSection508())
		$xt->assign_section("exam_venue_label","<label for=\"".GetInputElementId("exam_venue", $id, PAGE_ADD)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("stime_label","<label for=\"".GetInputElementId("stime", $id, PAGE_ADD)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("etime_label","<label for=\"".GetInputElementId("etime", $id, PAGE_ADD)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("program_label","<label for=\"".GetInputElementId("program", $id, PAGE_ADD)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("batch_label","<label for=\"".GetInputElementId("batch", $id, PAGE_ADD)."\">","</label>");
	...
	if(!$pageObject->isAppearOnTabs("course"))
		$xt->assign("course_fieldblock",true);
	else
		$xt->assign("course_tabfieldblock",true);
	$xt->assign("course_label",true);
	if(isEnableSection508())
		$xt->assign_section("course_label","<label for=\"".GetInputElementId("course", $id, PAGE_ADD)."\">","</label>");
	
	if(!$pageObject->isAppearOnTabs("types"))
		$xt->assign("types_fieldblock",true);
	else
		$xt->assign("types_tabfieldblock",true);
	$xt->assign("types_label",true);
	if(isEnableSection508())
		$xt->assign_section("types_label","<label for=\"".GetInputElementId("types", $id, PAGE_ADD)."\">","</label>");
...
//	show readonly fields
	$pageObject->readOnlyFields["types"] = htmlspecialchars($pageObject->showDBValue("types", $defvalues));
$linkdata="";
...
//	date_memo
	...
	if($display)
	{	
		$value = $pageObject->showDBValue("date_memo", $data, $keylink);
		$showValues["date_memo"] = $value;
		$showFields[] = "date_memo";
	}	
//	ref
	$display = false;
	if($inlineadd==ADD_MASTER)
	...
	if($display)
	{	
		$value = $pageObject->showDBValue("ref", $data, $keylink);
		$showValues["ref"] = $value;
		$showFields[] = "ref";
	}	
//	exam_date
	$display = false;
	...
	if($display)
	{	
		$value = $pageObject->showDBValue("exam_date", $data, $keylink);
		$showValues["exam_date"] = $value;
		$showFields[] = "exam_date";
	}	
//	exam_venue
	$display = false;
	if($inlineadd==ADD_MASTER)
	...
	if($display)
	{	
		$value = $pageObject->showDBValue("exam_venue", $data, $keylink);
		$showValues["exam_venue"] = $value;
		$showFields[] = "exam_venue";
	}	
//	stime
	$display = false;
	if($inlineadd==ADD_MASTER)
...
//	program
	...
	if($display)
	{	
		$value = $pageObject->showDBValue("program", $data, $keylink);
		$showValues["program"] = $value;
		$showFields[] = "program";
	}	
//	batch
	$display = false;
	...
	if($display)
	{	
		$value = $pageObject->showDBValue("batch", $data, $keylink);
		$showValues["batch"] = $value;
		$showFields[] = "batch";
	}	
//	class
	$display = false;
...
//	course
	...
	if($display)
	{	
		$value = $pageObject->showDBValue("course", $data, $keylink);
		$showValues["course"] = $value;
		$showFields[] = "course";
	}	
//	types
	$display = false;
	...
	if($display)
	{	
		$value = $pageObject->showDBValue("types", $data, $keylink);
		$showValues["types"] = $value;
		$showFields[] = "types";
	}	
//	opt
	$display = false;
	...
	if($display)
	{	
		$value = $pageObject->showDBValue("opt", $data, $keylink);
		$showValues["opt"] = $value;
		$showFields[] = "opt";
	}	
		$showRawValues["eid"] = substr($data["eid"],0,100);
		$showRawValues["date_memo"] = substr($data["date_memo"],0,100);
		$showRawValues["ref"] = substr($data["ref"],0,100);
		$showRawValues["exam_date"] = substr($data["exam_date"],0,100);
		$showRawValues["exam_venue"] = substr($data["exam_venue"],0,100);
		$showRawValues["stime"] = substr($data["stime"],0,100);
		$showRawValues["etime"] = substr($data["etime"],0,100);
		$showRawValues["program"] = substr($data["program"],0,100);
		$showRawValues["batch"] = substr($data["batch"],0,100);
		$showRawValues["class"] = substr($data["class"],0,100);
		$showRawValues["course"] = substr($data["course"],0,100);
		$showRawValues["types"] = substr($data["types"],0,100);
		$showRawValues["opt"] = substr($data["opt"],0,100);
...
if($pageObject->isShowDetailTables && ($inlineadd==ADD_SIMPLE || $inlineadd==ADD_POPUP) && !isMobile())
{
	...
	for($d=0;$d<count($dpParams['ids']);$d++)
	{
		...
		$options['masterTable'] = "exam_creator_moe";
		$options['firstTime'] = 1;
