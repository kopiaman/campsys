...
if($inlineadd == ADD_INLINE)
	$templatefile = "student_bill_statement_inline_add.htm";
else
	$templatefile = "student_bill_statement_add.htm";
...
if(isset($_REQUEST['afteradd'])){
		header('Location: student_bill_statement_add.php');
...
if(@$_POST["a"]=="added")
{
	...
	$avalues=array();
	$blobfields=array();
//	processing statement - start
	$inlineAddOption = true;
	$inlineAddOption = $inlineadd==ADD_INLINE;
	if($inlineAddOption)
	{
		$control_statement = $pageObject->getControl("statement", $id);
		$control_statement->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
...
//	insert masterkey value if exists and if not specified
	if(@$_SESSION[$sessionPrefix."_mastertable"]=="student_bill_list2")
	{
			...
			if($inlineadd==ADD_SIMPLE || $inlineadd==ADD_MASTER)
			{
				...
				if (count($keys))
				{
					...
					if($pageObject->pSet->hasEditPage() && $permis['edit'])
						$message .='&nbsp;<a href=\'student_bill_statement_edit.php?'.$keylink.'\'>'."Edit".'</a>&nbsp;';
					if($pageObject->pSet->hasViewPage() && $permis['search'])
						$message .='&nbsp;<a href=\'student_bill_statement_view.php?'.$keylink.'\'>'."View".'</a>&nbsp;';
				}
				$mesClass = "mes_ok";	
			}
		}
...
if (no_output_done() && $inlineadd==ADD_SIMPLE && $IsSaved)
{
	...
	// redirect
	header("Location: student_bill_statement_".$pageObject->getPageType().".php");
