...
if($inlineadd == ADD_INLINE)
	$templatefile = "staff_discipline_inline_add.htm";
else
	$templatefile = "staff_discipline_add.htm";
...
if(isset($_REQUEST['afteradd'])){
		header('Location: staff_discipline_add.php');
...
//	processing Date_Commited - start
	$inlineAddOption = true;
	$inlineAddOption = $inlineadd==ADD_INLINE;
	if($inlineAddOption)
	{
		$control_Date_Commited = $pageObject->getControl("Date_Commited", $id);
		$control_Date_Commited->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
			...
			if($inlineadd==ADD_SIMPLE || $inlineadd==ADD_MASTER)
			{
				...
				if (count($keys))
				{
					...
					if($pageObject->pSet->hasEditPage() && $permis['edit'])
						$message .='&nbsp;<a href=\'staff_discipline_edit.php?'.$keylink.'\'>'."Edit".'</a>&nbsp;';
					if($pageObject->pSet->hasViewPage() && $permis['search'])
						$message .='&nbsp;<a href=\'staff_discipline_view.php?'.$keylink.'\'>'."View".'</a>&nbsp;';
				}
				$mesClass = "mes_ok";	
			}
		}
...
if (no_output_done() && $inlineadd==ADD_SIMPLE && $IsSaved)
{
	...
	// redirect
	header("Location: staff_discipline_".$pageObject->getPageType().".php");
...
if(array_key_exists("copyid1",$_REQUEST) || array_key_exists("editid1",$_REQUEST))
{
	...
	{
		$copykeys["ID1"]=postvalue("copyid1");
	}
	else
	{
		$copykeys["ID1"]=postvalue("editid1");
	}
