...
if($inlineadd == ADD_INLINE)
	$templatefile = "calendar_qa_inline_add.htm";
else
	$templatefile = "calendar_qa_add.htm";
...
if(isset($_REQUEST['afteradd'])){
		header('Location: calendar_qa_add.php');
...
//	processing date_start - start
	$inlineAddOption = true;
	$inlineAddOption = $inlineadd==ADD_INLINE;
	if($inlineAddOption)
	{
		$control_date_start = $pageObject->getControl("date_start", $id);
		$control_date_start->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing date_start - end
//	processing date_end - start
	$inlineAddOption = true;
	$inlineAddOption = $inlineadd==ADD_INLINE;
	if($inlineAddOption)
	{
		$control_date_end = $pageObject->getControl("date_end", $id);
		$control_date_end->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
			...
			if($inlineadd==ADD_SIMPLE || $inlineadd==ADD_MASTER)
			{
				...
				if (count($keys))
				{
					...
					if($pageObject->pSet->hasEditPage() && $permis['edit'])
						$message .='&nbsp;<a href=\'calendar_qa_edit.php?'.$keylink.'\'>'."Edit".'</a>&nbsp;';
					if($pageObject->pSet->hasViewPage() && $permis['search'])
						$message .='&nbsp;<a href=\'calendar_qa_view.php?'.$keylink.'\'>'."View".'</a>&nbsp;';
				}
				$mesClass = "mes_ok";	
			}
		}
...
if (no_output_done() && $inlineadd==ADD_SIMPLE && $IsSaved)
{
	...
	// redirect
	header("Location: calendar_qa_".$pageObject->getPageType().".php");
