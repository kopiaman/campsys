...
$templatefile = ($inlineedit == EDIT_INLINE) ? "student_billing_item_inline_edit.htm" : "student_billing_item_edit.htm";
...
$skeys = "";
$savedKeys = array();
$keys["student_bill_id"] = urldecode(postvalue("editid1"));
$savedKeys["student_bill_id"] = urldecode(postvalue("editid1"));
...
if ($IsSaved && no_output_done() && $inlineedit == EDIT_SIMPLE)
{
	...
	// key get query
	$keyGetQ = "";
		$keyGetQ.="editid1=".rawurldecode($keys["student_bill_id"])."&";
	// cut last &
	$keyGetQ = substr($keyGetQ, 0, strlen($keyGetQ)-1);	
	// redirect
	header("Location: student_billing_item_".$pageObject->getPageType().".php?".$keyGetQ);
...
if(!$data)
{
	...
	{
		header("Location: student_billing_item_list.php?a=return");
		exit();
	}
	else
		$data = array();
...
if($inlineedit != EDIT_INLINE)
{
	...
	$xt->assign("show_key1", htmlspecialchars($pageObject->showDBValue("student_bill_id", $data)));
...
//	detail tables
	$showDetailKeys["student_bill_pay"]["masterkey1"] = $data["student_bill_id"];		
...
//	student_bill_id - 
	$value = $pageObject->showDBValue("student_bill_id", $data, $keylink);
	$showValues["student_bill_id"] = $value;
	$showFields[] = "student_bill_id";
		$showRawValues["student_bill_id"] = substr($data["student_bill_id"],0,100);
...
//	amount_balance - Custom
	$value = $pageObject->showDBValue("amount_balance", $data, $keylink);
	$showValues["amount_balance"] = $value;
	$showFields[] = "amount_balance";
		$showRawValues["amount_balance"] = substr($data["amount_balance"],0,100);
...
if($pageObject->isShowDetailTables && $inlineedit!=EDIT_INLINE && !isMobile())
{
	...
	for($d=0;$d<count($dpParams['ids']);$d++)
	{
		...
		$options['masterTable'] = "student_billing_item";
		$options['firstTime'] = 1;
...
$xt->assign("viewlink_attrs","id=\"viewButton".$id."\" name=\"viewButton".$id."\" onclick=\"window.location.href='student_billing_item_view.php?".$viewlink."'\"");
