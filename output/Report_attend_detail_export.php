...
include("include/Report_attend_detail_variables.php");
...
$layout->blocks["top"][] = "export";$page_layouts["Report_attend_detail_export"] = $layout;
...
$xt->display("Report_attend_detail_export.htm");

function ExportToExcel_old($cipherer)
{
	global $cCharset;
	header("Content-Type: application/vnd.ms-excel");
	header("Content-Disposition: attachment;Filename=Report_attend_detail.xls");
...
function ExportToWord($cipherer)
{
	...
	header("Content-Disposition: attachment;Filename=Report_attend_detail.doc");
...
function ExportToXML($cipherer)
{
	...
	header("Content-Disposition: attachment;Filename=Report_attend_detail.xml");
	...
	while((!$nPageSize || $i<$nPageSize) && $row)
	{
		$values = array();
			...
			$values["course"] = $pageObject->showDBValue("course", $row);
			$values["date"] = $pageObject->showDBValue("date", $row);
			$values["attend"] = $pageObject->showDBValue("attend", $row);
			...
			$values["hour"] = $pageObject->showDBValue("hour", $row);
			$values["Attendance"] = $pageObject->showDBValue("Attendance", $row);
...
function ExportToCSV($cipherer)
{
	...
	header("Content-Disposition: attachment;Filename=Report_attend_detail.csv");
...
// write data rows
	...
	while((!$nPageSize || $iNumberOfRows < $nPageSize) && $row)
	{
		$values = array();
			...
			$values["course"] = $pageObject->getViewControl("course")->showDBValue($row, "");
			$values["date"] = $pageObject->getViewControl("date")->showDBValue($row, "");
			$values["attend"] = $pageObject->getViewControl("attend")->showDBValue($row, "");
			...
			$values["hour"] = $pageObject->getViewControl("hour")->showDBValue($row, "");
			$values["Attendance"] = $pageObject->getViewControl("Attendance")->showDBValue($row, "");
		...
		if ($eventRes)
		{
			...
			if($outstr!="")
				$outstr.=",";
			$outstr.='"'.str_replace('"', '""', $values["course"]).'"';
			if($outstr!="")
				$outstr.=",";
			$outstr.='"'.str_replace('"', '""', $values["date"]).'"';
			if($outstr!="")
				$outstr.=",";
			$outstr.='"'.str_replace('"', '""', $values["attend"]).'"';
			if($outstr!="")
				$outstr.=",";
			...
			if($outstr!="")
				$outstr.=",";
			$outstr.='"'.str_replace('"', '""', $values["hour"]).'"';
			if($outstr!="")
				$outstr.=",";
			$outstr.='"'.str_replace('"', '""', $values["Attendance"]).'"';
			echo $outstr;
		}
...
// write header
	...
	if($_REQUEST["type"]=="excel")
	{
		...
		echo '<td style="width: 100" x:str>'.PrepareForExcel("Hour").'</td>';	
		echo '<td style="width: 100" x:str>'.PrepareForExcel("Attendance").'</td>';	
	}
	else
	{
		echo "<td>"."Name"."</td>";
		...
		if ($eventRes)
		{
			...
			echo '</td>';
							echo '<td>';
								...
								if($_REQUEST["type"]=="excel")
					echo PrepareForExcel($values["course"]);
				else
					echo $values["course"];//echo htmlspecialchars($values["course"]); commented for bug #6823
			...
			echo '</td>';
					...
					echo '<td>';
									...
									if($_REQUEST["type"]=="excel")
						echo PrepareForExcel($values["attend"]);
					else
						echo $values["attend"];
			echo '</td>';
							if($_REQUEST["type"]=="excel")
