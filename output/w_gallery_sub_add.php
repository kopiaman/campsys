...
if(@$_POST["a"]=="added")
{
	...
	$avalues=array();
	$blobfields=array();
//	processing wcat - start
	$inlineAddOption = true;
	$inlineAddOption = $inlineadd==ADD_INLINE;
	if($inlineAddOption)
	{
		$control_wcat = $pageObject->getControl("wcat", $id);
		$control_wcat->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing wcat - end
...
//	processing wcat - start
			...
			if($inlineAddOption)
			{
				$control_wcat->afterSuccessfulSave();
			}
//	processing wcat - end
//	processing sub_cat - start
			$inlineAddOption = true;
			...
			if($inlineadd==ADD_SIMPLE || $inlineadd==ADD_MASTER)
			{
				...
				if (count($keys))
				{
					...
					if($pageObject->pSet->hasEditPage() && $permis['edit'])
						$message .='&nbsp;<a href=\'w_gallery_sub_edit.php?'.$keylink.'\'>'."Edit".'</a>&nbsp;';
					if($pageObject->pSet->hasViewPage() && $permis['search'])
						$message .='&nbsp;<a href=\'w_gallery_sub_view.php?'.$keylink.'\'>'."View".'</a>&nbsp;';
				}
				$mesClass = "mes_ok";	
			}
		}
