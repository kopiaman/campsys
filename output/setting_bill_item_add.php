...
add_nocache_headers();
include("include/setting_bill_item_variables.php");
...
if($inlineadd == ADD_INLINE)
	$templatefile = "setting_bill_item_inline_add.htm";
else
	$templatefile = "setting_bill_item_add.htm";
...
if(isset($_REQUEST['afteradd'])){
		header('Location: setting_bill_item_add.php');
			...
			if($inlineadd==ADD_SIMPLE || $inlineadd==ADD_MASTER)
			{
				...
				if (count($keys))
				{
					...
					if($pageObject->pSet->hasEditPage() && $permis['edit'])
						$message .='&nbsp;<a href=\'setting_bill_item_edit.php?'.$keylink.'\'>'."Edit".'</a>&nbsp;';
					if($pageObject->pSet->hasViewPage() && $permis['search'])
						$message .='&nbsp;<a href=\'setting_bill_item_view.php?'.$keylink.'\'>'."View".'</a>&nbsp;';
				}
				$mesClass = "mes_ok";	
			}
		}
...
if (no_output_done() && $inlineadd==ADD_SIMPLE && $IsSaved)
{
	...
	// redirect
	header("Location: setting_bill_item_".$pageObject->getPageType().".php");
...
if(postvalue("table")=="program_billing" && postvalue("field")=="amount") 
	$defvalues["id"] = postvalue("category");
if(postvalue("table")=="student_billing" && postvalue("field")=="amount") 
	$defvalues["id"] = postvalue("category");
if(postvalue("table")=="student_billing_item" && postvalue("field")=="amount") 
	$defvalues["id"] = postvalue("category");
if($readavalues)
{
}
...
if($pageObject->isShowDetailTables && ($inlineadd==ADD_SIMPLE || $inlineadd==ADD_POPUP) && !isMobile())
{
	...
	for($d=0;$d<count($dpParams['ids']);$d++)
	{
		...
		$options['masterTable'] = "setting_bill_item";
		$options['firstTime'] = 1;
...
function GetAddedDataLookupQuery($pageObject, $keys, $forLookup)
{
	...
	if(postvalue("table")=="program_billing" && postvalue("field")=="item") 
	{
		$lookupFieldName = "item";
		$lookupTable = "setting_bill_item";
		$linkFieldName = "id";
		$dispfield = "items";
		// Table which contain lookup field!
		$lookupPSet = new ProjectSettings("program_billing", PAGE_ADD);
		$nLookupType = $lookupPSet->getLookupType($lookupFieldName);
		if($nLookupType == LT_QUERY)
		{
			if($lookupPSet->getCustomDisplay($lookupFieldName))
				$pageObject->pSet->getSQLQuery()->AddCustomExpression($dispfield, $pageObject->pSet, "program_billing", "item");
