	...
	*/
	...
	function save() 
	{
		if(postvalue("a")=="save")
		{
			...
			foreach(postvalue("username") as $user)
			{
				...
				$sql="delete from `campsysv621_ugmembers` where UserName=".db_prepare_string($user);
				...
				if(count($_POST["cb_".$useridx]))
				{
					...
					foreach(@$_POST["cb_".$useridx] as $g)
					{
						if($g<0)
						{
							if($user==$_SESSION["UserID"] && $g==-1)
								continue;
							$sql="insert into `campsysv621_ugmembers` (UserName,GroupID) values (".db_prepare_string(htmlspecialchars($user)).",".$g.")";
							db_exec($sql,$this->conn);
						}
						else
						{
					...
					if($glist!="")
					{
						$sql="insert into `campsysv621_ugmembers` (UserName,GroupID) select ".db_prepare_string(htmlspecialchars($user)).",GroupID from `campsysv621_uggroups` where GroupID in (".$glist.")";
						db_exec($sql,$this->conn);
					}
				}
			}
	...
	function fillGridData() 
	{
		...
		while($data && ($this->sortByGroup !== false || $this->recNo <= $this->pageSize || $this->pageSize==-1))
		{
			...
			$row["displayusername"] = htmlspecialchars($data["fullname"]); //htmlspecialchars($data["username"]);
	...
	{
		//	select members list		
		$trs = db_query("select `UserName`,`GroupID` from `campsysv621_ugmembers` order by `UserName`,`GroupID`",$this->conn);
	...
	function fillGroups() 
	{
		...
		$trs = db_query("select `GroupID`,`Label` from `campsysv621_uggroups` order by `Label`",$this->conn);
		while($tdata = db_fetch_numarray($trs))
		{
