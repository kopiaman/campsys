...
include("include/student_absent_variables.php");
...
$layout->blocks["top"][] = "export";$page_layouts["student_absent_export"] = $layout;
...
$xt->display("student_absent_export.htm");

function ExportToExcel_old($cipherer)
{
	global $cCharset;
	header("Content-Type: application/vnd.ms-excel");
	header("Content-Disposition: attachment;Filename=student_absent.xls");
...
function ExportToWord($cipherer)
{
	...
	header("Content-Disposition: attachment;Filename=student_absent.doc");
...
function ExportToXML($cipherer)
{
	...
	header("Content-Disposition: attachment;Filename=student_absent.xml");
...
function ExportToCSV($cipherer)
{
	...
	header("Content-Disposition: attachment;Filename=student_absent.csv");
...
// write header
	...
	{
		echo '<td style="width: 100" x:str>'.PrepareForExcel("Student Name").'</td>';	
	...
	while((!$nPageSize || $iNumberOfRows<$nPageSize) && $row)
	{
		...
		if ($eventRes)
		{
			...
			echo '</td>';
					...
					echo $values["attend"];//echo htmlspecialchars($values["attend"]); commented for bug #6823
