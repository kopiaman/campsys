...
$layout->blocks["top"][] = "export";$page_layouts["ojt_company_export"] = $layout;
...
$xt->display("ojt_company_export.htm");

function ExportToExcel_old($cipherer)
{
	global $cCharset;
	header("Content-Type: application/vnd.ms-excel");
	header("Content-Disposition: attachment;Filename=ojt_company.xls");
...
function ExportToWord($cipherer)
{
	...
	header("Content-Disposition: attachment;Filename=ojt_company.doc");
...
function ExportToXML($cipherer)
{
	...
	header("Content-Disposition: attachment;Filename=ojt_company.xml");
	...
	while((!$nPageSize || $i<$nPageSize) && $row)
	{
		$values = array();
			...
			$values["tel"] = $pageObject->showDBValue("tel", $row);
...
function ExportToCSV($cipherer)
{
	...
	header("Content-Disposition: attachment;Filename=ojt_company.csv");
...
// write data rows
	...
	while((!$nPageSize || $iNumberOfRows < $nPageSize) && $row)
	{
		$values = array();
			...
			$values["tel"] = $pageObject->getViewControl("tel")->showDBValue($row, "");
...
// write header
	...
	{
		echo '<td style="width: 100" x:str>'.PrepareForExcel("Organization Name").'</td>';	
		...
		if ($eventRes)
		{
			...
			echo '</td>';
					...
					echo '<td>';
									...
									if($_REQUEST["type"]=="excel")
						echo PrepareForExcel($values["tel"]);
					else
						echo $values["tel"];
			echo '</td>';
							if($_REQUEST["type"]=="excel")
