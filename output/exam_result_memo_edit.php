...
include("include/exam_result_memo_variables.php");
...
$templatefile = ($inlineedit == EDIT_INLINE) ? "exam_result_memo_inline_edit.htm" : "exam_result_memo_edit.htm";
...
//	processing date_memo2 - begin
	$condition = $inlineedit==EDIT_INLINE;//($inlineedit) inline mode

	if($condition)
	{
		$control_date_memo2 = $pageObject->getControl("date_memo2", $id);
		$control_date_memo2->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing date_memo2 - end
//	processing ref2 - begin
	$condition = $inlineedit==EDIT_INLINE;//($inlineedit) inline mode

	if($condition)
	{
		$control_ref2 = $pageObject->getControl("ref2", $id);
		$control_ref2->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);
	...
	//	do event
		...
		{		
			...
			//	processing date_memo2 - begin
				...
				if($condition)
				{
					$control_date_memo2->afterSuccessfulSave();
				}
	//	processing date_memo2 - end
			//	processing ref2 - begin
							$condition = $inlineedit==EDIT_INLINE;//($inlineedit) inline mode
				...
				if($condition)
				{
					$control_ref2->afterSuccessfulSave();
				}
	//	processing ref2 - end
...
if ($IsSaved && no_output_done() && $inlineedit == EDIT_SIMPLE)
{
	...
	// redirect
	header("Location: exam_result_memo_".$pageObject->getPageType().".php?".$keyGetQ);
...
if(!$data)
{
	...
	{
		header("Location: exam_result_memo_list.php?a=return");
		exit();
	}
	else
		$data = array();
...
if($readevalues)
{
	$data["date_memo2"] = $evalues["date_memo2"];
...
//	date_memo2 - Short Date
	$value = $pageObject->showDBValue("date_memo2", $data, $keylink);
	$showValues["date_memo2"] = $value;
	$showFields[] = "date_memo2";
		$showRawValues["date_memo2"] = substr($data["date_memo2"],0,100);

//	ref2 - 
	$value = $pageObject->showDBValue("ref2", $data, $keylink);
	$showValues["ref2"] = $value;
	$showFields[] = "ref2";
		$showRawValues["ref2"] = substr($data["ref2"],0,100);
...
if($pageObject->isShowDetailTables && $inlineedit!=EDIT_INLINE && !isMobile())
{
	...
	for($d=0;$d<count($dpParams['ids']);$d++)
	{
		...
		$options['masterTable'] = "exam_result_memo";
		$options['firstTime'] = 1;
		
		$strTableName = $dpParams['strTableNames'][$d];
		
		if(!CheckSecurity(@$_SESSION["_".$strTableName."_OwnerID"],"Search")){
			$strTableName = "exam_result_memo";
			continue;
		}
...
$xt->assign("viewlink_attrs","id=\"viewButton".$id."\" name=\"viewButton".$id."\" onclick=\"window.location.href='exam_result_memo_view.php?".$viewlink."'\"");
