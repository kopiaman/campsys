...
add_nocache_headers();
include("include/timetable_attendance_variables.php");
...
$layout->blocks["top"][] = "details";$page_layouts["timetable_attendance_add"] = $layout;
...
$layout->blocks["bare"][] = "1";$page_layouts["timetable_attendance_add_Datetime1"] = $layout;
...
if($inlineadd == ADD_INLINE)
	$templatefile = "timetable_attendance_inline_add.htm";
else
	$templatefile = "timetable_attendance_add.htm";
...
if(isset($_REQUEST['afteradd'])){
		header('Location: timetable_attendance_add.php');
			...
			if($inlineadd==ADD_SIMPLE || $inlineadd==ADD_MASTER)
			{
				...
				if (count($keys))
				{
					...
					if($pageObject->pSet->hasEditPage() && $permis['edit'])
						$message .='&nbsp;<a href=\'timetable_attendance_edit.php?'.$keylink.'\'>'."Edit".'</a>&nbsp;';
					if($pageObject->pSet->hasViewPage() && $permis['search'])
						$message .='&nbsp;<a href=\'timetable_attendance_view.php?'.$keylink.'\'>'."View".'</a>&nbsp;';
				}
				$mesClass = "mes_ok";	
			}
		}
...
if (no_output_done() && $inlineadd==ADD_SIMPLE && $IsSaved)
{
	...
	// redirect
	header("Location: timetable_attendance_".$pageObject->getPageType().".php");
...
//	teach_hour
	...
	if($display)
	{	
		$value = $pageObject->showDBValue("teach_hour", $data, $keylink);
		$showValues["teach_hour"] = $value;
		$showFields[] = "teach_hour";
	}	
//	date
	$display = false;
	if($inlineadd==ADD_MASTER)
...
//	class
	...
	{	
		...
		$showRawValues["teach_hour"] = substr($data["teach_hour"],0,100);
...
if($pageObject->isShowDetailTables && ($inlineadd==ADD_SIMPLE || $inlineadd==ADD_POPUP) && !isMobile())
{
	...
	for($d=0;$d<count($dpParams['ids']);$d++)
	{
		...
		$options['masterTable'] = "timetable_attendance";
		$options['firstTime'] = 1;
