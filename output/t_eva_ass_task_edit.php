...
$templatefile = ($inlineedit == EDIT_INLINE) ? "t_eva_ass_task_inline_edit.htm" : "t_eva_ass_task_edit.htm";
...
//	processing main_topic - begin
	$condition = $inlineedit==EDIT_INLINE;//($inlineedit) inline mode

	if($condition)
	{
		$control_main_topic = $pageObject->getControl("main_topic", $id);
		$control_main_topic->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing main_topic - end
//	processing sub_topic - begin
	$condition = $inlineedit==EDIT_INLINE;//($inlineedit) inline mode

	if($condition)
	{
		$control_sub_topic = $pageObject->getControl("sub_topic", $id);
		$control_sub_topic->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing sub_topic - end
...
//	processing taskno - begin
	$condition = $inlineedit==EDIT_INLINE;//($inlineedit) inline mode

	if($condition)
	{
		$control_taskno = $pageObject->getControl("taskno", $id);
		$control_taskno->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing taskno - end
	...
	//	do event
		...
		{		
			...
			//	processing main_topic - begin
				...
				if($condition)
				{
					$control_main_topic->afterSuccessfulSave();
				}
	//	processing main_topic - end
			//	processing sub_topic - begin
							$condition = $inlineedit==EDIT_INLINE;//($inlineedit) inline mode
				...
				if($condition)
				{
					$control_sub_topic->afterSuccessfulSave();
				}
	//	processing sub_topic - end
			//	processing title - begin
							$condition = $inlineedit==EDIT_INLINE;//($inlineedit) inline mode
	...
	//	processing title - end
			//	processing taskno - begin
				...
				if($condition)
				{
					$control_taskno->afterSuccessfulSave();
				}
	//	processing taskno - end
...
if ($IsSaved && no_output_done() && $inlineedit == EDIT_SIMPLE)
{
	...
	// redirect
	header("Location: t_eva_ass_task_".$pageObject->getPageType().".php?".$keyGetQ);
...
if(!$data)
{
	...
	{
		header("Location: t_eva_ass_task_list.php?a=return");
		exit();
	}
	else
		$data = array();
...
if($readevalues)
{
	$data["main_topic"] = $evalues["main_topic"];
	$data["sub_topic"] = $evalues["sub_topic"];
	$data["title"] = $evalues["title"];
	$data["taskno"] = $evalues["taskno"];
}
...
//	main_topic - 
	$value = $pageObject->showDBValue("main_topic", $data, $keylink);
	$showValues["main_topic"] = $value;
	$showFields[] = "main_topic";
		$showRawValues["main_topic"] = substr($data["main_topic"],0,100);

//	sub_topic - 
	$value = $pageObject->showDBValue("sub_topic", $data, $keylink);
	$showValues["sub_topic"] = $value;
	$showFields[] = "sub_topic";
		$showRawValues["sub_topic"] = substr($data["sub_topic"],0,100);
...
//	taskno - 
	$value = $pageObject->showDBValue("taskno", $data, $keylink);
	$showValues["taskno"] = $value;
	$showFields[] = "taskno";
		$showRawValues["taskno"] = substr($data["taskno"],0,100);
/////////////////////////////////////////////////////////////
//	start inline output
/////////////////////////////////////////////////////////////
...
if($pageObject->isShowDetailTables && $inlineedit!=EDIT_INLINE && !isMobile())
{
	...
	for($d=0;$d<count($dpParams['ids']);$d++)
	{
		...
		$options['masterTable'] = "t_eva_ass_task";
		$options['firstTime'] = 1;
...
$xt->assign("viewlink_attrs","id=\"viewButton".$id."\" name=\"viewButton".$id."\" onclick=\"window.location.href='t_eva_ass_task_view.php?".$viewlink."'\"");
