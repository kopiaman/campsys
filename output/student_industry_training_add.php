...
if($inlineadd == ADD_INLINE)
	$templatefile = "student_industry_training_inline_add.htm";
else
	$templatefile = "student_industry_training_add.htm";
...
if(isset($_REQUEST['afteradd'])){
		header('Location: student_industry_training_add.php');
...
if(@$_POST["a"]=="added")
{
	...
	$avalues=array();
	$blobfields=array();
//	processing organization_name - start
	$inlineAddOption = true;
	$inlineAddOption = $inlineadd==ADD_INLINE;
	if($inlineAddOption)
	{
		$control_organization_name = $pageObject->getControl("organization_name", $id);
		$control_organization_name->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing organization_name - end
//	processing unemployed - start
	$inlineAddOption = true;
	$inlineAddOption = $inlineadd==ADD_INLINE;
	if($inlineAddOption)
	{
		$control_unemployed = $pageObject->getControl("unemployed", $id);
		$control_unemployed->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
...
//	processing duration - start
	...
	if($inlineAddOption)
	{
		...
		$control_duration->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing duration - end
//	processing examStatus - start
	$inlineAddOption = true;
	$inlineAddOption = $inlineadd==ADD_INLINE;
	if($inlineAddOption)
	{
		$control_examStatus = $pageObject->getControl("examStatus", $id);
		$control_examStatus->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
...
//	processing remarks - start
	$inlineAddOption = true;
	$inlineAddOption = $inlineadd==ADD_INLINE;
	if($inlineAddOption)
	{
		$control_remarks = $pageObject->getControl("remarks", $id);
		$control_remarks->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
			...
			if($inlineadd==ADD_SIMPLE || $inlineadd==ADD_MASTER)
			{
				...
				if (count($keys))
				{
					...
					if($pageObject->pSet->hasEditPage() && $permis['edit'])
						$message .='&nbsp;<a href=\'student_industry_training_edit.php?'.$keylink.'\'>'."Edit".'</a>&nbsp;';
					if($pageObject->pSet->hasViewPage() && $permis['search'])
						$message .='&nbsp;<a href=\'student_industry_training_view.php?'.$keylink.'\'>'."View".'</a>&nbsp;';
				}
				$mesClass = "mes_ok";	
			}
		}
...
if (no_output_done() && $inlineadd==ADD_SIMPLE && $IsSaved)
{
	...
	// redirect
	header("Location: student_industry_training_".$pageObject->getPageType().".php");
