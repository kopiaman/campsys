...
$layout->blocks["top"][] = "details";$page_layouts["exam_creator_moe_edit"] = $layout;
...
$templatefile = "exam_creator_moe_edit.htm";
...
//	processing date_memo - begin
	$condition = $inlineedit!=EDIT_INLINE;//(!$inlineedit) edit simple mode

	if($condition)
	{
		$control_date_memo = $pageObject->getControl("date_memo", $id);
		$control_date_memo->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing date_memo - end
//	processing ref - begin
	$condition = $inlineedit!=EDIT_INLINE;//(!$inlineedit) edit simple mode

	if($condition)
	{
		$control_ref = $pageObject->getControl("ref", $id);
		$control_ref->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing ref - end
//	processing exam_date - begin
	$condition = $inlineedit!=EDIT_INLINE;//(!$inlineedit) edit simple mode

	if($condition)
	{
		$control_exam_date = $pageObject->getControl("exam_date", $id);
		$control_exam_date->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing exam_date - end
//	processing exam_venue - begin
	$condition = $inlineedit!=EDIT_INLINE;//(!$inlineedit) edit simple mode

	if($condition)
	{
		$control_exam_venue = $pageObject->getControl("exam_venue", $id);
		$control_exam_venue->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing exam_venue - end
//	processing stime - begin
	$condition = $inlineedit!=EDIT_INLINE;//(!$inlineedit) edit simple mode

	if($condition)
	{
		$control_stime = $pageObject->getControl("stime", $id);
		$control_stime->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing stime - end
//	processing etime - begin
	$condition = $inlineedit!=EDIT_INLINE;//(!$inlineedit) edit simple mode

	if($condition)
	{
		$control_etime = $pageObject->getControl("etime", $id);
		$control_etime->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);
...
//	processing course - begin
	$condition = $inlineedit!=EDIT_INLINE;//(!$inlineedit) edit simple mode

	if($condition)
	{
		$control_course = $pageObject->getControl("course", $id);
		$control_course->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing course - end
//	processing types - begin
	$condition = $inlineedit!=EDIT_INLINE;//(!$inlineedit) edit simple mode

	if($condition)
	{
		$control_types = $pageObject->getControl("types", $id);
		$control_types->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);
...
if ($IsSaved && no_output_done() && $inlineedit == EDIT_SIMPLE)
{
	...
	// redirect
	header("Location: exam_creator_moe_".$pageObject->getPageType().".php?".$keyGetQ);
...
if(!$data)
{
	...
	{
		header("Location: exam_creator_moe_list.php?a=return");
		exit();
	}
	else
		$data = array();
...
//	show readonly fields
	$pageObject->readOnlyFields["types"] = htmlspecialchars($pageObject->showDBValue("types", $data));

if($readevalues)
{
	$data["date_memo"] = $evalues["date_memo"];
	$data["ref"] = $evalues["ref"];
	$data["exam_date"] = $evalues["exam_date"];
	$data["exam_venue"] = $evalues["exam_venue"];
	$data["stime"] = $evalues["stime"];
	$data["etime"] = $evalues["etime"];
	$data["program"] = $evalues["program"];
	$data["batch"] = $evalues["batch"];
	$data["class"] = $evalues["class"];
	$data["course"] = $evalues["course"];
}
...
if($inlineedit != EDIT_INLINE)
{
	...
	if(isEnableSection508())
		$xt->assign_section("date_memo_label","<label for=\"".GetInputElementId("date_memo", $id, PAGE_EDIT)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("ref_label","<label for=\"".GetInputElementId("ref", $id, PAGE_EDIT)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("exam_date_label","<label for=\"".GetInputElementId("exam_date", $id, PAGE_EDIT)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("exam_venue_label","<label for=\"".GetInputElementId("exam_venue", $id, PAGE_EDIT)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("stime_label","<label for=\"".GetInputElementId("stime", $id, PAGE_EDIT)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("etime_label","<label for=\"".GetInputElementId("etime", $id, PAGE_EDIT)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("course_label","<label for=\"".GetInputElementId("course", $id, PAGE_EDIT)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("types_label","<label for=\"".GetInputElementId("types", $id, PAGE_EDIT)."\">","</label>");
...
$xt->assign("viewlink_attrs","id=\"viewButton".$id."\" name=\"viewButton".$id."\" onclick=\"window.location.href='exam_creator_moe_view.php?".$viewlink."'\"");
