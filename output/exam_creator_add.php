...
$layout->blocks["top"][] = "details";$page_layouts["exam_creator_add"] = $layout;
...
if($inlineadd == ADD_INLINE)
	$templatefile = "exam_creator_inline_add.htm";
else
	$templatefile = "exam_creator_add.htm";
...
if(isset($_REQUEST['afteradd'])){
		header('Location: exam_creator_add.php');
...
//	processing exam_remark - start
	$inlineAddOption = true;
	$inlineAddOption = $inlineadd!=ADD_INLINE;
	if($inlineAddOption)
	{
		$control_exam_remark = $pageObject->getControl("exam_remark", $id);
		$control_exam_remark->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing exam_remark - end
...
//	processing exam_remark - start
			...
			if($inlineAddOption)
			{
				$control_exam_remark->afterSuccessfulSave();
			}
//	processing exam_remark - end
			...
			if($inlineadd==ADD_SIMPLE || $inlineadd==ADD_MASTER)
			{
				...
				if (count($keys))
				{
					...
					if($pageObject->pSet->hasEditPage() && $permis['edit'])
						$message .='&nbsp;<a href=\'exam_creator_edit.php?'.$keylink.'\'>'."Edit".'</a>&nbsp;';
					if($pageObject->pSet->hasViewPage() && $permis['search'])
						$message .='&nbsp;<a href=\'exam_creator_view.php?'.$keylink.'\'>'."View".'</a>&nbsp;';
				}
				$mesClass = "mes_ok";	
			}
		}
...
if (no_output_done() && $inlineadd==ADD_SIMPLE && $IsSaved)
{
	...
	// redirect
	header("Location: exam_creator_".$pageObject->getPageType().".php");
...
if($readavalues)
{
	...
	$defvalues["exam_remark"]=@$avalues["exam_remark"];
}
...
if($inlineadd!=ADD_INLINE)
{
	...
	if(!$pageObject->isAppearOnTabs("exam_remark"))
		$xt->assign("exam_remark_fieldblock",true);
	else
		$xt->assign("exam_remark_tabfieldblock",true);
	$xt->assign("exam_remark_label",true);
	if(isEnableSection508())
		$xt->assign_section("exam_remark_label","<label for=\"".GetInputElementId("exam_remark", $id, PAGE_ADD)."\">","</label>");
...
//	exam_remark
	...
	if($display)
	{	
		$value = $pageObject->showDBValue("exam_remark", $data, $keylink);
		$showValues["exam_remark"] = $value;
		$showFields[] = "exam_remark";
	}	
		...
		$showRawValues["exam_remark"] = substr($data["exam_remark"],0,100);
...
if($pageObject->isShowDetailTables && ($inlineadd==ADD_SIMPLE || $inlineadd==ADD_POPUP) && !isMobile())
{
	...
	for($d=0;$d<count($dpParams['ids']);$d++)
	{
		...
		$options['masterTable'] = "exam_creator";
		$options['firstTime'] = 1;
