...
$layout->blocks["top"][] = "export";$page_layouts["Report_Assesment_export"] = $layout;
...
$xt->display("Report_Assesment_export.htm");

function ExportToExcel_old($cipherer)
{
	global $cCharset;
	header("Content-Type: application/vnd.ms-excel");
	header("Content-Disposition: attachment;Filename=Report_Assesment.xls");
...
function ExportToWord($cipherer)
{
	...
	header("Content-Disposition: attachment;Filename=Report_Assesment.doc");
...
function ExportToXML($cipherer)
{
	...
	header("Content-Disposition: attachment;Filename=Report_Assesment.xml");
...
function ExportToCSV($cipherer)
{
	...
	header("Content-Disposition: attachment;Filename=Report_Assesment.csv");
	...
	while((!$nPageSize || $iNumberOfRows<$nPageSize) && $row)
	{
		...
		if ($eventRes)
		{
			...
			echo '</td>';
					...
					echo $values["BatchID"];//echo htmlspecialchars($values["BatchID"]); commented for bug #6823
			...
			echo '</td>';
					...
					echo $values["group"];//echo htmlspecialchars($values["group"]); commented for bug #6823
