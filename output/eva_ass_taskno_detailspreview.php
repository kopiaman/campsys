...
$layout->blocks["bare"][] = "detailspreviewgrid";$page_layouts["eva_ass_taskno_detailspreview"] = $layout;
...
if($mastertable == "eva_ass_topic")
{
	$where = "";
		$where .= GetFullFieldName("sub_topic", $strTableName, false)."=".make_db_value("sub_topic",$_SESSION[$strTableName."_masterkey1"]);
}
...
if($rowcount) {
	...
	//	taskno - 
		    ...
		    $value = $viewContainer->showDBValue("taskno", $data, $keylink);
			$row["taskno_value"] = $value;
		$rowinfo[] = $row;
		$data = $cipherer->DecryptFetchedArray($rs);
	}
...
$xt->load_template("eva_ass_taskno_detailspreview.htm");
