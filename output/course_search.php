...
if ($mode==SEARCH_SIMPLE)
{
	...
	if($pageObject->pSet->getLookupTable("color"))
		$pageObject->settingsMap["globalSettings"]['shortTNames'][$pageObject->pSet->getLookupTable("color")] = GetTableURL($pageObject->pSet->getLookupTable("color"));
	
	$pageObject->fillFieldToolTips("color");	
	
	$srchFields = $pageObject->searchClauseObj->getSearchCtrlParams("color");
	...
	if(isEnableSection508())
		$xt->assign_section("color_label","<label for=\"".GetInputElementId("color", $id, PAGE_SEARCH)."\">","</label>");
	else 
	...
	$isFieldNeedSecCtrl = $searchControlBuilder->isNeedSecondCtrl("color");
	$ctrlInd = 0;
	if ($isFieldNeedSecCtrl)
	{
		$pageObject->controlsMap["search"]["searchBlocks"][] = array('fName'=>"color", 'recId'=>$id, 'ctrlsMap'=>array(0=>$ctrlInd, 1=>($ctrlInd+1)));
		$ctrlInd+=2;
	}
	else
	{
		$pageObject->controlsMap["search"]["searchBlocks"][] = array('fName'=>"color", 'recId'=>$id, 'ctrlsMap'=>array(0=>$ctrlInd));			
		$ctrlInd++;
	}
	// search fields data
