...
$templatefile = ($inlineedit == EDIT_INLINE) ? "exam_eligible_seatno_inline_edit.htm" : "exam_eligible_seatno_edit.htm";
...
//	processing Seat - begin
	$condition = $inlineedit==EDIT_INLINE;//($inlineedit) inline mode

	if($condition)
	{
		$control_Seat = $pageObject->getControl("Seat", $id);
		$control_Seat->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);
...
if ($IsSaved && no_output_done() && $inlineedit == EDIT_SIMPLE)
{
	...
	// redirect
	header("Location: exam_eligible_seatno_".$pageObject->getPageType().".php?".$keyGetQ);
...
if(!$data)
{
	...
	{
		header("Location: exam_eligible_seatno_list.php?a=return");
		exit();
	}
	else
		$data = array();
...
$xt->assign("viewlink_attrs","id=\"viewButton".$id."\" name=\"viewButton".$id."\" onclick=\"window.location.href='exam_eligible_seatno_view.php?".$viewlink."'\"");
