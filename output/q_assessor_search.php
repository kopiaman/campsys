...
include("include/q_assessor_variables.php");
...
$layout->blocks["top"][] = "search";$page_layouts["q_assessor_search"] = $layout;
...
$mode = SEARCH_SIMPLE;
$templatefile = "q_assessor_search.htm";
...
$params['shortTableName'] = 'q_assessor';
...
if ($mode==SEARCH_SIMPLE)
{
	...
	if($pageObject->pSet->getLookupTable("ext"))
		$pageObject->settingsMap["globalSettings"]['shortTNames'][$pageObject->pSet->getLookupTable("ext")] = GetTableURL($pageObject->pSet->getLookupTable("ext"));
	
	$pageObject->fillFieldToolTips("ext");	
	
	$srchFields = $pageObject->searchClauseObj->getSearchCtrlParams("ext");
	$firstFieldParams = array();
	if (count($srchFields))
	{
		$firstFieldParams = $srchFields[0];
	...
	if(isEnableSection508())
		$xt->assign_section("ext_label","<label for=\"".GetInputElementId("ext", $id, PAGE_SEARCH)."\">","</label>");
	else 
		$xt->assign("ext_label", true);
	
	$xt->assign("ext_fieldblock", true);
	$xt->assignbyref("ext_editcontrol", $ctrlBlockArr['searchcontrol']);
	$xt->assign("ext_notbox", $ctrlBlockArr['notbox']);
	// create second control, if need it
	$xt->assignbyref("ext_editcontrol1", $ctrlBlockArr['searchcontrol1']);
	// create search type select
	$xt->assign("searchtype_ext", $ctrlBlockArr['searchtype']);
	$isFieldNeedSecCtrl = $searchControlBuilder->isNeedSecondCtrl("ext");
	$ctrlInd = 0;
	if ($isFieldNeedSecCtrl)
	{
		$pageObject->controlsMap["search"]["searchBlocks"][] = array('fName'=>"ext", 'recId'=>$id, 'ctrlsMap'=>array(0=>$ctrlInd, 1=>($ctrlInd+1)));
		$ctrlInd+=2;
	}
	else
	{
		$pageObject->controlsMap["search"]["searchBlocks"][] = array('fName'=>"ext", 'recId'=>$id, 'ctrlsMap'=>array(0=>$ctrlInd));			
		$ctrlInd++;
	}
	// search fields data
