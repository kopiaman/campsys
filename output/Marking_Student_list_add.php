...
$layout = new TLayout("add","MATA_StyleMATA_purple","MobileMATA_purple");
$layout->blocks["top"] = array();
$layout->containers["fields"] = array();

$layout->containers["fields"][] = array("name"=>"addheader","block"=>"","substyle"=>2);
...
$layout->blocks["top"][] = "details";$page_layouts["Marking_Student_list_add"] = $layout;
...
$layout->blocks["bare"][] = "1";$page_layouts["Marking_Student_list_add_Personal1"] = $layout;
...
$layout->skins["1"] = "fields";
$layout->blocks["bare"][] = "1";$page_layouts["Marking_Student_list_add_Family_Info1"] = $layout;
...
$layout->skins["1"] = "fields";
$layout->blocks["bare"][] = "1";$page_layouts["Marking_Student_list_add_Next_Of_Kin1"] = $layout;
...
$layout->skins["1"] = "fields";
$layout->blocks["bare"][] = "1";$page_layouts["Marking_Student_list_add_Campus_Info1"] = $layout;
...
$layout->skins["1"] = "fields";
$layout->blocks["bare"][] = "1";$page_layouts["Marking_Student_list_add_Qualification1"] = $layout;
...
$layout->skins["1"] = "fields";
$layout->blocks["bare"][] = "1";$page_layouts["Marking_Student_list_add_Medical1"] = $layout;
...
if($inlineadd == ADD_INLINE)
	$templatefile = "Marking_Student_list_inline_add.htm";
else
	$templatefile = "Marking_Student_list_add.htm";
...
if(isset($_REQUEST['afteradd'])){
		header('Location: Marking_Student_list_add.php');
...
//	processing Nogilir - start
	$inlineAddOption = true;
	$inlineAddOption = $inlineadd!=ADD_INLINE;
	if($inlineAddOption)
	{
		$control_Nogilir = $pageObject->getControl("Nogilir", $id);
		$control_Nogilir->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
			...
			if($inlineadd==ADD_SIMPLE || $inlineadd==ADD_MASTER)
			{
				...
				if (count($keys))
				{
					...
					if($pageObject->pSet->hasEditPage() && $permis['edit'])
						$message .='&nbsp;<a href=\'Marking_Student_list_edit.php?'.$keylink.'\'>'."Edit".'</a>&nbsp;';
					if($pageObject->pSet->hasViewPage() && $permis['search'])
						$message .='&nbsp;<a href=\'Marking_Student_list_view.php?'.$keylink.'\'>'."View".'</a>&nbsp;';
				}
				$mesClass = "mes_ok";	
			}
		}
...
if (no_output_done() && $inlineadd==ADD_SIMPLE && $IsSaved)
{
	...
	// redirect
	header("Location: Marking_Student_list_".$pageObject->getPageType().".php");
...
if($readavalues)
{
	...
	$defvalues["Nogilir"]=@$avalues["Nogilir"];
...
if($inlineadd!=ADD_INLINE)
{
	...
	if(isEnableSection508())
		$xt->assign_section("Nogilir_label","<label for=\"".GetInputElementId("Nogilir", $id, PAGE_ADD)."\">","</label>");
...
if(@$_POST["a"]=="added" && ($inlineadd == ADD_INLINE || $inlineadd == ADD_MASTER || $inlineadd==ADD_POPUP)) 
{
	...
	$showDetailKeys["Marking_Batch"]["masterkey1"] = $data["StudentID"];	
...
//	Nogilir
	...
	if($display)
	{	
		$value = $pageObject->showDBValue("Nogilir", $data, $keylink);
		$showValues["Nogilir"] = $value;
		$showFields[] = "Nogilir";
	}	
//	City
	$display = false;
...
//	f_job
	...
	{	
		...
		$showRawValues["Nogilir"] = substr($data["Nogilir"],0,100);
...
if($pageObject->isShowDetailTables && ($inlineadd==ADD_SIMPLE || $inlineadd==ADD_POPUP) && !isMobile())
{
	...
	for($d=0;$d<count($dpParams['ids']);$d++)
	{
		...
		$options['masterTable'] = "Marking_Student_list";
		$options['firstTime'] = 1;
