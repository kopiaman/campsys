...
include("include/student_course_moe_variables.php");
...
$templatefile = ($inlineedit == EDIT_INLINE) ? "student_course_moe_inline_edit.htm" : "student_course_moe_edit.htm";
...
//	processing Final_Exam - begin
	$condition = $inlineedit==EDIT_INLINE;//($inlineedit) inline mode

	if($condition)
	{
		$control_Final_Exam = $pageObject->getControl("Final_Exam", $id);
		$control_Final_Exam->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing Final_Exam - end
	...
	//	processing Exam_Remark - end
			//	processing Final_Exam - begin
				...
				if($condition)
				{
					$control_Final_Exam->afterSuccessfulSave();
				}
	//	processing Final_Exam - end
...
if ($IsSaved && no_output_done() && $inlineedit == EDIT_SIMPLE)
{
	...
	// redirect
	header("Location: student_course_moe_".$pageObject->getPageType().".php?".$keyGetQ);
...
if(!$data)
{
	...
	{
		header("Location: student_course_moe_list.php?a=return");
		exit();
	}
	else
		$data = array();
...
if($readevalues)
{
	...
	$data["sem"] = $evalues["sem"];
	$data["Exam_Remark"] = $evalues["Exam_Remark"];
	$data["Final_Exam"] = $evalues["Final_Exam"];
}
...
//	Final_Exam - 
	$value = $pageObject->showDBValue("Final_Exam", $data, $keylink);
	$showValues["Final_Exam"] = $value;
	$showFields[] = "Final_Exam";
		$showRawValues["Final_Exam"] = substr($data["Final_Exam"],0,100);
/////////////////////////////////////////////////////////////
//	start inline output
/////////////////////////////////////////////////////////////
...
if($pageObject->isShowDetailTables && $inlineedit!=EDIT_INLINE && !isMobile())
{
	...
	for($d=0;$d<count($dpParams['ids']);$d++)
	{
		...
		$options['masterTable'] = "student_course_moe";
		$options['firstTime'] = 1;
...
$xt->assign("viewlink_attrs","id=\"viewButton".$id."\" name=\"viewButton".$id."\" onclick=\"window.location.href='student_course_moe_view.php?".$viewlink."'\"");
