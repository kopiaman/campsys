...
$layout->blocks["top"][] = "details";$page_layouts["support_add"] = $layout;
...
if($inlineadd == ADD_INLINE)
	$templatefile = "support_inline_add.htm";
else
	$templatefile = "support_add.htm";
...
//	processing topic - start
	$inlineAddOption = true;
	$inlineAddOption = $inlineadd!=ADD_INLINE;
	if($inlineAddOption)
	{
		$control_topic = $pageObject->getControl("topic", $id);
		$control_topic->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing topic - end
//	processing stitle - start
	$inlineAddOption = true;
	$inlineAddOption = $inlineadd!=ADD_INLINE;
	if($inlineAddOption)
	{
		$control_stitle = $pageObject->getControl("stitle", $id);
		$control_stitle->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing stitle - end
//	processing inquiry - start
	$inlineAddOption = true;
	$inlineAddOption = $inlineadd!=ADD_INLINE;
	if($inlineAddOption)
	{
		$control_inquiry = $pageObject->getControl("inquiry", $id);
		$control_inquiry->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
...
//	insert ownerid value if exists
	if($strOriginalTableName == $pageObject->pSet->getOwnerTable("staff"))
		$avalues["staff"] = prepare_for_db("staff",$_SESSION["_".$strTableName."_OwnerID"]);
			...
			if($inlineadd==ADD_SIMPLE || $inlineadd==ADD_MASTER)
			{
				...
				if (count($keys))
				{
					...
					if($pageObject->pSet->hasEditPage() && $permis['edit'])
						$message .='&nbsp;<a href=\'support_edit.php?'.$keylink.'\'>'."Edit".'</a>&nbsp;';
					if($pageObject->pSet->hasViewPage() && $permis['search'])
						$message .='&nbsp;<a href=\'support_view.php?'.$keylink.'\'>'."View".'</a>&nbsp;';
				}
				$mesClass = "mes_ok";	
			}
		}
...
if (no_output_done() && $inlineadd==ADD_SIMPLE && $IsSaved)
{
	...
	// redirect
	header("Location: support_".$pageObject->getPageType().".php");
...
//	insert default ownerid value if exists
	if($strOriginalTableName == $pageObject->pSet->getOwnerTable("staff"))
		$defvalues["staff"] = prepare_for_db("staff",$_SESSION["_".$strTableName."_OwnerID"]);


if($readavalues)
{
	$defvalues["topic"]=@$avalues["topic"];
	$defvalues["staff"]=@$avalues["staff"];
	$defvalues["inquiry"]=@$avalues["inquiry"];
	$defvalues["sstatus"]=@$avalues["sstatus"];
	$defvalues["sdate"]=@$avalues["sdate"];
	$defvalues["stitle"]=@$avalues["stitle"];
}
...
if($inlineadd!=ADD_INLINE)
{
	...
	if(isEnableSection508())
		$xt->assign_section("topic_label","<label for=\"".GetInputElementId("topic", $id, PAGE_ADD)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("staff_label","<label for=\"".GetInputElementId("staff", $id, PAGE_ADD)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("inquiry_label","<label for=\"".GetInputElementId("inquiry", $id, PAGE_ADD)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("sdate_label","<label for=\"".GetInputElementId("sdate", $id, PAGE_ADD)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("stitle_label","<label for=\"".GetInputElementId("stitle", $id, PAGE_ADD)."\">","</label>");
...
//	topic
	...
	if($display)
	{	
		$value = $pageObject->showDBValue("topic", $data, $keylink);
		$showValues["topic"] = $value;
		$showFields[] = "topic";
	}	
//	staff
	$display = false;
...
//	inquiry
	...
	if($display)
	{	
		$value = $pageObject->showDBValue("inquiry", $data, $keylink);
		$showValues["inquiry"] = $value;
		$showFields[] = "inquiry";
	}	
//	sstatus
	$display = false;
...
//	stitle
	...
	if($display)
	{	
		$value = $pageObject->showDBValue("stitle", $data, $keylink);
		$showValues["stitle"] = $value;
		$showFields[] = "stitle";
	}	
//	attach
	$display = false;
	...
	{	
		...
		$showRawValues["topic"] = substr($data["topic"],0,100);
		$showRawValues["staff"] = substr($data["staff"],0,100);
		$showRawValues["inquiry"] = substr($data["inquiry"],0,100);
		$showRawValues["sstatus"] = substr($data["sstatus"],0,100);
		$showRawValues["answer"] = substr($data["answer"],0,100);
		$showRawValues["sdate"] = substr($data["sdate"],0,100);
		$showRawValues["stitle"] = substr($data["stitle"],0,100);
		$showRawValues["attach"] = substr($data["attach"],0,100);
		$showRawValues["attachAns"] = substr($data["attachAns"],0,100);
