...
$templatefile = ($inlineedit == EDIT_INLINE) ? "program_inline_edit.htm" : "program_edit.htm";
...
//	processing type - end
//	processing shortcut_name - begin
	$condition = $inlineedit==EDIT_INLINE;//($inlineedit) inline mode

	if($condition)
	{
		$control_shortcut_name = $pageObject->getControl("shortcut_name", $id);
		$control_shortcut_name->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing shortcut_name - end
	...
	//	processing type - end
			//	processing shortcut_name - begin
				...
				if($condition)
				{
					$control_shortcut_name->afterSuccessfulSave();
				}
	//	processing shortcut_name - end
...
if ($IsSaved && no_output_done() && $inlineedit == EDIT_SIMPLE)
{
	...
	// redirect
	header("Location: program_".$pageObject->getPageType().".php?".$keyGetQ);
...
if(!$data)
{
	...
	{
		header("Location: program_list.php?a=return");
		exit();
	}
	else
		$data = array();
...
if($readevalues)
{
	...
	$data["shortcut_name"] = $evalues["shortcut_name"];
}
...
//	priority - 
	$value = $pageObject->showDBValue("priority", $data, $keylink);
	$showValues["priority"] = $value;
	$showFields[] = "priority";
		$showRawValues["priority"] = substr($data["priority"],0,100);
...
//	shortcut_name - 
	$value = $pageObject->showDBValue("shortcut_name", $data, $keylink);
	$showValues["shortcut_name"] = $value;
	$showFields[] = "shortcut_name";
		$showRawValues["shortcut_name"] = substr($data["shortcut_name"],0,100);
/////////////////////////////////////////////////////////////
//	start inline output
/////////////////////////////////////////////////////////////
...
$xt->assign("viewlink_attrs","id=\"viewButton".$id."\" name=\"viewButton".$id."\" onclick=\"window.location.href='program_view.php?".$viewlink."'\"");
