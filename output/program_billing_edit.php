...
$templatefile = ($inlineedit == EDIT_INLINE) ? "program_billing_inline_edit.htm" : "program_billing_edit.htm";
...
$skeys = "";
$savedKeys = array();
$keys["proid"] = urldecode(postvalue("editid1"));
$savedKeys["proid"] = urldecode(postvalue("editid1"));
...
//	processing item - begin
	$condition = $inlineedit==EDIT_INLINE;//($inlineedit) inline mode

	if($condition)
	{
		$control_item = $pageObject->getControl("item", $id);
		$control_item->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);
...
//	processing bill_date - begin
	$condition = $inlineedit==EDIT_INLINE;//($inlineedit) inline mode

	if($condition)
	{
		$control_bill_date = $pageObject->getControl("bill_date", $id);
		$control_bill_date->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing bill_date - end
//	processing bill_no - begin
	$condition = $inlineedit==EDIT_INLINE;//($inlineedit) inline mode

	if($condition)
	{
		$control_bill_no = $pageObject->getControl("bill_no", $id);
		$control_bill_no->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);
	...
	//	do event
		...
		{		
			...
			//	processing item - begin
				...
				if($condition)
				{
					$control_item->afterSuccessfulSave();
				}
	//	processing item - end
			//	processing amount - begin
							$condition = $inlineedit==EDIT_INLINE;//($inlineedit) inline mode
...
if ($IsSaved && no_output_done() && $inlineedit == EDIT_SIMPLE)
{
	...
	// key get query
	$keyGetQ = "";
		$keyGetQ.="editid1=".rawurldecode($keys["proid"])."&";
	// cut last &
	$keyGetQ = substr($keyGetQ, 0, strlen($keyGetQ)-1);	
	// redirect
	header("Location: program_billing_".$pageObject->getPageType().".php?".$keyGetQ);
...
if(!$data)
{
	...
	{
		header("Location: program_billing_list.php?a=return");
		exit();
	}
	else
		$data = array();
...
if($readevalues)
{
	$data["item"] = $evalues["item"];
	$data["amount"] = $evalues["amount"];
	$data["bill_date"] = $evalues["bill_date"];
	$data["bill_no"] = $evalues["bill_no"];
}
...
if($inlineedit != EDIT_INLINE)
{
	...
	$xt->assign("show_key1", htmlspecialchars($pageObject->showDBValue("proid", $data)));
...
//	detail tables
	...
	$keylink = "";
	$keylink.= "&key1=".htmlspecialchars(rawurlencode(@$data["proid"]));


//	proid - 
	$value = $pageObject->showDBValue("proid", $data, $keylink);
	$showValues["proid"] = $value;
	$showFields[] = "proid";
		$showRawValues["proid"] = substr($data["proid"],0,100);
...
//	item - 
	$value = $pageObject->showDBValue("item", $data, $keylink);
	$showValues["item"] = $value;
	$showFields[] = "item";
		$showRawValues["item"] = substr($data["item"],0,100);
...
if($pageObject->isShowDetailTables && $inlineedit!=EDIT_INLINE && !isMobile())
{
	...
	for($d=0;$d<count($dpParams['ids']);$d++)
	{
		...
		$options['masterTable'] = "program_billing";
		$options['firstTime'] = 1;
...
$xt->assign("viewlink_attrs","id=\"viewButton".$id."\" name=\"viewButton".$id."\" onclick=\"window.location.href='program_billing_view.php?".$viewlink."'\"");
