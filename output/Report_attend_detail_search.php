...
$layout->blocks["top"][] = "search";$page_layouts["Report_attend_detail_search"] = $layout;
...
$mode = SEARCH_SIMPLE;
$templatefile = "Report_attend_detail_search.htm";
...
$params['shortTableName'] = 'Report_attend_detail';
...
if ($mode==SEARCH_SIMPLE)
{
	...
	if($pageObject->pSet->getLookupTable("hour"))
		$pageObject->settingsMap["globalSettings"]['shortTNames'][$pageObject->pSet->getLookupTable("hour")] = GetTableURL($pageObject->pSet->getLookupTable("hour"));
	
	$pageObject->fillFieldToolTips("hour");	
	
	$srchFields = $pageObject->searchClauseObj->getSearchCtrlParams("hour");
	$firstFieldParams = array();
	if (count($srchFields))
	{
		$firstFieldParams = $srchFields[0];
	...
	if(isEnableSection508())
		$xt->assign_section("hour_label","<label for=\"".GetInputElementId("hour", $id, PAGE_SEARCH)."\">","</label>");
	else 
		$xt->assign("hour_label", true);
	
	$xt->assign("hour_fieldblock", true);
	$xt->assignbyref("hour_editcontrol", $ctrlBlockArr['searchcontrol']);
	$xt->assign("hour_notbox", $ctrlBlockArr['notbox']);
	// create second control, if need it
	$xt->assignbyref("hour_editcontrol1", $ctrlBlockArr['searchcontrol1']);
	// create search type select
	$xt->assign("searchtype_hour", $ctrlBlockArr['searchtype']);
	$isFieldNeedSecCtrl = $searchControlBuilder->isNeedSecondCtrl("hour");
	$ctrlInd = 0;
	if ($isFieldNeedSecCtrl)
	{
		$pageObject->controlsMap["search"]["searchBlocks"][] = array('fName'=>"hour", 'recId'=>$id, 'ctrlsMap'=>array(0=>$ctrlInd, 1=>($ctrlInd+1)));
		$ctrlInd+=2;
	}
	else
	{
		$pageObject->controlsMap["search"]["searchBlocks"][] = array('fName'=>"hour", 'recId'=>$id, 'ctrlsMap'=>array(0=>$ctrlInd));			
		$ctrlInd++;
	}
	// search fields data
