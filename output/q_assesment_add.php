...
if($inlineadd == ADD_INLINE)
	$templatefile = "q_assesment_inline_add.htm";
else
	$templatefile = "q_assesment_add.htm";
...
if(isset($_REQUEST['afteradd'])){
		header('Location: q_assesment_add.php');
...
if(@$_POST["a"]=="added")
{
	...
	$avalues=array();
	$blobfields=array();
//	processing task_no - start
	$inlineAddOption = true;
	$inlineAddOption = $inlineadd==ADD_INLINE;
	if($inlineAddOption)
	{
		$control_task_no = $pageObject->getControl("task_no", $id);
		$control_task_no->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing task_no - end
//	processing project - start
	$inlineAddOption = true;
	$inlineAddOption = $inlineadd==ADD_INLINE;
	if($inlineAddOption)
	{
		$control_project = $pageObject->getControl("project", $id);
		$control_project->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing project - end
//	processing as_name - start
	$inlineAddOption = true;
	$inlineAddOption = $inlineadd==ADD_INLINE;
	if($inlineAddOption)
	{
		$control_as_name = $pageObject->getControl("as_name", $id);
		$control_as_name->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing as_name - end
//	processing result - start
	$inlineAddOption = true;
	$inlineAddOption = $inlineadd==ADD_INLINE;
	if($inlineAddOption)
	{
		$control_result = $pageObject->getControl("result", $id);
		$control_result->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing result - end
//	processing as_date - start
	$inlineAddOption = true;
	$inlineAddOption = $inlineadd==ADD_INLINE;
	if($inlineAddOption)
	{
		$control_as_date = $pageObject->getControl("as_date", $id);
		$control_as_date->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing as_date - end
//	processing as_comment - start
	$inlineAddOption = true;
	$inlineAddOption = $inlineadd==ADD_INLINE;
	if($inlineAddOption)
	{
		$control_as_comment = $pageObject->getControl("as_comment", $id);
		$control_as_comment->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing as_comment - end


//	insert masterkey value if exists and if not specified
	if(@$_SESSION[$sessionPrefix."_mastertable"]=="Report_Assesment")
	{
			...
			if($inlineadd==ADD_SIMPLE || $inlineadd==ADD_MASTER)
			{
				...
				if (count($keys))
				{
					...
					if($pageObject->pSet->hasEditPage() && $permis['edit'])
						$message .='&nbsp;<a href=\'q_assesment_edit.php?'.$keylink.'\'>'."Edit".'</a>&nbsp;';
					if($pageObject->pSet->hasViewPage() && $permis['search'])
						$message .='&nbsp;<a href=\'q_assesment_view.php?'.$keylink.'\'>'."View".'</a>&nbsp;';
				}
				$mesClass = "mes_ok";	
			}
		}
...
if (no_output_done() && $inlineadd==ADD_SIMPLE && $IsSaved)
{
	...
	// redirect
	header("Location: q_assesment_".$pageObject->getPageType().".php");
