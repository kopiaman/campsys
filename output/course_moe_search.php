...
include("include/course_moe_variables.php");
...
$layout->blocks["top"][] = "search";$page_layouts["course_moe_search"] = $layout;
...
$mode = SEARCH_SIMPLE;
$templatefile = "course_moe_search.htm";
...
$params['shortTableName'] = 'course_moe';
...
if ($mode==SEARCH_SIMPLE)
{
	...
	if($pageObject->pSet->getLookupTable("sem"))
		$pageObject->settingsMap["globalSettings"]['shortTNames'][$pageObject->pSet->getLookupTable("sem")] = GetTableURL($pageObject->pSet->getLookupTable("sem"));
	
	$pageObject->fillFieldToolTips("sem");	
	
	$srchFields = $pageObject->searchClauseObj->getSearchCtrlParams("sem");
	$firstFieldParams = array();
	if (count($srchFields))
	{
		$firstFieldParams = $srchFields[0];
	...
	if(isEnableSection508())
		$xt->assign_section("sem_label","<label for=\"".GetInputElementId("sem", $id, PAGE_SEARCH)."\">","</label>");
	else 
		$xt->assign("sem_label", true);
	
	$xt->assign("sem_fieldblock", true);
	$xt->assignbyref("sem_editcontrol", $ctrlBlockArr['searchcontrol']);
	$xt->assign("sem_notbox", $ctrlBlockArr['notbox']);
	// create second control, if need it
	$xt->assignbyref("sem_editcontrol1", $ctrlBlockArr['searchcontrol1']);
	// create search type select
	$xt->assign("searchtype_sem", $ctrlBlockArr['searchtype']);
	$isFieldNeedSecCtrl = $searchControlBuilder->isNeedSecondCtrl("sem");
	$ctrlInd = 0;
	if ($isFieldNeedSecCtrl)
	{
		$pageObject->controlsMap["search"]["searchBlocks"][] = array('fName'=>"sem", 'recId'=>$id, 'ctrlsMap'=>array(0=>$ctrlInd, 1=>($ctrlInd+1)));
		$ctrlInd+=2;
	}
	else
	{
		$pageObject->controlsMap["search"]["searchBlocks"][] = array('fName'=>"sem", 'recId'=>$id, 'ctrlsMap'=>array(0=>$ctrlInd));			
		$ctrlInd++;
	}
