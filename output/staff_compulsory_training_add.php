...
if($inlineadd == ADD_INLINE)
	$templatefile = "staff_compulsory_training_inline_add.htm";
else
	$templatefile = "staff_compulsory_training_add.htm";
...
if(isset($_REQUEST['afteradd'])){
		header('Location: staff_compulsory_training_add.php');
...
if(@$_POST["a"]=="added")
{
	...
	$avalues=array();
	$blobfields=array();
//	processing TrainingID - start
	$inlineAddOption = true;
	$inlineAddOption = $inlineadd==ADD_INLINE;
	if($inlineAddOption)
	{
		$control_TrainingID = $pageObject->getControl("TrainingID", $id);
		$control_TrainingID->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing TrainingID - end
//	processing Training_Date - start
	$inlineAddOption = true;
	$inlineAddOption = $inlineadd==ADD_INLINE;
	if($inlineAddOption)
	{
		$control_Training_Date = $pageObject->getControl("Training_Date", $id);
		$control_Training_Date->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing Training_Date - end
//	processing Venue - start
	$inlineAddOption = true;
	$inlineAddOption = $inlineadd==ADD_INLINE;
	if($inlineAddOption)
	{
		$control_Venue = $pageObject->getControl("Venue", $id);
		$control_Venue->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing Venue - end
//	processing Provider - start
	$inlineAddOption = true;
	$inlineAddOption = $inlineadd==ADD_INLINE;
	if($inlineAddOption)
	{
		$control_Provider = $pageObject->getControl("Provider", $id);
		$control_Provider->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing Provider - end
//	processing Attendance - start
	$inlineAddOption = true;
	$inlineAddOption = $inlineadd==ADD_INLINE;
	if($inlineAddOption)
	{
		$control_Attendance = $pageObject->getControl("Attendance", $id);
		$control_Attendance->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
			...
			if($inlineadd==ADD_SIMPLE || $inlineadd==ADD_MASTER)
			{
				...
				if (count($keys))
				{
					...
					if($pageObject->pSet->hasEditPage() && $permis['edit'])
						$message .='&nbsp;<a href=\'staff_compulsory_training_edit.php?'.$keylink.'\'>'."Edit".'</a>&nbsp;';
					if($pageObject->pSet->hasViewPage() && $permis['search'])
						$message .='&nbsp;<a href=\'staff_compulsory_training_view.php?'.$keylink.'\'>'."View".'</a>&nbsp;';
				}
				$mesClass = "mes_ok";	
			}
		}
...
if (no_output_done() && $inlineadd==ADD_SIMPLE && $IsSaved)
{
	...
	// redirect
	header("Location: staff_compulsory_training_".$pageObject->getPageType().".php");
