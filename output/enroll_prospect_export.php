...
$layout->blocks["top"][] = "export";$page_layouts["enroll_prospect_export"] = $layout;
...
$xt->display("enroll_prospect_export.htm");

function ExportToExcel_old($cipherer)
{
	global $cCharset;
	header("Content-Type: application/vnd.ms-excel");
	header("Content-Disposition: attachment;Filename=enroll_prospect.xls");
...
function ExportToWord($cipherer)
{
	...
	header("Content-Disposition: attachment;Filename=enroll_prospect.doc");
...
function ExportToXML($cipherer)
{
	...
	header("Content-Disposition: attachment;Filename=enroll_prospect.xml");
	...
	{
		$values = array();
			$values["enrolID"] = $pageObject->showDBValue("enrolID", $row);
...
function ExportToCSV($cipherer)
{
	...
	header("Content-Disposition: attachment;Filename=enroll_prospect.csv");
...
// write data rows
	...
	{
		$values = array();
			$values["enrolID"] = $pageObject->getViewControl("enrolID")->showDBValue($row, "");
		...
		if ($eventRes)
		{
			...
			if($outstr!="")
				$outstr.=",";
			$outstr.='"'.str_replace('"', '""', $values["enrolID"]).'"';
			if($outstr!="")
				$outstr.=",";
...
// write header
	...
	{
		echo '<td style="width: 100" x:str>'.PrepareForExcel("Enrol ID").'</td>';	
		echo '<td style="width: 100" x:str>'.PrepareForExcel("Name").'</td>';	
		echo '<td style="width: 100" x:str>'.PrepareForExcel("Course").'</td>';	
		echo '<td style="width: 100" x:str>'.PrepareForExcel("Finance").'</td>';	
		echo '<td style="width: 100" x:str>'.PrepareForExcel("Key In Date").'</td>';	
		...
		echo '<td style="width: 100" x:str>'.PrepareForExcel("School").'</td>';	
		...
		echo '<td style="width: 100" x:str>'.PrepareForExcel("IELTS/ TOEFL/ MUET").'</td>';	
		...
		echo '<td style="width: 100" x:str>'.PrepareForExcel("University / Institute / School").'</td>';	
