...
if($inlineadd == ADD_INLINE)
	$templatefile = "staff_employement_history_inline_add.htm";
else
	$templatefile = "staff_employement_history_add.htm";
...
if(isset($_REQUEST['afteradd'])){
		header('Location: staff_employement_history_add.php');
...
//	processing Organisation - start
	$inlineAddOption = true;
	$inlineAddOption = $inlineadd==ADD_INLINE;
	if($inlineAddOption)
	{
		$control_Organisation = $pageObject->getControl("Organisation", $id);
		$control_Organisation->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
...
//	processing DateStart - start
	$inlineAddOption = true;
	$inlineAddOption = $inlineadd==ADD_INLINE;
	if($inlineAddOption)
	{
		$control_DateStart = $pageObject->getControl("DateStart", $id);
		$control_DateStart->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing DateStart - end
//	processing DateEnd - start
	$inlineAddOption = true;
	$inlineAddOption = $inlineadd==ADD_INLINE;
	if($inlineAddOption)
	{
		$control_DateEnd = $pageObject->getControl("DateEnd", $id);
		$control_DateEnd->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
			...
			if($inlineadd==ADD_SIMPLE || $inlineadd==ADD_MASTER)
			{
				...
				if (count($keys))
				{
					...
					if($pageObject->pSet->hasEditPage() && $permis['edit'])
						$message .='&nbsp;<a href=\'staff_employement_history_edit.php?'.$keylink.'\'>'."Edit".'</a>&nbsp;';
					if($pageObject->pSet->hasViewPage() && $permis['search'])
						$message .='&nbsp;<a href=\'staff_employement_history_view.php?'.$keylink.'\'>'."View".'</a>&nbsp;';
				}
				$mesClass = "mes_ok";	
			}
		}
...
if (no_output_done() && $inlineadd==ADD_SIMPLE && $IsSaved)
{
	...
	// redirect
	header("Location: staff_employement_history_".$pageObject->getPageType().".php");
