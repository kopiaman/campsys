...
$templatefile = ($inlineedit == EDIT_INLINE) ? "q_assesment_inline_edit.htm" : "q_assesment_edit.htm";
...
//	processing task_no - begin
	$condition = $inlineedit==EDIT_INLINE;//($inlineedit) inline mode

	if($condition)
	{
		$control_task_no = $pageObject->getControl("task_no", $id);
		$control_task_no->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing task_no - end
//	processing project - begin
	$condition = $inlineedit==EDIT_INLINE;//($inlineedit) inline mode

	if($condition)
	{
		$control_project = $pageObject->getControl("project", $id);
		$control_project->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing project - end
//	processing as_name - begin
	$condition = $inlineedit==EDIT_INLINE;//($inlineedit) inline mode

	if($condition)
	{
		$control_as_name = $pageObject->getControl("as_name", $id);
		$control_as_name->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing as_name - end
//	processing as_comment - begin
	$condition = $inlineedit==EDIT_INLINE;//($inlineedit) inline mode

	if($condition)
	{
		$control_as_comment = $pageObject->getControl("as_comment", $id);
		$control_as_comment->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing as_comment - end
//	processing result - begin
	$condition = $inlineedit==EDIT_INLINE;//($inlineedit) inline mode

	if($condition)
	{
		$control_result = $pageObject->getControl("result", $id);
		$control_result->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing result - end
//	processing as_date - begin
	$condition = $inlineedit==EDIT_INLINE;//($inlineedit) inline mode

	if($condition)
	{
		$control_as_date = $pageObject->getControl("as_date", $id);
		$control_as_date->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing as_date - end
	...
	//	do event
		...
		{		
			...
			//	processing task_no - begin
				...
				if($condition)
				{
					$control_task_no->afterSuccessfulSave();
				}
	//	processing task_no - end
			//	processing project - begin
							$condition = $inlineedit==EDIT_INLINE;//($inlineedit) inline mode
				...
				if($condition)
				{
					$control_project->afterSuccessfulSave();
				}
	//	processing project - end
			//	processing as_name - begin
							$condition = $inlineedit==EDIT_INLINE;//($inlineedit) inline mode
				...
				if($condition)
				{
					$control_as_name->afterSuccessfulSave();
				}
	//	processing as_name - end
			//	processing as_comment - begin
							$condition = $inlineedit==EDIT_INLINE;//($inlineedit) inline mode
				...
				if($condition)
				{
					$control_as_comment->afterSuccessfulSave();
				}
	//	processing as_comment - end
			//	processing result - begin
							$condition = $inlineedit==EDIT_INLINE;//($inlineedit) inline mode
				...
				if($condition)
				{
					$control_result->afterSuccessfulSave();
				}
	//	processing result - end
			//	processing as_date - begin
							$condition = $inlineedit==EDIT_INLINE;//($inlineedit) inline mode
				...
				if($condition)
				{
					$control_as_date->afterSuccessfulSave();
				}
	//	processing as_date - end
...
if ($IsSaved && no_output_done() && $inlineedit == EDIT_SIMPLE)
{
	...
	// redirect
	header("Location: q_assesment_".$pageObject->getPageType().".php?".$keyGetQ);
...
if(!$data)
{
	...
	{
		header("Location: q_assesment_list.php?a=return");
		exit();
	}
	else
		$data = array();
...
if($readevalues)
{
	$data["task_no"] = $evalues["task_no"];
	$data["project"] = $evalues["project"];
	$data["as_name"] = $evalues["as_name"];
	$data["as_comment"] = $evalues["as_comment"];
	$data["result"] = $evalues["result"];
	$data["as_date"] = $evalues["as_date"];
}
...
//	task_no - 
	$value = $pageObject->showDBValue("task_no", $data, $keylink);
	$showValues["task_no"] = $value;
	$showFields[] = "task_no";
		$showRawValues["task_no"] = substr($data["task_no"],0,100);

//	project - 
	$value = $pageObject->showDBValue("project", $data, $keylink);
	$showValues["project"] = $value;
	$showFields[] = "project";
		$showRawValues["project"] = substr($data["project"],0,100);

//	as_name - 
	$value = $pageObject->showDBValue("as_name", $data, $keylink);
	$showValues["as_name"] = $value;
	$showFields[] = "as_name";
		$showRawValues["as_name"] = substr($data["as_name"],0,100);

//	as_comment - 
	$value = $pageObject->showDBValue("as_comment", $data, $keylink);
	$showValues["as_comment"] = $value;
	$showFields[] = "as_comment";
		$showRawValues["as_comment"] = substr($data["as_comment"],0,100);

//	result - 
	$value = $pageObject->showDBValue("result", $data, $keylink);
	$showValues["result"] = $value;
	$showFields[] = "result";
		$showRawValues["result"] = substr($data["result"],0,100);
...
//	as_date - Short Date
	$value = $pageObject->showDBValue("as_date", $data, $keylink);
	$showValues["as_date"] = $value;
	$showFields[] = "as_date";
		$showRawValues["as_date"] = substr($data["as_date"],0,100);
/////////////////////////////////////////////////////////////
//	start inline output
/////////////////////////////////////////////////////////////
...
if($pageObject->isShowDetailTables && $inlineedit!=EDIT_INLINE && !isMobile())
{
	...
	for($d=0;$d<count($dpParams['ids']);$d++)
	{
		...
		$options['masterTable'] = "q_assesment";
		$options['firstTime'] = 1;
...
$xt->assign("viewlink_attrs","id=\"viewButton".$id."\" name=\"viewButton".$id."\" onclick=\"window.location.href='q_assesment_view.php?".$viewlink."'\"");
