...
$templatefile = ($inlineedit == EDIT_INLINE) ? "Student_Data_Generator_inline_edit.htm" : "Student_Data_Generator_edit.htm";
...
if ($IsSaved && no_output_done() && $inlineedit == EDIT_SIMPLE)
{
	...
	// redirect
	header("Location: Student_Data_Generator_".$pageObject->getPageType().".php?".$keyGetQ);
...
if(!$data)
{
	...
	{
		header("Location: Student_Data_Generator_list.php?a=return");
		exit();
	}
	else
		$data = array();
...
//	data_status - 
	$value = $pageObject->showDBValue("data_status", $data, $keylink);
	$showValues["data_status"] = $value;
	$showFields[] = "data_status";
		$showRawValues["data_status"] = substr($data["data_status"],0,100);
...
if($pageObject->isShowDetailTables && $inlineedit!=EDIT_INLINE && !isMobile())
{
	...
	for($d=0;$d<count($dpParams['ids']);$d++)
	{
		...
		$options['masterTable'] = "Student_Data_Generator";
		$options['firstTime'] = 1;
...
$xt->assign("viewlink_attrs","id=\"viewButton".$id."\" name=\"viewButton".$id."\" onclick=\"window.location.href='Student_Data_Generator_view.php?".$viewlink."'\"");
