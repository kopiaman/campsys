...
if($inlineadd == ADD_INLINE)
	$templatefile = "program_billing_item_inline_add.htm";
else
	$templatefile = "program_billing_item_add.htm";
...
if(isset($_REQUEST['afteradd'])){
		header('Location: program_billing_item_add.php');
...
//	insert masterkey value if exists and if not specified
	if(@$_SESSION[$sessionPrefix."_mastertable"]=="program_billing")
	{
		if(postvalue("masterkey1"))
			$_SESSION[$sessionPrefix."_masterkey1"] = postvalue("masterkey1");
		
		if($avalues["program_billing_ID"]==""){
			$avalues["program_billing_ID"] = prepare_for_db("program_billing_ID",$_SESSION[$sessionPrefix."_masterkey1"]);
		}
			...
			if($inlineadd==ADD_SIMPLE || $inlineadd==ADD_MASTER)
			{
				...
				if (count($keys))
				{
					...
					if($pageObject->pSet->hasEditPage() && $permis['edit'])
						$message .='&nbsp;<a href=\'program_billing_item_edit.php?'.$keylink.'\'>'."Edit".'</a>&nbsp;';
					if($pageObject->pSet->hasViewPage() && $permis['search'])
						$message .='&nbsp;<a href=\'program_billing_item_view.php?'.$keylink.'\'>'."View".'</a>&nbsp;';
				}
				$mesClass = "mes_ok";	
			}
		}
...
if (no_output_done() && $inlineadd==ADD_SIMPLE && $IsSaved)
{
	...
	// redirect
	header("Location: program_billing_item_".$pageObject->getPageType().".php");
...
if(@$_SESSION[$sessionPrefix."_mastertable"]=="program_billing")
{
	...
	if(postvalue("mainMPageType")<>"add")
		$defvalues["program_billing_ID"] = @$_SESSION[$sessionPrefix."_masterkey1"];	
