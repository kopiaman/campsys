...
if($inlineadd == ADD_INLINE)
	$templatefile = "Report_Student_Hostel_inline_add.htm";
else
	$templatefile = "Report_Student_Hostel_add.htm";
...
if(isset($_REQUEST['afteradd'])){
		header('Location: Report_Student_Hostel_add.php');
...
//	processing HostelID - start
	$inlineAddOption = true;
	$inlineAddOption = $inlineadd==ADD_INLINE;
	if($inlineAddOption)
	{
		$control_HostelID = $pageObject->getControl("HostelID", $id);
		$control_HostelID->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing HostelID - end
//	processing Room_No - start
	$inlineAddOption = true;
	$inlineAddOption = $inlineadd==ADD_INLINE;
	if($inlineAddOption)
	{
		$control_Room_No = $pageObject->getControl("Room_No", $id);
		$control_Room_No->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing Room_No - end
//	processing CheckInDate - start
	$inlineAddOption = true;
	$inlineAddOption = $inlineadd==ADD_INLINE;
	if($inlineAddOption)
	{
		$control_CheckInDate = $pageObject->getControl("CheckInDate", $id);
		$control_CheckInDate->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing CheckInDate - end
//	processing CheckOutDate - start
	$inlineAddOption = true;
	$inlineAddOption = $inlineadd==ADD_INLINE;
	if($inlineAddOption)
	{
		$control_CheckOutDate = $pageObject->getControl("CheckOutDate", $id);
		$control_CheckOutDate->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
...
//	processing Items - start
	...
	if($inlineAddOption)
	{
		...
		$control_Items->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
			...
			if($inlineadd==ADD_SIMPLE || $inlineadd==ADD_MASTER)
			{
				...
				if (count($keys))
				{
					...
					if($pageObject->pSet->hasEditPage() && $permis['edit'])
						$message .='&nbsp;<a href=\'Report_Student_Hostel_edit.php?'.$keylink.'\'>'."Edit".'</a>&nbsp;';
					if($pageObject->pSet->hasViewPage() && $permis['search'])
						$message .='&nbsp;<a href=\'Report_Student_Hostel_view.php?'.$keylink.'\'>'."View".'</a>&nbsp;';
				}
				$mesClass = "mes_ok";	
			}
		}
...
if (no_output_done() && $inlineadd==ADD_SIMPLE && $IsSaved)
{
	...
	// redirect
	header("Location: Report_Student_Hostel_".$pageObject->getPageType().".php");
