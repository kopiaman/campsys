...
//	processing attend - start
	$inlineAddOption = true;
	$inlineAddOption = $inlineadd==ADD_INLINE;
	if($inlineAddOption)
	{
		$control_attend = $pageObject->getControl("attend", $id);
		$control_attend->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing attend - end
...
//	processing attend - start
			...
			if($inlineAddOption)
			{
				$control_attend->afterSuccessfulSave();
			}
//	processing attend - end
//	processing description - start
			$inlineAddOption = true;
			...
			if($inlineadd==ADD_SIMPLE || $inlineadd==ADD_MASTER)
			{
				...
				if (count($keys))
				{
					...
					if($pageObject->pSet->hasEditPage() && $permis['edit'])
						$message .='&nbsp;<a href=\'student_attendance_edit.php?'.$keylink.'\'>'."Edit".'</a>&nbsp;';
					if($pageObject->pSet->hasViewPage() && $permis['search'])
						$message .='&nbsp;<a href=\'student_attendance_view.php?'.$keylink.'\'>'."View".'</a>&nbsp;';
				}
				$mesClass = "mes_ok";	
			}
		}
...
//	attend
	...
	if($display)
	{	
		$value = $pageObject->showDBValue("attend", $data, $keylink);
		$showValues["attend"] = $value;
		$showFields[] = "attend";
	}	
//	description
	$display = false;
	...
	{	
		...
		$showRawValues["attend"] = substr($data["attend"],0,100);
		...
		$showRawValues["attend_id"] = substr($data["attend_id"],0,100);
