...
$templatefile = ($inlineedit == EDIT_INLINE) ? "Report_Student_Hostel_inline_edit.htm" : "Report_Student_Hostel_edit.htm";
...
//	processing HostelID - begin
	$condition = $inlineedit==EDIT_INLINE;//($inlineedit) inline mode

	if($condition)
	{
		$control_HostelID = $pageObject->getControl("HostelID", $id);
		$control_HostelID->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing HostelID - end
//	processing Room_No - begin
	$condition = $inlineedit==EDIT_INLINE;//($inlineedit) inline mode

	if($condition)
	{
		$control_Room_No = $pageObject->getControl("Room_No", $id);
		$control_Room_No->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing Room_No - end
//	processing CheckInDate - begin
	$condition = $inlineedit==EDIT_INLINE;//($inlineedit) inline mode

	if($condition)
	{
		$control_CheckInDate = $pageObject->getControl("CheckInDate", $id);
		$control_CheckInDate->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing CheckInDate - end
//	processing CheckOutDate - begin
	$condition = $inlineedit==EDIT_INLINE;//($inlineedit) inline mode

	if($condition)
	{
		$control_CheckOutDate = $pageObject->getControl("CheckOutDate", $id);
		$control_CheckOutDate->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing CheckOutDate - end
//	processing Items - begin
	$condition = $inlineedit==EDIT_INLINE;//($inlineedit) inline mode

	if($condition)
	{
		$control_Items = $pageObject->getControl("Items", $id);
		$control_Items->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);
...
//	processing Items - end
	...
	//	processing StudentID - end
			//	processing HostelID - begin
				...
				if($condition)
				{
					$control_HostelID->afterSuccessfulSave();
				}
	//	processing HostelID - end
			//	processing Room_No - begin
							$condition = $inlineedit==EDIT_INLINE;//($inlineedit) inline mode
				...
				if($condition)
				{
					$control_Room_No->afterSuccessfulSave();
				}
	//	processing Room_No - end
			//	processing CheckInDate - begin
							$condition = $inlineedit==EDIT_INLINE;//($inlineedit) inline mode
				...
				if($condition)
				{
					$control_CheckInDate->afterSuccessfulSave();
				}
	//	processing CheckInDate - end
			//	processing CheckOutDate - begin
							$condition = $inlineedit==EDIT_INLINE;//($inlineedit) inline mode
				...
				if($condition)
				{
					$control_CheckOutDate->afterSuccessfulSave();
				}
	//	processing CheckOutDate - end
			//	processing Items - begin
							$condition = $inlineedit==EDIT_INLINE;//($inlineedit) inline mode
				...
				if($condition)
				{
					$control_Items->afterSuccessfulSave();
				}
	//	processing Items - end
...
if ($IsSaved && no_output_done() && $inlineedit == EDIT_SIMPLE)
{
	...
	// redirect
	header("Location: Report_Student_Hostel_".$pageObject->getPageType().".php?".$keyGetQ);
...
if(!$data)
{
	...
	{
		header("Location: Report_Student_Hostel_list.php?a=return");
		exit();
	}
	else
		$data = array();
...
if($readevalues)
{
	...
	$data["HostelID"] = $evalues["HostelID"];
	$data["Room_No"] = $evalues["Room_No"];
	$data["CheckInDate"] = $evalues["CheckInDate"];
	$data["CheckOutDate"] = $evalues["CheckOutDate"];
	$data["Items"] = $evalues["Items"];
}
...
//	HostelID - 
	$value = $pageObject->showDBValue("HostelID", $data, $keylink);
	$showValues["HostelID"] = $value;
	$showFields[] = "HostelID";
		$showRawValues["HostelID"] = substr($data["HostelID"],0,100);

//	Room_No - 
	$value = $pageObject->showDBValue("Room_No", $data, $keylink);
	$showValues["Room_No"] = $value;
	$showFields[] = "Room_No";
		$showRawValues["Room_No"] = substr($data["Room_No"],0,100);

//	CheckInDate - Short Date
	$value = $pageObject->showDBValue("CheckInDate", $data, $keylink);
	$showValues["CheckInDate"] = $value;
	$showFields[] = "CheckInDate";
		$showRawValues["CheckInDate"] = substr($data["CheckInDate"],0,100);

//	CheckOutDate - Short Date
	$value = $pageObject->showDBValue("CheckOutDate", $data, $keylink);
	$showValues["CheckOutDate"] = $value;
	$showFields[] = "CheckOutDate";
		$showRawValues["CheckOutDate"] = substr($data["CheckOutDate"],0,100);

//	Items - 
	$value = $pageObject->showDBValue("Items", $data, $keylink);
	$showValues["Items"] = $value;
	$showFields[] = "Items";
		$showRawValues["Items"] = substr($data["Items"],0,100);
...
if($pageObject->isShowDetailTables && $inlineedit!=EDIT_INLINE && !isMobile())
{
	...
	for($d=0;$d<count($dpParams['ids']);$d++)
	{
		...
		$options['masterTable'] = "Report_Student_Hostel";
		$options['firstTime'] = 1;
...
$xt->assign("viewlink_attrs","id=\"viewButton".$id."\" name=\"viewButton".$id."\" onclick=\"window.location.href='Report_Student_Hostel_view.php?".$viewlink."'\"");
