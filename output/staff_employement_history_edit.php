...
include("include/staff_employement_history_variables.php");
...
$templatefile = ($inlineedit == EDIT_INLINE) ? "staff_employement_history_inline_edit.htm" : "staff_employement_history_edit.htm";
...
//	processing Organisation - begin
	$condition = $inlineedit==EDIT_INLINE;//($inlineedit) inline mode

	if($condition)
	{
		$control_Organisation = $pageObject->getControl("Organisation", $id);
		$control_Organisation->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing Organisation - end
...
//	processing DateStart - begin
	$condition = $inlineedit==EDIT_INLINE;//($inlineedit) inline mode

	if($condition)
	{
		$control_DateStart = $pageObject->getControl("DateStart", $id);
		$control_DateStart->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing DateStart - end
//	processing DateEnd - begin
	$condition = $inlineedit==EDIT_INLINE;//($inlineedit) inline mode

	if($condition)
	{
		$control_DateEnd = $pageObject->getControl("DateEnd", $id);
		$control_DateEnd->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing DateEnd - end
...
//	processing Reason - end
	...
	//	do event
		...
		{		
			...
			//	processing Organisation - begin
				...
				if($condition)
				{
					$control_Organisation->afterSuccessfulSave();
				}
	//	processing Organisation - end
			//	processing Position - begin
							$condition = $inlineedit==EDIT_INLINE;//($inlineedit) inline mode
	...
	//	processing Position - end
			//	processing DateStart - begin
				...
				if($condition)
				{
					$control_DateStart->afterSuccessfulSave();
				}
	//	processing DateStart - end
			//	processing DateEnd - begin
							$condition = $inlineedit==EDIT_INLINE;//($inlineedit) inline mode
				...
				if($condition)
				{
					$control_DateEnd->afterSuccessfulSave();
				}
	//	processing DateEnd - end
			//	processing Reason - begin
							$condition = $inlineedit==EDIT_INLINE;//($inlineedit) inline mode
...
if ($IsSaved && no_output_done() && $inlineedit == EDIT_SIMPLE)
{
	...
	// redirect
	header("Location: staff_employement_history_".$pageObject->getPageType().".php?".$keyGetQ);
...
if(!$data)
{
	...
	{
		header("Location: staff_employement_history_list.php?a=return");
		exit();
	}
	else
		$data = array();
...
if($readevalues)
{
	$data["Organisation"] = $evalues["Organisation"];
	$data["Position"] = $evalues["Position"];
	$data["DateStart"] = $evalues["DateStart"];
	$data["DateEnd"] = $evalues["DateEnd"];
...
//	Organisation - 
	$value = $pageObject->showDBValue("Organisation", $data, $keylink);
	$showValues["Organisation"] = $value;
	$showFields[] = "Organisation";
		$showRawValues["Organisation"] = substr($data["Organisation"],0,100);
...
//	DateStart - Short Date
	$value = $pageObject->showDBValue("DateStart", $data, $keylink);
	$showValues["DateStart"] = $value;
	$showFields[] = "DateStart";
		$showRawValues["DateStart"] = substr($data["DateStart"],0,100);

//	DateEnd - Short Date
	$value = $pageObject->showDBValue("DateEnd", $data, $keylink);
	$showValues["DateEnd"] = $value;
	$showFields[] = "DateEnd";
		$showRawValues["DateEnd"] = substr($data["DateEnd"],0,100);
...
if($pageObject->isShowDetailTables && $inlineedit!=EDIT_INLINE && !isMobile())
{
	...
	for($d=0;$d<count($dpParams['ids']);$d++)
	{
		...
		$options['masterTable'] = "staff_employement_history";
		$options['firstTime'] = 1;
...
$xt->assign("viewlink_attrs","id=\"viewButton".$id."\" name=\"viewButton".$id."\" onclick=\"window.location.href='staff_employement_history_view.php?".$viewlink."'\"");
