...
$layout->blocks["top"][] = "details";$page_layouts["student_school_add"] = $layout;
...
if($inlineadd == ADD_INLINE)
	$templatefile = "student_school_inline_add.htm";
else
	$templatefile = "student_school_add.htm";
...
if(isset($_REQUEST['afteradd'])){
		header('Location: student_school_add.php');
...
//	processing subject - start
	...
	if($inlineAddOption)
	{
		...
		$control_subject->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
			...
			if($inlineadd==ADD_SIMPLE || $inlineadd==ADD_MASTER)
			{
				...
				if (count($keys))
				{
					...
					if($pageObject->pSet->hasEditPage() && $permis['edit'])
						$message .='&nbsp;<a href=\'student_school_edit.php?'.$keylink.'\'>'."Edit".'</a>&nbsp;';
					if($pageObject->pSet->hasViewPage() && $permis['search'])
						$message .='&nbsp;<a href=\'student_school_view.php?'.$keylink.'\'>'."View".'</a>&nbsp;';
				}
				$mesClass = "mes_ok";	
			}
		}
...
if (no_output_done() && $inlineadd==ADD_SIMPLE && $IsSaved)
{
	...
	// redirect
	header("Location: student_school_".$pageObject->getPageType().".php");
...
if($inlineadd!=ADD_INLINE)
{
	...
	if(isEnableSection508())
		$xt->assign_section("subject_label","<label for=\"".GetInputElementId("subject", $id, PAGE_ADD)."\">","</label>");
