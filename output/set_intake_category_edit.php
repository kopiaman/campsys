...
$templatefile = ($inlineedit == EDIT_INLINE) ? "set_intake_category_inline_edit.htm" : "set_intake_category_edit.htm";
...
//	processing Intake_Code - begin
	$condition = $inlineedit==EDIT_INLINE;//($inlineedit) inline mode

	if($condition)
	{
		$control_Intake_Code = $pageObject->getControl("Intake_Code", $id);
		$control_Intake_Code->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);
...
if ($IsSaved && no_output_done() && $inlineedit == EDIT_SIMPLE)
{
	...
	// redirect
	header("Location: set_intake_category_".$pageObject->getPageType().".php?".$keyGetQ);
...
if(!$data)
{
	...
	{
		header("Location: set_intake_category_list.php?a=return");
		exit();
	}
	else
		$data = array();
...
if($readevalues)
{
	...
	$data["Intake_Code"] = $evalues["Intake_Code"];
}
...
$xt->assign("viewlink_attrs","id=\"viewButton".$id."\" name=\"viewButton".$id."\" onclick=\"window.location.href='set_intake_category_view.php?".$viewlink."'\"");
