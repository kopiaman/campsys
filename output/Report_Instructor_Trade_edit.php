...
$templatefile = ($inlineedit == EDIT_INLINE) ? "Report_Instructor_Trade_inline_edit.htm" : "Report_Instructor_Trade_edit.htm";
...
//	processing trades - begin
	$condition = $inlineedit==EDIT_INLINE;//($inlineedit) inline mode

	if($condition)
	{
		$control_trades = $pageObject->getControl("trades", $id);
		$control_trades->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing trades - end
//	processing trades2 - begin
	$condition = $inlineedit==EDIT_INLINE;//($inlineedit) inline mode

	if($condition)
	{
		$control_trades2 = $pageObject->getControl("trades2", $id);
		$control_trades2->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing trades2 - end
//	processing subModule - begin
	$condition = $inlineedit==EDIT_INLINE;//($inlineedit) inline mode

	if($condition)
	{
		$control_subModule = $pageObject->getControl("subModule", $id);
		$control_subModule->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing subModule - end
	...
	//	do event
		...
		{		
			...
			//	processing trades - begin
				...
				if($condition)
				{
					$control_trades->afterSuccessfulSave();
				}
	//	processing trades - end
			//	processing trades2 - begin
							$condition = $inlineedit==EDIT_INLINE;//($inlineedit) inline mode
				...
				if($condition)
				{
					$control_trades2->afterSuccessfulSave();
				}
	//	processing trades2 - end
			//	processing subModule - begin
							$condition = $inlineedit==EDIT_INLINE;//($inlineedit) inline mode
				...
				if($condition)
				{
					$control_subModule->afterSuccessfulSave();
				}
	//	processing subModule - end
...
if ($IsSaved && no_output_done() && $inlineedit == EDIT_SIMPLE)
{
	...
	// redirect
	header("Location: Report_Instructor_Trade_".$pageObject->getPageType().".php?".$keyGetQ);
...
if(!$data)
{
	...
	{
		header("Location: Report_Instructor_Trade_list.php?a=return");
		exit();
	}
	else
		$data = array();
...
if($readevalues)
{
	$data["trades"] = $evalues["trades"];
	$data["trades2"] = $evalues["trades2"];
	$data["subModule"] = $evalues["subModule"];
}
...
//	trades - 
	$value = $pageObject->showDBValue("trades", $data, $keylink);
	$showValues["trades"] = $value;
	$showFields[] = "trades";
		$showRawValues["trades"] = substr($data["trades"],0,100);

//	trades2 - 
	$value = $pageObject->showDBValue("trades2", $data, $keylink);
	$showValues["trades2"] = $value;
	$showFields[] = "trades2";
		$showRawValues["trades2"] = substr($data["trades2"],0,100);

//	subModule - 
	$value = $pageObject->showDBValue("subModule", $data, $keylink);
	$showValues["subModule"] = $value;
	$showFields[] = "subModule";
		$showRawValues["subModule"] = substr($data["subModule"],0,100);
...
if($pageObject->isShowDetailTables && $inlineedit!=EDIT_INLINE && !isMobile())
{
	...
	for($d=0;$d<count($dpParams['ids']);$d++)
	{
		...
		$options['masterTable'] = "Report_Instructor_Trade";
		$options['firstTime'] = 1;
...
$xt->assign("viewlink_attrs","id=\"viewButton".$id."\" name=\"viewButton".$id."\" onclick=\"window.location.href='Report_Instructor_Trade_view.php?".$viewlink."'\"");
