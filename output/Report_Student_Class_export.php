...
$layout->blocks["top"][] = "export";$page_layouts["Report_Student_Class_export"] = $layout;
...
$xt->display("Report_Student_Class_export.htm");

function ExportToExcel_old($cipherer)
{
	global $cCharset;
	header("Content-Type: application/vnd.ms-excel");
	header("Content-Disposition: attachment;Filename=Report_Student_Class.xls");
...
function ExportToWord($cipherer)
{
	...
	header("Content-Disposition: attachment;Filename=Report_Student_Class.doc");
...
function ExportToXML($cipherer)
{
	...
	header("Content-Disposition: attachment;Filename=Report_Student_Class.xml");
	...
	while((!$nPageSize || $i<$nPageSize) && $row)
	{
		$values = array();
			...
			$values["Intake"] = $pageObject->showDBValue("Intake", $row);
			...
			$values["BranchID"] = $pageObject->showDBValue("BranchID", $row);
...
function ExportToCSV($cipherer)
{
	...
	header("Content-Disposition: attachment;Filename=Report_Student_Class.csv");
...
// write data rows
	...
	while((!$nPageSize || $iNumberOfRows < $nPageSize) && $row)
	{
		$values = array();
			...
			$values["Intake"] = $pageObject->getViewControl("Intake")->showDBValue($row, "");
			...
			$values["BranchID"] = $pageObject->getViewControl("BranchID")->showDBValue($row, "");
		...
		if ($eventRes)
		{
			...
			if($outstr!="")
				$outstr.=",";
			$outstr.='"'.str_replace('"', '""', $values["Intake"]).'"';
			if($outstr!="")
				$outstr.=",";
			...
			if($outstr!="")
				$outstr.=",";
			$outstr.='"'.str_replace('"', '""', $values["BranchID"]).'"';
			if($outstr!="")
				$outstr.=",";
		...
		if ($eventRes)
		{
			...
			echo '</td>';
					...
					echo '<td>';
								...
								if($_REQUEST["type"]=="excel")
					echo PrepareForExcel($values["Intake"]);
				else
					echo $values["Intake"];//echo htmlspecialchars($values["Intake"]); commented for bug #6823
			...
			echo '</td>';
							echo '<td>';
								...
								if($_REQUEST["type"]=="excel")
					echo PrepareForExcel($values["BranchID"]);
				else
					echo $values["BranchID"];//echo htmlspecialchars($values["BranchID"]); commented for bug #6823
