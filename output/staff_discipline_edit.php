...
include("include/staff_discipline_variables.php");
...
$templatefile = ($inlineedit == EDIT_INLINE) ? "staff_discipline_inline_edit.htm" : "staff_discipline_edit.htm";
...
$skeys = "";
$savedKeys = array();
$keys["ID1"] = urldecode(postvalue("editid1"));
$savedKeys["ID1"] = urldecode(postvalue("editid1"));
...
//	processing Date_Commited - begin
	$condition = $inlineedit==EDIT_INLINE;//($inlineedit) inline mode

	if($condition)
	{
		$control_Date_Commited = $pageObject->getControl("Date_Commited", $id);
		$control_Date_Commited->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing Date_Commited - end
...
//	processing Comment - end
	...
	//	processing Level - end
			//	processing Date_Commited - begin
				...
				if($condition)
				{
					$control_Date_Commited->afterSuccessfulSave();
				}
	//	processing Date_Commited - end
			//	processing Action_Taken - begin
							$condition = $inlineedit==EDIT_INLINE;//($inlineedit) inline mode
...
if ($IsSaved && no_output_done() && $inlineedit == EDIT_SIMPLE)
{
	...
	// key get query
	$keyGetQ = "";
		$keyGetQ.="editid1=".rawurldecode($keys["ID1"])."&";
	// cut last &
	$keyGetQ = substr($keyGetQ, 0, strlen($keyGetQ)-1);	
	// redirect
	header("Location: staff_discipline_".$pageObject->getPageType().".php?".$keyGetQ);
...
if(!$data)
{
	...
	{
		header("Location: staff_discipline_list.php?a=return");
		exit();
	}
	else
		$data = array();
...
if($readevalues)
{
	...
	$data["Date_Commited"] = $evalues["Date_Commited"];
...
if($inlineedit != EDIT_INLINE)
{
	...
	$xt->assign("show_key1", htmlspecialchars($pageObject->showDBValue("ID1", $data)));
...
//	detail tables
	...
	$keylink = "";
	$keylink.= "&key1=".htmlspecialchars(rawurlencode(@$data["ID1"]));
...
//	Date_Commited - Short Date
	$value = $pageObject->showDBValue("Date_Commited", $data, $keylink);
	$showValues["Date_Commited"] = $value;
	$showFields[] = "Date_Commited";
		$showRawValues["Date_Commited"] = substr($data["Date_Commited"],0,100);
...
if($pageObject->isShowDetailTables && $inlineedit!=EDIT_INLINE && !isMobile())
{
	...
	for($d=0;$d<count($dpParams['ids']);$d++)
	{
		...
		$options['masterTable'] = "staff_discipline";
		$options['firstTime'] = 1;
...
$xt->assign("viewlink_attrs","id=\"viewButton".$id."\" name=\"viewButton".$id."\" onclick=\"window.location.href='staff_discipline_view.php?".$viewlink."'\"");
