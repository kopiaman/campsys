...
include("include/Report_Sponsor_Overall_variables.php");
...
$layout->blocks["top"][] = "export";$page_layouts["Report_Sponsor_Overall_export"] = $layout;
...
$xt->display("Report_Sponsor_Overall_export.htm");

function ExportToExcel_old($cipherer)
{
	global $cCharset;
	header("Content-Type: application/vnd.ms-excel");
	header("Content-Disposition: attachment;Filename=Report_Sponsor_Overall.xls");
...
function ExportToWord($cipherer)
{
	...
	header("Content-Disposition: attachment;Filename=Report_Sponsor_Overall.doc");
...
function ExportToXML($cipherer)
{
	...
	header("Content-Disposition: attachment;Filename=Report_Sponsor_Overall.xml");
	...
	while((!$nPageSize || $i<$nPageSize) && $row)
	{
		$values = array();
			...
			$values["bil_student"] = $pageObject->showDBValue("bil_student", $row);
...
function ExportToCSV($cipherer)
{
	...
	header("Content-Disposition: attachment;Filename=Report_Sponsor_Overall.csv");
...
// write data rows
	...
	while((!$nPageSize || $iNumberOfRows < $nPageSize) && $row)
	{
		$values = array();
			...
			$values["bil_student"] = $pageObject->getViewControl("bil_student")->showDBValue($row, "");
		...
		if ($eventRes)
		{
			...
			if($outstr!="")
				$outstr.=",";
			$outstr.='"'.str_replace('"', '""', $values["bil_student"]).'"';
			echo $outstr;
		}
...
// write header
	...
	if($_REQUEST["type"]=="excel")
	{
		...
		echo '<td style="width: 100" x:str>'.PrepareForExcel("No of Student").'</td>';	
	}
	else
	{
		echo "<td>"."Course"."</td>";
		...
		echo "<td>"."No of Student"."</td>";
	}
	echo "</tr>";
			$totals = array();
		$totals["bil_student"] = array("value" => 0, "numRows" => 0);
		$totalsFields[] = array('fName'=>"bil_student", 'totalsType'=>'TOTAL', 'viewFormat'=>"");
...
// write data rows
	...
	echo "<td>";
						echo "Total".": ";
	echo htmlspecialchars(GetTotals("bil_student", $totals["bil_student"]["value"], "TOTAL", $totals["bil_student"]["numRows"], "", PAGE_EXPORT));
	echo "</td>";
	echo "</tr>";
}
