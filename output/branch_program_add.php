...
if($inlineadd == ADD_INLINE)
	$templatefile = "branch_program_inline_add.htm";
else
	$templatefile = "branch_program_add.htm";
...
if(isset($_REQUEST['afteradd'])){
		header('Location: branch_program_add.php');
...
if(@$_POST["a"]=="added")
{
	...
	$avalues=array();
	$blobfields=array();
//	processing branch_id - start
	$inlineAddOption = true;
	$inlineAddOption = $inlineadd==ADD_INLINE;
	if($inlineAddOption)
	{
		$control_branch_id = $pageObject->getControl("branch_id", $id);
		$control_branch_id->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing branch_id - end
//	processing program_id - start
	$inlineAddOption = true;
	$inlineAddOption = $inlineadd==ADD_INLINE;
	if($inlineAddOption)
	{
		$control_program_id = $pageObject->getControl("program_id", $id);
		$control_program_id->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
			...
			if($inlineadd==ADD_SIMPLE || $inlineadd==ADD_MASTER)
			{
				...
				if (count($keys))
				{
					...
					if($pageObject->pSet->hasEditPage() && $permis['edit'])
						$message .='&nbsp;<a href=\'branch_program_edit.php?'.$keylink.'\'>'."Edit".'</a>&nbsp;';
					if($pageObject->pSet->hasViewPage() && $permis['search'])
						$message .='&nbsp;<a href=\'branch_program_view.php?'.$keylink.'\'>'."View".'</a>&nbsp;';
				}
				$mesClass = "mes_ok";	
			}
		}
...
if (no_output_done() && $inlineadd==ADD_SIMPLE && $IsSaved)
{
	...
	// redirect
	header("Location: branch_program_".$pageObject->getPageType().".php");
