...
$templatefile = ($inlineedit == EDIT_INLINE) ? "program_course_inline_edit.htm" : "program_course_edit.htm";
...
$skeys = "";
$savedKeys = array();
$keys["pcid"] = urldecode(postvalue("editid1"));
$savedKeys["pcid"] = urldecode(postvalue("editid1"));
...
//	processing Semester - begin
	$condition = $inlineedit==EDIT_INLINE;//($inlineedit) inline mode

	if($condition)
	{
		$control_Semester = $pageObject->getControl("Semester", $id);
		$control_Semester->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing Semester - end
//	processing total_hour - begin
	$condition = $inlineedit==EDIT_INLINE;//($inlineedit) inline mode

	if($condition)
	{
		$control_total_hour = $pageObject->getControl("total_hour", $id);
		$control_total_hour->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing total_hour - end
//	processing credit - begin
	$condition = $inlineedit==EDIT_INLINE;//($inlineedit) inline mode

	if($condition)
	{
		$control_credit = $pageObject->getControl("credit", $id);
		$control_credit->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing credit - end
	...
	//	do event
		...
		{		
			...
			//	processing Semester - begin
				...
				if($condition)
				{
					$control_Semester->afterSuccessfulSave();
				}
	//	processing Semester - end
			//	processing total_hour - begin
							$condition = $inlineedit==EDIT_INLINE;//($inlineedit) inline mode
				...
				if($condition)
				{
					$control_total_hour->afterSuccessfulSave();
				}
	//	processing total_hour - end
			//	processing credit - begin
							$condition = $inlineedit==EDIT_INLINE;//($inlineedit) inline mode
				...
				if($condition)
				{
					$control_credit->afterSuccessfulSave();
				}
	//	processing credit - end
...
if ($IsSaved && no_output_done() && $inlineedit == EDIT_SIMPLE)
{
	...
	// key get query
	$keyGetQ = "";
		$keyGetQ.="editid1=".rawurldecode($keys["pcid"])."&";
	// cut last &
	$keyGetQ = substr($keyGetQ, 0, strlen($keyGetQ)-1);	
	// redirect
	header("Location: program_course_".$pageObject->getPageType().".php?".$keyGetQ);
...
if(!$data)
{
	...
	{
		header("Location: program_course_list.php?a=return");
		exit();
	}
	else
		$data = array();
...
if($readevalues)
{
	...
	$data["total_hour"] = $evalues["total_hour"];
	$data["credit"] = $evalues["credit"];
}
...
if($inlineedit != EDIT_INLINE)
{
	...
	$xt->assign("show_key1", htmlspecialchars($pageObject->showDBValue("pcid", $data)));
...
//	pcid - 
	$value = $pageObject->showDBValue("pcid", $data, $keylink);
	$showValues["pcid"] = $value;
	$showFields[] = "pcid";
		$showRawValues["pcid"] = substr($data["pcid"],0,100);
...
//	Semester - 
	$value = $pageObject->showDBValue("Semester", $data, $keylink);
	$showValues["Semester"] = $value;
	$showFields[] = "Semester";
		$showRawValues["Semester"] = substr($data["Semester"],0,100);

//	total_hour - 
	$value = $pageObject->showDBValue("total_hour", $data, $keylink);
	$showValues["total_hour"] = $value;
	$showFields[] = "total_hour";
		$showRawValues["total_hour"] = substr($data["total_hour"],0,100);
...
if($pageObject->isShowDetailTables && $inlineedit!=EDIT_INLINE && !isMobile())
{
	...
	for($d=0;$d<count($dpParams['ids']);$d++)
	{
		...
		$options['masterTable'] = "program_course";
		$options['firstTime'] = 1;
...
$xt->assign("viewlink_attrs","id=\"viewButton".$id."\" name=\"viewButton".$id."\" onclick=\"window.location.href='program_course_view.php?".$viewlink."'\"");
