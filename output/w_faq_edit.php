...
$layout->blocks["top"][] = "details";$page_layouts["w_faq_edit"] = $layout;
...
$skeys = "";
$savedKeys = array();
$keys["faqid"] = urldecode(postvalue("editid1"));
$savedKeys["faqid"] = urldecode(postvalue("editid1"));
...
//	processing question - begin
	$condition = $inlineedit!=EDIT_INLINE;//(!$inlineedit) edit simple mode

	if($condition)
	{
		$control_question = $pageObject->getControl("question", $id);
		$control_question->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);
...
if ($IsSaved && no_output_done() && $inlineedit == EDIT_SIMPLE)
{
	...
	// key get query
	$keyGetQ = "";
		$keyGetQ.="editid1=".rawurldecode($keys["faqid"])."&";
	// cut last &
	$keyGetQ = substr($keyGetQ, 0, strlen($keyGetQ)-1);	
	// redirect
	header("Location: w_faq_".$pageObject->getPageType().".php?".$keyGetQ);
...
if(!$data)
{
	...
	{
		header("Location: w_faq_list.php?a=return");
		exit();
	}
	else
		$data = array();
...
if($readevalues)
{
	$data["question"] = $evalues["question"];
...
if($inlineedit != EDIT_INLINE)
{
	...
	if(isEnableSection508())
		$xt->assign_section("question_label","<label for=\"".GetInputElementId("question", $id, PAGE_EDIT)."\">","</label>");
	...
	$xt->assign("show_key1", htmlspecialchars($pageObject->showDBValue("faqid", $data)));
...
$xt->assign("viewlink_attrs","id=\"viewButton".$id."\" name=\"viewButton".$id."\" onclick=\"window.location.href='w_faq_view.php?".$viewlink."'\"");
