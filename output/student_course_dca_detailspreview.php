...
$layout->blocks["bare"][] = "detailspreviewgrid";$page_layouts["student_course_dca_detailspreview"] = $layout;
...
if($mastertable == "student_course_DCA_List_student2")
{
	$where = "";
...
if($rowcount) {
	...
	//	Exam_Status - 
			$viewContainer->recId = $recordsCounter;
		    $value = $viewContainer->showDBValue("Exam_Status", $data, $keylink);
			$row["Exam_Status_value"] = $value;
	//	Exam_Remark - 
			$viewContainer->recId = $recordsCounter;
		    $value = $viewContainer->showDBValue("Exam_Remark", $data, $keylink);
			$row["Exam_Remark_value"] = $value;
	//	Exam_Date - Short Date
			$viewContainer->recId = $recordsCounter;
		    $value = $viewContainer->showDBValue("Exam_Date", $data, $keylink);
			$row["Exam_Date_value"] = $value;
		$rowinfo[] = $row;
		$data = $cipherer->DecryptFetchedArray($rs);
	}
...
$xt->load_template("student_course_dca_detailspreview.htm");
