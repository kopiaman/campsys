...
add_nocache_headers();
include("include/student_bill_pay_variables.php");
...
if($inlineadd == ADD_INLINE)
	$templatefile = "student_bill_pay_inline_add.htm";
else
	$templatefile = "student_bill_pay_add.htm";
...
if(isset($_REQUEST['afteradd'])){
		header('Location: student_bill_pay_add.php');
...
//	insert masterkey value if exists and if not specified
	if(@$_SESSION[$sessionPrefix."_mastertable"]=="student_billing_item")
	{
		if(postvalue("masterkey1"))
			$_SESSION[$sessionPrefix."_masterkey1"] = postvalue("masterkey1");
		
		if($avalues["student_bill_id"]==""){
			$avalues["student_bill_id"] = prepare_for_db("student_bill_id",$_SESSION[$sessionPrefix."_masterkey1"]);
		}
			...
			if($inlineadd==ADD_SIMPLE || $inlineadd==ADD_MASTER)
			{
				...
				if (count($keys))
				{
					...
					if($pageObject->pSet->hasEditPage() && $permis['edit'])
						$message .='&nbsp;<a href=\'student_bill_pay_edit.php?'.$keylink.'\'>'."Edit".'</a>&nbsp;';
					if($pageObject->pSet->hasViewPage() && $permis['search'])
						$message .='&nbsp;<a href=\'student_bill_pay_view.php?'.$keylink.'\'>'."View".'</a>&nbsp;';
				}
				$mesClass = "mes_ok";	
			}
		}
...
if (no_output_done() && $inlineadd==ADD_SIMPLE && $IsSaved)
{
	...
	// redirect
	header("Location: student_bill_pay_".$pageObject->getPageType().".php");
...
if(@$_SESSION[$sessionPrefix."_mastertable"]=="student_billing_item")
{
	...
	if(postvalue("mainMPageType")<>"add")
		$defvalues["student_bill_id"] = @$_SESSION[$sessionPrefix."_masterkey1"];	
...
if($pageObject->isShowDetailTables && ($inlineadd==ADD_SIMPLE || $inlineadd==ADD_POPUP) && !isMobile())
{
	...
	for($d=0;$d<count($dpParams['ids']);$d++)
	{
		...
		$options['masterTable'] = "student_bill_pay";
		$options['firstTime'] = 1;
