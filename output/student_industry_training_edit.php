...
include("include/student_industry_training_variables.php");
...
$templatefile = ($inlineedit == EDIT_INLINE) ? "student_industry_training_inline_edit.htm" : "student_industry_training_edit.htm";
...
//	processing organization_name - begin
	$condition = $inlineedit==EDIT_INLINE;//($inlineedit) inline mode

	if($condition)
	{
		$control_organization_name = $pageObject->getControl("organization_name", $id);
		$control_organization_name->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing organization_name - end
...
//	processing duration - begin
	$condition = $inlineedit==EDIT_INLINE;//($inlineedit) inline mode

	if($condition)
	{
		$control_duration = $pageObject->getControl("duration", $id);
		$control_duration->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing duration - end
//	processing examStatus - begin
	$condition = $inlineedit==EDIT_INLINE;//($inlineedit) inline mode

	if($condition)
	{
		$control_examStatus = $pageObject->getControl("examStatus", $id);
		$control_examStatus->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing examStatus - end
//	processing unemployed - begin
	$condition = $inlineedit==EDIT_INLINE;//($inlineedit) inline mode

	if($condition)
	{
		$control_unemployed = $pageObject->getControl("unemployed", $id);
		$control_unemployed->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing unemployed - end
//	processing remarks - begin
	$condition = $inlineedit==EDIT_INLINE;//($inlineedit) inline mode

	if($condition)
	{
		$control_remarks = $pageObject->getControl("remarks", $id);
		$control_remarks->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing remarks - end
	...
	//	do event
		...
		{		
			...
			//	processing organization_name - begin
				...
				if($condition)
				{
					$control_organization_name->afterSuccessfulSave();
				}
	//	processing organization_name - end
			//	processing start_date - begin
							$condition = $inlineedit==EDIT_INLINE;//($inlineedit) inline mode
	...
	//	processing end_date - end
			//	processing duration - begin
				...
				if($condition)
				{
					$control_duration->afterSuccessfulSave();
				}
	//	processing duration - end
			//	processing examStatus - begin
							$condition = $inlineedit==EDIT_INLINE;//($inlineedit) inline mode
				...
				if($condition)
				{
					$control_examStatus->afterSuccessfulSave();
				}
	//	processing examStatus - end
			//	processing unemployed - begin
							$condition = $inlineedit==EDIT_INLINE;//($inlineedit) inline mode
				...
				if($condition)
				{
					$control_unemployed->afterSuccessfulSave();
				}
	//	processing unemployed - end
			//	processing remarks - begin
							$condition = $inlineedit==EDIT_INLINE;//($inlineedit) inline mode
				...
				if($condition)
				{
					$control_remarks->afterSuccessfulSave();
				}
	//	processing remarks - end
...
if ($IsSaved && no_output_done() && $inlineedit == EDIT_SIMPLE)
{
	...
	// redirect
	header("Location: student_industry_training_".$pageObject->getPageType().".php?".$keyGetQ);
...
if(!$data)
{
	...
	{
		header("Location: student_industry_training_list.php?a=return");
		exit();
	}
	else
		$data = array();
...
if($readevalues)
{
	$data["organization_name"] = $evalues["organization_name"];
	$data["start_date"] = $evalues["start_date"];
	$data["end_date"] = $evalues["end_date"];
	$data["duration"] = $evalues["duration"];
	$data["examStatus"] = $evalues["examStatus"];
	$data["unemployed"] = $evalues["unemployed"];
	$data["remarks"] = $evalues["remarks"];
}
...
//	organization_name - 
	$value = $pageObject->showDBValue("organization_name", $data, $keylink);
	$showValues["organization_name"] = $value;
	$showFields[] = "organization_name";
		$showRawValues["organization_name"] = substr($data["organization_name"],0,100);

//	organization_supervisor - 
	$value = $pageObject->showDBValue("organization_supervisor", $data, $keylink);
	$showValues["organization_supervisor"] = $value;
	$showFields[] = "organization_supervisor";
		$showRawValues["organization_supervisor"] = substr($data["organization_supervisor"],0,100);

//	academic supervisor - 
	$value = $pageObject->showDBValue("academic supervisor", $data, $keylink);
	$showValues["academic supervisor"] = $value;
	$showFields[] = "academic supervisor";
		$showRawValues["academic supervisor"] = substr($data["academic supervisor"],0,100);
...
//	orga_super_tel - 
	$value = $pageObject->showDBValue("orga_super_tel", $data, $keylink);
	$showValues["orga_super_tel"] = $value;
	$showFields[] = "orga_super_tel";
		$showRawValues["orga_super_tel"] = substr($data["orga_super_tel"],0,100);

//	duration - 
	$value = $pageObject->showDBValue("duration", $data, $keylink);
	$showValues["duration"] = $value;
	$showFields[] = "duration";
		$showRawValues["duration"] = substr($data["duration"],0,100);

//	examStatus - 
	$value = $pageObject->showDBValue("examStatus", $data, $keylink);
	$showValues["examStatus"] = $value;
	$showFields[] = "examStatus";
		$showRawValues["examStatus"] = substr($data["examStatus"],0,100);

//	unemployed - 
	$value = $pageObject->showDBValue("unemployed", $data, $keylink);
	$showValues["unemployed"] = $value;
	$showFields[] = "unemployed";
		$showRawValues["unemployed"] = substr($data["unemployed"],0,100);

//	remarks - 
	$value = $pageObject->showDBValue("remarks", $data, $keylink);
	$showValues["remarks"] = $value;
	$showFields[] = "remarks";
		$showRawValues["remarks"] = substr($data["remarks"],0,100);
/////////////////////////////////////////////////////////////
//	start inline output
/////////////////////////////////////////////////////////////
...
if($pageObject->isShowDetailTables && $inlineedit!=EDIT_INLINE && !isMobile())
{
	...
	for($d=0;$d<count($dpParams['ids']);$d++)
	{
		...
		$options['masterTable'] = "student_industry_training";
		$options['firstTime'] = 1;
		
		$strTableName = $dpParams['strTableNames'][$d];
		
		if(!CheckSecurity(@$_SESSION["_".$strTableName."_OwnerID"],"Search")){
			$strTableName = "student_industry_training";
			continue;
		}
...
$xt->assign("viewlink_attrs","id=\"viewButton".$id."\" name=\"viewButton".$id."\" onclick=\"window.location.href='student_industry_training_view.php?".$viewlink."'\"");
