...
$layout->blocks["top"][] = "details";$page_layouts["Report__Industry_Training_edit"] = $layout;
...
$templatefile = "Report__Industry_Training_edit.htm";
...
if ($IsSaved && no_output_done() && $inlineedit == EDIT_SIMPLE)
{
	...
	// redirect
	header("Location: Report__Industry_Training_".$pageObject->getPageType().".php?".$keyGetQ);
...
if(!$data)
{
	...
	{
		header("Location: Report__Industry_Training_list.php?a=return");
		exit();
	}
	else
		$data = array();
...
if($inlineedit != EDIT_INLINE)
{
	...
	if(!$pageObject->isAppearOnTabs("organization_name"))
		$xt->assign("organization_name_fieldblock",true);
	else
		$xt->assign("organization_name_tabfieldblock",true);
	$xt->assign("organization_name_label",true);
	if(isEnableSection508())
		$xt->assign_section("organization_name_label","<label for=\"".GetInputElementId("organization_name", $id, PAGE_EDIT)."\">","</label>");
	...
	if(!$pageObject->isAppearOnTabs("examStatus"))
		$xt->assign("examStatus_fieldblock",true);
	else
		$xt->assign("examStatus_tabfieldblock",true);
	$xt->assign("examStatus_label",true);
	if(isEnableSection508())
		$xt->assign_section("examStatus_label","<label for=\"".GetInputElementId("examStatus", $id, PAGE_EDIT)."\">","</label>");
		
	if(!$pageObject->isAppearOnTabs("duration"))
		$xt->assign("duration_fieldblock",true);
	else
		$xt->assign("duration_tabfieldblock",true);
	$xt->assign("duration_label",true);
	if(isEnableSection508())
		$xt->assign_section("duration_label","<label for=\"".GetInputElementId("duration", $id, PAGE_EDIT)."\">","</label>");
		
	if(!$pageObject->isAppearOnTabs("unemployed"))
		$xt->assign("unemployed_fieldblock",true);
	else
		$xt->assign("unemployed_tabfieldblock",true);
	$xt->assign("unemployed_label",true);
	if(isEnableSection508())
		$xt->assign_section("unemployed_label","<label for=\"".GetInputElementId("unemployed", $id, PAGE_EDIT)."\">","</label>");
		
	if(!$pageObject->isAppearOnTabs("remarks"))
		$xt->assign("remarks_fieldblock",true);
	else
		$xt->assign("remarks_tabfieldblock",true);
	$xt->assign("remarks_label",true);
	if(isEnableSection508())
		$xt->assign_section("remarks_label","<label for=\"".GetInputElementId("remarks", $id, PAGE_EDIT)."\">","</label>");
...
if($pageObject->isShowDetailTables && $inlineedit!=EDIT_INLINE && !isMobile())
{
	...
	for($d=0;$d<count($dpParams['ids']);$d++)
	{
		...
		$options['masterTable'] = "Report-_Industry_Training";
		$options['firstTime'] = 1;
...
$xt->assign("viewlink_attrs","id=\"viewButton".$id."\" name=\"viewButton".$id."\" onclick=\"window.location.href='Report__Industry_Training_view.php?".$viewlink."'\"");
