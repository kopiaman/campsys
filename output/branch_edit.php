...
$layout->blocks["top"][] = "details";$page_layouts["branch_edit"] = $layout;
...
//	processing city - end
//	processing poscode - begin
	$condition = $inlineedit!=EDIT_INLINE;//(!$inlineedit) edit simple mode

	if($condition)
	{
		$control_poscode = $pageObject->getControl("poscode", $id);
		$control_poscode->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);
...
//	processing telephoneNo - begin
	$condition = $inlineedit!=EDIT_INLINE;//(!$inlineedit) edit simple mode

	if($condition)
	{
		$control_telephoneNo = $pageObject->getControl("telephoneNo", $id);
		$control_telephoneNo->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing telephoneNo - end
//	processing OperationStart - begin
	$condition = $inlineedit!=EDIT_INLINE;//(!$inlineedit) edit simple mode

	if($condition)
	{
		$control_OperationStart = $pageObject->getControl("OperationStart", $id);
		$control_OperationStart->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing OperationStart - end
//	processing ContactPerson - begin
	$condition = $inlineedit!=EDIT_INLINE;//(!$inlineedit) edit simple mode

	if($condition)
	{
		$control_ContactPerson = $pageObject->getControl("ContactPerson", $id);
		$control_ContactPerson->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);
...
if ($IsSaved && no_output_done() && $inlineedit == EDIT_SIMPLE)
{
	...
	// redirect
	header("Location: branch_".$pageObject->getPageType().".php?".$keyGetQ);
...
if($readevalues)
{
	...
	$data["poscode"] = $evalues["poscode"];
	$data["stateID"] = $evalues["stateID"];
	$data["faxNo"] = $evalues["faxNo"];
	$data["telephoneNo"] = $evalues["telephoneNo"];
	$data["OperationStart"] = $evalues["OperationStart"];
	$data["ContactPerson"] = $evalues["ContactPerson"];
}
...
if($inlineedit != EDIT_INLINE)
{
	...
	if(isEnableSection508())
		$xt->assign_section("poscode_label","<label for=\"".GetInputElementId("poscode", $id, PAGE_EDIT)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("telephoneNo_label","<label for=\"".GetInputElementId("telephoneNo", $id, PAGE_EDIT)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("OperationStart_label","<label for=\"".GetInputElementId("OperationStart", $id, PAGE_EDIT)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("ContactPerson_label","<label for=\"".GetInputElementId("ContactPerson", $id, PAGE_EDIT)."\">","</label>");
...
$xt->assign("viewlink_attrs","id=\"viewButton".$id."\" name=\"viewButton".$id."\" onclick=\"window.location.href='branch_view.php?".$viewlink."'\"");
