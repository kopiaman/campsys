...
$layout->blocks["top"][] = "search";$page_layouts["Report_Student_Class_search"] = $layout;
...
$mode = SEARCH_SIMPLE;
$templatefile = "Report_Student_Class_search.htm";
...
$params['shortTableName'] = 'Report_Student_Class';
...
if ($mode==SEARCH_SIMPLE)
{
	...
	if($pageObject->pSet->getLookupTable("BranchID"))
		$pageObject->settingsMap["globalSettings"]['shortTNames'][$pageObject->pSet->getLookupTable("BranchID")] = GetTableURL($pageObject->pSet->getLookupTable("BranchID"));
	
	$pageObject->fillFieldToolTips("BranchID");	
	
	$srchFields = $pageObject->searchClauseObj->getSearchCtrlParams("BranchID");
	$firstFieldParams = array();
	if (count($srchFields))
	{
		$firstFieldParams = $srchFields[0];
	...
	{
		$firstFieldParams['fName'] = "BranchID";
		$firstFieldParams['eType'] = '';
		$firstFieldParams['value1'] = '';
		$firstFieldParams['opt'] = '';
		$firstFieldParams['value2'] = '';
	...
	if(isEnableSection508())
		$xt->assign_section("BranchID_label","<label for=\"".GetInputElementId("BranchID", $id, PAGE_SEARCH)."\">","</label>");
	else 
		$xt->assign("BranchID_label", true);
	
	$xt->assign("BranchID_fieldblock", true);
	$xt->assignbyref("BranchID_editcontrol", $ctrlBlockArr['searchcontrol']);
	$xt->assign("BranchID_notbox", $ctrlBlockArr['notbox']);
	// create second control, if need it
	$xt->assignbyref("BranchID_editcontrol1", $ctrlBlockArr['searchcontrol1']);
	// create search type select
	$xt->assign("searchtype_BranchID", $ctrlBlockArr['searchtype']);
	$isFieldNeedSecCtrl = $searchControlBuilder->isNeedSecondCtrl("BranchID");
	$ctrlInd = 0;
	if ($isFieldNeedSecCtrl)
	{
		$pageObject->controlsMap["search"]["searchBlocks"][] = array('fName'=>"BranchID", 'recId'=>$id, 'ctrlsMap'=>array(0=>$ctrlInd, 1=>($ctrlInd+1)));
		$ctrlInd+=2;
	}
	else
	{
		$pageObject->controlsMap["search"]["searchBlocks"][] = array('fName'=>"BranchID", 'recId'=>$id, 'ctrlsMap'=>array(0=>$ctrlInd));			
		$ctrlInd++;
	}
	// search fields data
	
	if($pageObject->pSet->getLookupTable("Intake"))
		$pageObject->settingsMap["globalSettings"]['shortTNames'][$pageObject->pSet->getLookupTable("Intake")] = GetTableURL($pageObject->pSet->getLookupTable("Intake"));
	
	$pageObject->fillFieldToolTips("Intake");	
	
	$srchFields = $pageObject->searchClauseObj->getSearchCtrlParams("Intake");
	$firstFieldParams = array();
	if (count($srchFields))
	{
		$firstFieldParams = $srchFields[0];
	...
	if(isEnableSection508())
		$xt->assign_section("Intake_label","<label for=\"".GetInputElementId("Intake", $id, PAGE_SEARCH)."\">","</label>");
	else 
		$xt->assign("Intake_label", true);
	
	$xt->assign("Intake_fieldblock", true);
	$xt->assignbyref("Intake_editcontrol", $ctrlBlockArr['searchcontrol']);
	$xt->assign("Intake_notbox", $ctrlBlockArr['notbox']);
	// create second control, if need it
	$xt->assignbyref("Intake_editcontrol1", $ctrlBlockArr['searchcontrol1']);
	// create search type select
	$xt->assign("searchtype_Intake", $ctrlBlockArr['searchtype']);
	$isFieldNeedSecCtrl = $searchControlBuilder->isNeedSecondCtrl("Intake");
	$ctrlInd = 0;
	if ($isFieldNeedSecCtrl)
	{
		$pageObject->controlsMap["search"]["searchBlocks"][] = array('fName'=>"Intake", 'recId'=>$id, 'ctrlsMap'=>array(0=>$ctrlInd, 1=>($ctrlInd+1)));
		$ctrlInd+=2;
	}
	else
	{
		$pageObject->controlsMap["search"]["searchBlocks"][] = array('fName'=>"Intake", 'recId'=>$id, 'ctrlsMap'=>array(0=>$ctrlInd));			
		$ctrlInd++;
	}
	// search fields data
