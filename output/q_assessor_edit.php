...
$templatefile = ($inlineedit == EDIT_INLINE) ? "q_assessor_inline_edit.htm" : "q_assessor_edit.htm";
...
//	processing name - end
//	processing ext - begin
	$condition = $inlineedit==EDIT_INLINE;//($inlineedit) inline mode

	if($condition)
	{
		$control_ext = $pageObject->getControl("ext", $id);
		$control_ext->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);
...
if ($IsSaved && no_output_done() && $inlineedit == EDIT_SIMPLE)
{
	...
	// redirect
	header("Location: q_assessor_".$pageObject->getPageType().".php?".$keyGetQ);
...
if(!$data)
{
	...
	{
		header("Location: q_assessor_list.php?a=return");
		exit();
	}
	else
		$data = array();
...
$xt->assign("viewlink_attrs","id=\"viewButton".$id."\" name=\"viewButton".$id."\" onclick=\"window.location.href='q_assessor_view.php?".$viewlink."'\"");
