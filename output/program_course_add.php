...
if($inlineadd == ADD_INLINE)
	$templatefile = "program_course_inline_add.htm";
else
	$templatefile = "program_course_add.htm";
...
if(isset($_REQUEST['afteradd'])){
		header('Location: program_course_add.php');
...
//	processing credit - start
	$inlineAddOption = true;
	$inlineAddOption = $inlineadd==ADD_INLINE;
	if($inlineAddOption)
	{
		$control_credit = $pageObject->getControl("credit", $id);
		$control_credit->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
...
//	processing Semester - start
	...
	if($inlineAddOption)
	{
		...
		$control_Semester->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing Semester - end
//	processing total_hour - start
	$inlineAddOption = true;
	$inlineAddOption = $inlineadd==ADD_INLINE;
	if($inlineAddOption)
	{
		$control_total_hour = $pageObject->getControl("total_hour", $id);
		$control_total_hour->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing total_hour - end


//	insert masterkey value if exists and if not specified
	if(@$_SESSION[$sessionPrefix."_mastertable"]=="Program_Batch")
	{
		if(postvalue("masterkey1"))
			$_SESSION[$sessionPrefix."_masterkey1"] = postvalue("masterkey1");
		
		if($avalues["ProgramID"]==""){
			$avalues["ProgramID"] = prepare_for_db("ProgramID",$_SESSION[$sessionPrefix."_masterkey1"]);
		}
		...
		if($avalues["BatchID"]==""){
			$avalues["BatchID"] = prepare_for_db("BatchID",$_SESSION[$sessionPrefix."_masterkey2"]);
		}
			...
			if($inlineadd==ADD_SIMPLE || $inlineadd==ADD_MASTER)
			{
				...
				if (count($keys))
				{
					...
					if($pageObject->pSet->hasEditPage() && $permis['edit'])
						$message .='&nbsp;<a href=\'program_course_edit.php?'.$keylink.'\'>'."Edit".'</a>&nbsp;';
					if($pageObject->pSet->hasViewPage() && $permis['search'])
						$message .='&nbsp;<a href=\'program_course_view.php?'.$keylink.'\'>'."View".'</a>&nbsp;';
				}
				$mesClass = "mes_ok";	
			}
		}
...
if (no_output_done() && $inlineadd==ADD_SIMPLE && $IsSaved)
{
	...
	// redirect
	header("Location: program_course_".$pageObject->getPageType().".php");
...
if(array_key_exists("copyid1",$_REQUEST) || array_key_exists("editid1",$_REQUEST))
{
	...
	{
		$copykeys["pcid"]=postvalue("copyid1");
	}
	else
	{
		$copykeys["pcid"]=postvalue("editid1");
	}
...
if(@$_SESSION[$sessionPrefix."_mastertable"]=="Program_Batch")
{
	...
	if(postvalue("mainMPageType")<>"add")
		$defvalues["ProgramID"] = @$_SESSION[$sessionPrefix."_masterkey1"];	
	
	if(postvalue("masterkey2"))
		$_SESSION[$sessionPrefix."_masterkey2"] = postvalue("masterkey2");

	if(postvalue("mainMPageType")<>"add")
		$defvalues["BatchID"] = @$_SESSION[$sessionPrefix."_masterkey2"];	
