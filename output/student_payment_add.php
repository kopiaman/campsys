...
//	insert masterkey value if exists and if not specified
	if(@$_SESSION[$sessionPrefix."_mastertable"]=="student_billing")
	{
		...
		if($avalues["studentID"]==""){
			$avalues["studentID"] = prepare_for_db("studentID",$_SESSION[$sessionPrefix."_masterkey2"]);
		}
			...
			if($inlineadd==ADD_SIMPLE || $inlineadd==ADD_MASTER)
			{
				...
				if (count($keys))
				{
					...
					if($pageObject->pSet->hasEditPage() && $permis['edit'])
						$message .='&nbsp;<a href=\'student_payment_edit.php?'.$keylink.'\'>'."Edit".'</a>&nbsp;';
					if($pageObject->pSet->hasViewPage() && $permis['search'])
						$message .='&nbsp;<a href=\'student_payment_view.php?'.$keylink.'\'>'."View".'</a>&nbsp;';
				}
				$mesClass = "mes_ok";	
			}
		}
...
if(@$_SESSION[$sessionPrefix."_mastertable"]=="student_billing")
{
	...
	if(postvalue("mainMPageType")<>"add")
		$defvalues["studentID"] = @$_SESSION[$sessionPrefix."_masterkey2"];	
