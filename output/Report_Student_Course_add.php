...
if($inlineadd == ADD_INLINE)
	$templatefile = "Report_Student_Course_inline_add.htm";
else
	$templatefile = "Report_Student_Course_add.htm";
...
if(isset($_REQUEST['afteradd'])){
		header('Location: Report_Student_Course_add.php');
...
//	processing e1 - start
	...
	if($inlineAddOption)
	{
		...
		$control_e1->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing e1 - end
//	processing e2 - start
	$inlineAddOption = true;
	...
	if($inlineAddOption)
	{
		...
		$control_e2->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing e2 - end
//	processing e3 - start
	$inlineAddOption = true;
	...
	if($inlineAddOption)
	{
		...
		$control_e3->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing e3 - end
//	processing e4 - start
	$inlineAddOption = true;
	...
	if($inlineAddOption)
	{
		...
		$control_e4->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing e4 - end


//	insert masterkey value if exists and if not specified
	if(@$_SESSION[$sessionPrefix."_mastertable"]=="Report_Student_Course_List")
	{
			...
			if($inlineadd==ADD_SIMPLE || $inlineadd==ADD_MASTER)
			{
				...
				if (count($keys))
				{
					...
					if($pageObject->pSet->hasEditPage() && $permis['edit'])
						$message .='&nbsp;<a href=\'Report_Student_Course_edit.php?'.$keylink.'\'>'."Edit".'</a>&nbsp;';
					if($pageObject->pSet->hasViewPage() && $permis['search'])
						$message .='&nbsp;<a href=\'Report_Student_Course_view.php?'.$keylink.'\'>'."View".'</a>&nbsp;';
				}
				$mesClass = "mes_ok";	
			}
		}
...
if (no_output_done() && $inlineadd==ADD_SIMPLE && $IsSaved)
{
	...
	// redirect
	header("Location: Report_Student_Course_".$pageObject->getPageType().".php");
