...
include("include/Report_Attendance_variables.php");
...
$layout->blocks["bare"][] = "detailspreviewgrid";$page_layouts["Report_Attendance_detailspreview"] = $layout;
...
if($mastertable == "Report_Attendance_Student_List")
{
	$where = "";
...
if($rowcount) {
	...
	//	abs_count - 
			$viewContainer->recId = $recordsCounter;
		    $value = $viewContainer->showDBValue("abs_count", $data, $keylink);
			$row["abs_count_value"] = $value;
	//	abs_hour - 
			$viewContainer->recId = $recordsCounter;
		    $value = $viewContainer->showDBValue("abs_hour", $data, $keylink);
			$row["abs_hour_value"] = $value;
	//	total_hour - 
			$viewContainer->recId = $recordsCounter;
	...
	//	att_percent - Custom
			$viewContainer->recId = $recordsCounter;
		    $value = $viewContainer->showDBValue("att_percent", $data, $keylink);
			$row["att_percent_value"] = $value;
		$rowinfo[] = $row;
		$data = $cipherer->DecryptFetchedArray($rs);
	}
...
$xt->load_template("Report_Attendance_detailspreview.htm");
