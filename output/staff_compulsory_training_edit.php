...
include("include/staff_compulsory_training_variables.php");
...
$templatefile = ($inlineedit == EDIT_INLINE) ? "staff_compulsory_training_inline_edit.htm" : "staff_compulsory_training_edit.htm";
...
//	processing TrainingID - begin
	$condition = $inlineedit==EDIT_INLINE;//($inlineedit) inline mode

	if($condition)
	{
		$control_TrainingID = $pageObject->getControl("TrainingID", $id);
		$control_TrainingID->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing TrainingID - end
//	processing Attendance - begin
	$condition = $inlineedit==EDIT_INLINE;//($inlineedit) inline mode

	if($condition)
	{
		$control_Attendance = $pageObject->getControl("Attendance", $id);
		$control_Attendance->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing Attendance - end
//	processing Training_Date - begin
	$condition = $inlineedit==EDIT_INLINE;//($inlineedit) inline mode

	if($condition)
	{
		$control_Training_Date = $pageObject->getControl("Training_Date", $id);
		$control_Training_Date->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing Training_Date - end
//	processing Venue - begin
	$condition = $inlineedit==EDIT_INLINE;//($inlineedit) inline mode

	if($condition)
	{
		$control_Venue = $pageObject->getControl("Venue", $id);
		$control_Venue->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing Venue - end
//	processing Provider - begin
	$condition = $inlineedit==EDIT_INLINE;//($inlineedit) inline mode

	if($condition)
	{
		$control_Provider = $pageObject->getControl("Provider", $id);
		$control_Provider->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing Provider - end
	...
	//	do event
		...
		{		
			...
			//	processing TrainingID - begin
				...
				if($condition)
				{
					$control_TrainingID->afterSuccessfulSave();
				}
	//	processing TrainingID - end
			//	processing Attendance - begin
							$condition = $inlineedit==EDIT_INLINE;//($inlineedit) inline mode
				...
				if($condition)
				{
					$control_Attendance->afterSuccessfulSave();
				}
	//	processing Attendance - end
			//	processing Training_Date - begin
							$condition = $inlineedit==EDIT_INLINE;//($inlineedit) inline mode
				...
				if($condition)
				{
					$control_Training_Date->afterSuccessfulSave();
				}
	//	processing Training_Date - end
			//	processing Venue - begin
							$condition = $inlineedit==EDIT_INLINE;//($inlineedit) inline mode
				...
				if($condition)
				{
					$control_Venue->afterSuccessfulSave();
				}
	//	processing Venue - end
			//	processing Provider - begin
							$condition = $inlineedit==EDIT_INLINE;//($inlineedit) inline mode
				...
				if($condition)
				{
					$control_Provider->afterSuccessfulSave();
				}
	//	processing Provider - end
...
if ($IsSaved && no_output_done() && $inlineedit == EDIT_SIMPLE)
{
	...
	// redirect
	header("Location: staff_compulsory_training_".$pageObject->getPageType().".php?".$keyGetQ);
...
if(!$data)
{
	...
	{
		header("Location: staff_compulsory_training_list.php?a=return");
		exit();
	}
	else
		$data = array();
...
if($readevalues)
{
	$data["TrainingID"] = $evalues["TrainingID"];
	$data["Attendance"] = $evalues["Attendance"];
	$data["Training_Date"] = $evalues["Training_Date"];
	$data["Venue"] = $evalues["Venue"];
	$data["Provider"] = $evalues["Provider"];
}
...
//	TrainingID - 
	$value = $pageObject->showDBValue("TrainingID", $data, $keylink);
	$showValues["TrainingID"] = $value;
	$showFields[] = "TrainingID";
		$showRawValues["TrainingID"] = substr($data["TrainingID"],0,100);

//	Attendance - 
	$value = $pageObject->showDBValue("Attendance", $data, $keylink);
	$showValues["Attendance"] = $value;
	$showFields[] = "Attendance";
		$showRawValues["Attendance"] = substr($data["Attendance"],0,100);

//	Training_Date - Short Date
	$value = $pageObject->showDBValue("Training_Date", $data, $keylink);
	$showValues["Training_Date"] = $value;
	$showFields[] = "Training_Date";
		$showRawValues["Training_Date"] = substr($data["Training_Date"],0,100);

//	Venue - 
	$value = $pageObject->showDBValue("Venue", $data, $keylink);
	$showValues["Venue"] = $value;
	$showFields[] = "Venue";
		$showRawValues["Venue"] = substr($data["Venue"],0,100);

//	Provider - 
	$value = $pageObject->showDBValue("Provider", $data, $keylink);
	$showValues["Provider"] = $value;
	$showFields[] = "Provider";
		$showRawValues["Provider"] = substr($data["Provider"],0,100);
/////////////////////////////////////////////////////////////
//	start inline output
/////////////////////////////////////////////////////////////
...
if($pageObject->isShowDetailTables && $inlineedit!=EDIT_INLINE && !isMobile())
{
	...
	for($d=0;$d<count($dpParams['ids']);$d++)
	{
		...
		$options['masterTable'] = "staff_compulsory_training";
		$options['firstTime'] = 1;
...
$xt->assign("viewlink_attrs","id=\"viewButton".$id."\" name=\"viewButton".$id."\" onclick=\"window.location.href='staff_compulsory_training_view.php?".$viewlink."'\"");
