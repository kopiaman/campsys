...
add_nocache_headers();
include("include/exam_eligible_seatno_variables.php");
...
if($inlineadd == ADD_INLINE)
	$templatefile = "exam_eligible_seatno_inline_add.htm";
else
	$templatefile = "exam_eligible_seatno_add.htm";
...
if(isset($_REQUEST['afteradd'])){
		header('Location: exam_eligible_seatno_add.php');
...
//	processing Seat - start
	$inlineAddOption = true;
	$inlineAddOption = $inlineadd==ADD_INLINE;
	if($inlineAddOption)
	{
		$control_Seat = $pageObject->getControl("Seat", $id);
		$control_Seat->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing Seat - end
...
//	processing Seat - start
			...
			if($inlineAddOption)
			{
				$control_Seat->afterSuccessfulSave();
			}
//	processing Seat - end
			...
			if($inlineadd==ADD_SIMPLE || $inlineadd==ADD_MASTER)
			{
				...
				if (count($keys))
				{
					...
					if($pageObject->pSet->hasEditPage() && $permis['edit'])
						$message .='&nbsp;<a href=\'exam_eligible_seatno_edit.php?'.$keylink.'\'>'."Edit".'</a>&nbsp;';
					if($pageObject->pSet->hasViewPage() && $permis['search'])
						$message .='&nbsp;<a href=\'exam_eligible_seatno_view.php?'.$keylink.'\'>'."View".'</a>&nbsp;';
				}
				$mesClass = "mes_ok";	
			}
		}
...
if (no_output_done() && $inlineadd==ADD_SIMPLE && $IsSaved)
{
	...
	// redirect
	header("Location: exam_eligible_seatno_".$pageObject->getPageType().".php");
...
//	Seat
	...
	if($display)
	{	
		$value = $pageObject->showDBValue("Seat", $data, $keylink);
		$showValues["Seat"] = $value;
		$showFields[] = "Seat";
	}	
//	BatchID
	$display = false;
...
//	Semester
	...
	{	
		...
		$showRawValues["Seat"] = substr($data["Seat"],0,100);
...
if($pageObject->isShowDetailTables && ($inlineadd==ADD_SIMPLE || $inlineadd==ADD_POPUP) && !isMobile())
{
	...
	for($d=0;$d<count($dpParams['ids']);$d++)
	{
		...
		$options['masterTable'] = "exam_eligible_seatno";
		$options['firstTime'] = 1;
