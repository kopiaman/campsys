...
$templatefile = ($inlineedit == EDIT_INLINE) ? "calendar_qa_inline_edit.htm" : "calendar_qa_edit.htm";
...
//	processing date_start - begin
	$condition = $inlineedit==EDIT_INLINE;//($inlineedit) inline mode

	if($condition)
	{
		$control_date_start = $pageObject->getControl("date_start", $id);
		$control_date_start->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing date_start - end
//	processing date_end - begin
	$condition = $inlineedit==EDIT_INLINE;//($inlineedit) inline mode

	if($condition)
	{
		$control_date_end = $pageObject->getControl("date_end", $id);
		$control_date_end->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing date_end - end
	...
	//	processing category - end
			//	processing date_start - begin
				...
				if($condition)
				{
					$control_date_start->afterSuccessfulSave();
				}
	//	processing date_start - end
			//	processing date_end - begin
							$condition = $inlineedit==EDIT_INLINE;//($inlineedit) inline mode
				...
				if($condition)
				{
					$control_date_end->afterSuccessfulSave();
				}
	//	processing date_end - end
...
if ($IsSaved && no_output_done() && $inlineedit == EDIT_SIMPLE)
{
	...
	// redirect
	header("Location: calendar_qa_".$pageObject->getPageType().".php?".$keyGetQ);
...
if(!$data)
{
	...
	{
		header("Location: calendar_qa_list.php?a=return");
		exit();
	}
	else
		$data = array();
...
if($readevalues)
{
	...
	$data["date_start"] = $evalues["date_start"];
	$data["date_end"] = $evalues["date_end"];
}
...
//	branch_id - 
	$value = $pageObject->showDBValue("branch_id", $data, $keylink);
	$showValues["branch_id"] = $value;
	$showFields[] = "branch_id";
		$showRawValues["branch_id"] = substr($data["branch_id"],0,100);
...
//	date_start - Short Date
	$value = $pageObject->showDBValue("date_start", $data, $keylink);
	$showValues["date_start"] = $value;
	$showFields[] = "date_start";
		$showRawValues["date_start"] = substr($data["date_start"],0,100);

//	date_end - Short Date
	$value = $pageObject->showDBValue("date_end", $data, $keylink);
	$showValues["date_end"] = $value;
	$showFields[] = "date_end";
		$showRawValues["date_end"] = substr($data["date_end"],0,100);

//	days - 
	$value = $pageObject->showDBValue("days", $data, $keylink);
	$showValues["days"] = $value;
	$showFields[] = "days";
		$showRawValues["days"] = substr($data["days"],0,100);
/////////////////////////////////////////////////////////////
//	start inline output
/////////////////////////////////////////////////////////////
...
if($pageObject->isShowDetailTables && $inlineedit!=EDIT_INLINE && !isMobile())
{
	...
	for($d=0;$d<count($dpParams['ids']);$d++)
	{
		...
		$options['masterTable'] = "calendar_qa";
		$options['firstTime'] = 1;
...
$xt->assign("viewlink_attrs","id=\"viewButton".$id."\" name=\"viewButton".$id."\" onclick=\"window.location.href='calendar_qa_view.php?".$viewlink."'\"");
