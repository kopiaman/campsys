...
include("include/Student_Data_Generator_variables.php");
...
$layout->blocks["top"][] = "search";$page_layouts["Student_Data_Generator_search"] = $layout;
...
$mode = SEARCH_SIMPLE;
$templatefile = "Student_Data_Generator_search.htm";
...
$params['shortTableName'] = 'Student_Data_Generator';
...
if ($mode==SEARCH_SIMPLE)
{
	...
	if($pageObject->pSet->getLookupTable("data_status"))
		$pageObject->settingsMap["globalSettings"]['shortTNames'][$pageObject->pSet->getLookupTable("data_status")] = GetTableURL($pageObject->pSet->getLookupTable("data_status"));
	
	$pageObject->fillFieldToolTips("data_status");	
	
	$srchFields = $pageObject->searchClauseObj->getSearchCtrlParams("data_status");
	$firstFieldParams = array();
	if (count($srchFields))
	{
		$firstFieldParams = $srchFields[0];
	...
	{
		$firstFieldParams['fName'] = "data_status";
		$firstFieldParams['eType'] = '';
		$firstFieldParams['value1'] = '';
		$firstFieldParams['opt'] = '';
		$firstFieldParams['value2'] = '';
	...
	if(isEnableSection508())
		$xt->assign_section("data_status_label","<label for=\"".GetInputElementId("data_status", $id, PAGE_SEARCH)."\">","</label>");
	else 
		$xt->assign("data_status_label", true);
	
	$xt->assign("data_status_fieldblock", true);
	$xt->assignbyref("data_status_editcontrol", $ctrlBlockArr['searchcontrol']);
	$xt->assign("data_status_notbox", $ctrlBlockArr['notbox']);
	// create second control, if need it
	$xt->assignbyref("data_status_editcontrol1", $ctrlBlockArr['searchcontrol1']);
	// create search type select
	$xt->assign("searchtype_data_status", $ctrlBlockArr['searchtype']);
	$isFieldNeedSecCtrl = $searchControlBuilder->isNeedSecondCtrl("data_status");
	$ctrlInd = 0;
	if ($isFieldNeedSecCtrl)
	{
		$pageObject->controlsMap["search"]["searchBlocks"][] = array('fName'=>"data_status", 'recId'=>$id, 'ctrlsMap'=>array(0=>$ctrlInd, 1=>($ctrlInd+1)));
		$ctrlInd+=2;
	}
	else
	{
		$pageObject->controlsMap["search"]["searchBlocks"][] = array('fName'=>"data_status", 'recId'=>$id, 'ctrlsMap'=>array(0=>$ctrlInd));			
		$ctrlInd++;
	}
	// search fields data
