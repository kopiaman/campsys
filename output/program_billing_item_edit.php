...
$templatefile = ($inlineedit == EDIT_INLINE) ? "program_billing_item_inline_edit.htm" : "program_billing_item_edit.htm";
...
if ($IsSaved && no_output_done() && $inlineedit == EDIT_SIMPLE)
{
	...
	// redirect
	header("Location: program_billing_item_".$pageObject->getPageType().".php?".$keyGetQ);
...
if(!$data)
{
	...
	{
		header("Location: program_billing_item_list.php?a=return");
		exit();
	}
	else
		$data = array();
...
//	program_billing_ID - 
	$value = $pageObject->showDBValue("program_billing_ID", $data, $keylink);
	$showValues["program_billing_ID"] = $value;
	$showFields[] = "program_billing_ID";
		$showRawValues["program_billing_ID"] = substr($data["program_billing_ID"],0,100);
...
if($pageObject->isShowDetailTables && $inlineedit!=EDIT_INLINE && !isMobile())
{
	...
	for($d=0;$d<count($dpParams['ids']);$d++)
	{
		...
		$options['masterTable'] = "program_billing_item";
		$options['firstTime'] = 1;
...
$xt->assign("viewlink_attrs","id=\"viewButton".$id."\" name=\"viewButton".$id."\" onclick=\"window.location.href='program_billing_item_view.php?".$viewlink."'\"");
