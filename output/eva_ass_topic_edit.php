...
$templatefile = ($inlineedit == EDIT_INLINE) ? "eva_ass_topic_inline_edit.htm" : "eva_ass_topic_edit.htm";
...
if ($IsSaved && no_output_done() && $inlineedit == EDIT_SIMPLE)
{
	...
	// redirect
	header("Location: eva_ass_topic_".$pageObject->getPageType().".php?".$keyGetQ);
...
if(!$data)
{
	...
	{
		header("Location: eva_ass_topic_list.php?a=return");
		exit();
	}
	else
		$data = array();
...
//	detail tables
	$showDetailKeys["eva_ass_taskno"]["masterkey1"] = $data["sub_topic"];		
...
if($pageObject->isShowDetailTables && $inlineedit!=EDIT_INLINE && !isMobile())
{
	...
	for($d=0;$d<count($dpParams['ids']);$d++)
	{
		...
		$options['masterTable'] = "eva_ass_topic";
		$options['firstTime'] = 1;
...
$xt->assign("viewlink_attrs","id=\"viewButton".$id."\" name=\"viewButton".$id."\" onclick=\"window.location.href='eva_ass_topic_view.php?".$viewlink."'\"");
