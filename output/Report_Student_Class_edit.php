...
$templatefile = ($inlineedit == EDIT_INLINE) ? "Report_Student_Class_inline_edit.htm" : "Report_Student_Class_edit.htm";
...
if ($IsSaved && no_output_done() && $inlineedit == EDIT_SIMPLE)
{
	...
	// redirect
	header("Location: Report_Student_Class_".$pageObject->getPageType().".php?".$keyGetQ);
...
if(!$data)
{
	...
	{
		header("Location: Report_Student_Class_list.php?a=return");
		exit();
	}
	else
		$data = array();
...
//	Intake - 
	$value = $pageObject->showDBValue("Intake", $data, $keylink);
	$showValues["Intake"] = $value;
	$showFields[] = "Intake";
...
if($pageObject->isShowDetailTables && $inlineedit!=EDIT_INLINE && !isMobile())
{
	...
	for($d=0;$d<count($dpParams['ids']);$d++)
	{
		...
		$options['masterTable'] = "Report_Student_Class";
		$options['firstTime'] = 1;
		
		$strTableName = $dpParams['strTableNames'][$d];
		
		if(!CheckSecurity(@$_SESSION["_".$strTableName."_OwnerID"],"Search")){
			$strTableName = "Report_Student_Class";
			continue;
		}
...
$xt->assign("viewlink_attrs","id=\"viewButton".$id."\" name=\"viewButton".$id."\" onclick=\"window.location.href='Report_Student_Class_view.php?".$viewlink."'\"");
