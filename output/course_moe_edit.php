...
$templatefile = ($inlineedit == EDIT_INLINE) ? "course_moe_inline_edit.htm" : "course_moe_edit.htm";
...
//	processing Code - end
//	processing Shortname - begin
	$condition = $inlineedit==EDIT_INLINE;//($inlineedit) inline mode

	if($condition)
	{
		$control_Shortname = $pageObject->getControl("Shortname", $id);
		$control_Shortname->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing Shortname - end
...
//	processing sem - end
	...
	//	processing Code - end
			//	processing Shortname - begin
				...
				if($condition)
				{
					$control_Shortname->afterSuccessfulSave();
				}
	//	processing Shortname - end
			//	processing sem - begin
							$condition = $inlineedit==EDIT_INLINE;//($inlineedit) inline mode
...
if ($IsSaved && no_output_done() && $inlineedit == EDIT_SIMPLE)
{
	...
	// redirect
	header("Location: course_moe_".$pageObject->getPageType().".php?".$keyGetQ);
...
if(!$data)
{
	...
	{
		header("Location: course_moe_list.php?a=return");
		exit();
	}
	else
		$data = array();
...
if($readevalues)
{
	...
	$data["Shortname"] = $evalues["Shortname"];
...
//	Shortname - 
	$value = $pageObject->showDBValue("Shortname", $data, $keylink);
	$showValues["Shortname"] = $value;
	$showFields[] = "Shortname";
		$showRawValues["Shortname"] = substr($data["Shortname"],0,100);

//	color - 
	$value = $pageObject->showDBValue("color", $data, $keylink);
	$showValues["color"] = $value;
	$showFields[] = "color";
		$showRawValues["color"] = substr($data["color"],0,100);
...
if($pageObject->isShowDetailTables && $inlineedit!=EDIT_INLINE && !isMobile())
{
	...
	for($d=0;$d<count($dpParams['ids']);$d++)
	{
		...
		$options['masterTable'] = "course_moe";
		$options['firstTime'] = 1;
...
$xt->assign("viewlink_attrs","id=\"viewButton".$id."\" name=\"viewButton".$id."\" onclick=\"window.location.href='course_moe_view.php?".$viewlink."'\"");
