...
include("include/branch_program_variables.php");
...
$templatefile = ($inlineedit == EDIT_INLINE) ? "branch_program_inline_edit.htm" : "branch_program_edit.htm";
...
//	processing branch_id - begin
	$condition = $inlineedit==EDIT_INLINE;//($inlineedit) inline mode

	if($condition)
	{
		$control_branch_id = $pageObject->getControl("branch_id", $id);
		$control_branch_id->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing branch_id - end
//	processing program_id - begin
	$condition = $inlineedit==EDIT_INLINE;//($inlineedit) inline mode

	if($condition)
	{
		$control_program_id = $pageObject->getControl("program_id", $id);
		$control_program_id->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing program_id - end
	...
	//	do event
		...
		{		
			...
			//	processing branch_id - begin
				...
				if($condition)
				{
					$control_branch_id->afterSuccessfulSave();
				}
	//	processing branch_id - end
			//	processing program_id - begin
							$condition = $inlineedit==EDIT_INLINE;//($inlineedit) inline mode
				...
				if($condition)
				{
					$control_program_id->afterSuccessfulSave();
				}
	//	processing program_id - end
...
if ($IsSaved && no_output_done() && $inlineedit == EDIT_SIMPLE)
{
	...
	// redirect
	header("Location: branch_program_".$pageObject->getPageType().".php?".$keyGetQ);
...
if(!$data)
{
	...
	{
		header("Location: branch_program_list.php?a=return");
		exit();
	}
	else
		$data = array();
...
if($readevalues)
{
	$data["branch_id"] = $evalues["branch_id"];
	$data["program_id"] = $evalues["program_id"];
}
...
//	program_id - 
	$value = $pageObject->showDBValue("program_id", $data, $keylink);
	$showValues["program_id"] = $value;
	$showFields[] = "program_id";
		$showRawValues["program_id"] = substr($data["program_id"],0,100);
/////////////////////////////////////////////////////////////
//	start inline output
/////////////////////////////////////////////////////////////
...
if($pageObject->isShowDetailTables && $inlineedit!=EDIT_INLINE && !isMobile())
{
	...
	for($d=0;$d<count($dpParams['ids']);$d++)
	{
		...
		$options['masterTable'] = "branch_program";
		$options['firstTime'] = 1;
...
$xt->assign("viewlink_attrs","id=\"viewButton".$id."\" name=\"viewButton".$id."\" onclick=\"window.location.href='branch_program_view.php?".$viewlink."'\"");
