...
$sqlmax = "
		...
		AND batch=$batch AND class=$class AND program=$programid
    ...
    <table width="49%" border="0" class="tablehead">
      <tr>
        ...
        <td width="78%"> Time Table <br>
          ...
          <br></td>
        <td width="17%" class="navi"><a href="x_timetable_class.php?week=<?php echo $week-1?>&&year=<?php echo $year?>&&programid=<?php echo $programid?>&&batch=<?php echo $batch?>&&class=<?php echo $class?>">PREV WEEK</a><br>
          <a href="x_timetable_class.php?week=<?php echo $week+1?>&&year=<?php echo $year?>&&programid=<?php echo $programid?>&&batch=<?php echo $batch?>&&class=<?php echo $class?>">NEXT WEEK</a></td>
      </tr>
    </table>
  </div>
  <?php
...
{	
	...
	//---check maximum time------ ..if end time more than 3 pm than the col span will expand
	  ...
	  AND t_facilities_timetable.program=$programid AND t_facilities_timetable.batch=$batch AND t_facilities_timetable.class=$class 
	  ORDER BY end_time ";
...
/*	
	if($days == 1){ $hari = "Monday"; } else if($days == 2){ $hari = "Tuesday"; } else if($days == 3){ $hari = "Wednesday"; } else if($days == 4){ $hari = "Thursday"; } else if($days == 5){ $hari = "Friday"; } else if($days == 6){ $hari = "Saturday"; } else if($days == 7){ $hari = "Sunday"; }
	*/
...
"; 
				...
				while($out = mysql_fetch_array($result_db))
					 { 
					    $colspan = 1;
					 	$course = $out['module'];
						$lect = $out['staff'];$lect2 = $out['staff2'];
						$day = $out['day'];
						...
						$color=$out['color'];
							...
							if($lect2){ //show staff 2 if exist	
		 					   $lect2text="/ ".$lect2; 
								}else{  $lect2text=''; }
		
							echo '<br><strong>'.$course."</strong><br/>".$lect.$lect2text."<br> (".$room.")";
							$count=1;
							echo "</color></p></div></td>";
							} 
