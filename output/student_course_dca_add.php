...
if($inlineadd == ADD_INLINE)
	$templatefile = "student_course_dca_inline_add.htm";
else
	$templatefile = "student_course_dca_add.htm";
...
if(isset($_REQUEST['afteradd'])){
		header('Location: student_course_dca_add.php');
...
//	processing Exam_Status - start
	$inlineAddOption = true;
	$inlineAddOption = $inlineadd==ADD_INLINE;
	if($inlineAddOption)
	{
		$control_Exam_Status = $pageObject->getControl("Exam_Status", $id);
		$control_Exam_Status->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
...
//	insert masterkey value if exists and if not specified
	if(@$_SESSION[$sessionPrefix."_mastertable"]=="student_course_DCA_List_student2")
	{
			...
			if($inlineadd==ADD_SIMPLE || $inlineadd==ADD_MASTER)
			{
				...
				if (count($keys))
				{
					...
					if($pageObject->pSet->hasEditPage() && $permis['edit'])
						$message .='&nbsp;<a href=\'student_course_dca_edit.php?'.$keylink.'\'>'."Edit".'</a>&nbsp;';
					if($pageObject->pSet->hasViewPage() && $permis['search'])
						$message .='&nbsp;<a href=\'student_course_dca_view.php?'.$keylink.'\'>'."View".'</a>&nbsp;';
				}
				$mesClass = "mes_ok";	
			}
		}
...
if (no_output_done() && $inlineadd==ADD_SIMPLE && $IsSaved)
{
	...
	// redirect
	header("Location: student_course_dca_".$pageObject->getPageType().".php");
