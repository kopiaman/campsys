...
//	processing father - begin
	$condition = $inlineedit==EDIT_INLINE;//($inlineedit) inline mode

	if($condition)
	{
		$control_father = $pageObject->getControl("father", $id);
		$control_father->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing father - end
//	processing father_tel - begin
	$condition = $inlineedit==EDIT_INLINE;//($inlineedit) inline mode

	if($condition)
	{
		$control_father_tel = $pageObject->getControl("father_tel", $id);
		$control_father_tel->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing father_tel - end
//	processing mother - begin
	$condition = $inlineedit==EDIT_INLINE;//($inlineedit) inline mode

	if($condition)
	{
		$control_mother = $pageObject->getControl("mother", $id);
		$control_mother->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing mother - end
//	processing mother_tel - begin
	$condition = $inlineedit==EDIT_INLINE;//($inlineedit) inline mode

	if($condition)
	{
		$control_mother_tel = $pageObject->getControl("mother_tel", $id);
		$control_mother_tel->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing mother_tel - end
	...
	//	do event
		...
		{		
			...
			//	processing father - begin
				...
				if($condition)
				{
					$control_father->afterSuccessfulSave();
				}
	//	processing father - end
			//	processing father_tel - begin
							$condition = $inlineedit==EDIT_INLINE;//($inlineedit) inline mode
				...
				if($condition)
				{
					$control_father_tel->afterSuccessfulSave();
				}
	//	processing father_tel - end
			//	processing mother - begin
							$condition = $inlineedit==EDIT_INLINE;//($inlineedit) inline mode
				...
				if($condition)
				{
					$control_mother->afterSuccessfulSave();
				}
	//	processing mother - end
			//	processing mother_tel - begin
							$condition = $inlineedit==EDIT_INLINE;//($inlineedit) inline mode
				...
				if($condition)
				{
					$control_mother_tel->afterSuccessfulSave();
				}
	//	processing mother_tel - end
			//	processing Address1 - begin
							$condition = $inlineedit==EDIT_INLINE;//($inlineedit) inline mode
...
if($readevalues)
{
	$data["father"] = $evalues["father"];
	$data["father_tel"] = $evalues["father_tel"];
	$data["mother"] = $evalues["mother"];
	$data["mother_tel"] = $evalues["mother_tel"];
...
//	father - 
	$value = $pageObject->showDBValue("father", $data, $keylink);
	$showValues["father"] = $value;
	$showFields[] = "father";
		$showRawValues["father"] = substr($data["father"],0,100);

//	father_tel - 
	$value = $pageObject->showDBValue("father_tel", $data, $keylink);
	$showValues["father_tel"] = $value;
	$showFields[] = "father_tel";
		$showRawValues["father_tel"] = substr($data["father_tel"],0,100);

//	mother - 
	$value = $pageObject->showDBValue("mother", $data, $keylink);
	$showValues["mother"] = $value;
	$showFields[] = "mother";
		$showRawValues["mother"] = substr($data["mother"],0,100);

//	mother_tel - 
	$value = $pageObject->showDBValue("mother_tel", $data, $keylink);
	$showValues["mother_tel"] = $value;
	$showFields[] = "mother_tel";
		$showRawValues["mother_tel"] = substr($data["mother_tel"],0,100);
