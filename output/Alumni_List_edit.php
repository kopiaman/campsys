...
//	processing job - begin
	$condition = $inlineedit!=EDIT_INLINE;//(!$inlineedit) edit simple mode

	if($condition)
	{
		$control_job = $pageObject->getControl("job", $id);
		$control_job->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing job - end
//	processing licences_no - begin
	$condition = $inlineedit!=EDIT_INLINE;//(!$inlineedit) edit simple mode

	if($condition)
	{
		$control_licences_no = $pageObject->getControl("licences_no", $id);
		$control_licences_no->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing licences_no - end
	...
	//	processing blood - end
			//	processing job - begin
				...
				if($condition)
				{
					$control_job->afterSuccessfulSave();
				}
	//	processing job - end
			//	processing licences_no - begin
							$condition = $inlineedit!=EDIT_INLINE;//(!$inlineedit) edit simple mode
				...
				if($condition)
				{
					$control_licences_no->afterSuccessfulSave();
				}
	//	processing licences_no - end
...
if($readevalues)
{
	...
	$data["job"] = $evalues["job"];
	$data["licences_no"] = $evalues["licences_no"];
}
...
if($inlineedit != EDIT_INLINE)
{
	...
	if(isEnableSection508())
		$xt->assign_section("job_label","<label for=\"".GetInputElementId("job", $id, PAGE_EDIT)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("licences_no_label","<label for=\"".GetInputElementId("licences_no", $id, PAGE_EDIT)."\">","</label>");
