...
$templatefile = ($inlineedit == EDIT_INLINE) ? "student_course_dca_inline_edit.htm" : "student_course_dca_edit.htm";
...
//	processing Exam_Status - begin
	$condition = $inlineedit==EDIT_INLINE;//($inlineedit) inline mode

	if($condition)
	{
		$control_Exam_Status = $pageObject->getControl("Exam_Status", $id);
		$control_Exam_Status->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing Exam_Status - end
	...
	//	do event
		...
		{		
			...
			//	processing Exam_Status - begin
				...
				if($condition)
				{
					$control_Exam_Status->afterSuccessfulSave();
				}
	//	processing Exam_Status - end
			//	processing Exam_Remark - begin
							$condition = $inlineedit==EDIT_INLINE;//($inlineedit) inline mode
...
if ($IsSaved && no_output_done() && $inlineedit == EDIT_SIMPLE)
{
	...
	// redirect
	header("Location: student_course_dca_".$pageObject->getPageType().".php?".$keyGetQ);
...
if(!$data)
{
	...
	{
		header("Location: student_course_dca_list.php?a=return");
		exit();
	}
	else
		$data = array();
...
if($readevalues)
{
	$data["Exam_Status"] = $evalues["Exam_Status"];
...
//	Exam_Status - 
	$value = $pageObject->showDBValue("Exam_Status", $data, $keylink);
	$showValues["Exam_Status"] = $value;
	$showFields[] = "Exam_Status";
		$showRawValues["Exam_Status"] = substr($data["Exam_Status"],0,100);
...
//	ProgramID - 
	$value = $pageObject->showDBValue("ProgramID", $data, $keylink);
	$showValues["ProgramID"] = $value;
	$showFields[] = "ProgramID";
		$showRawValues["ProgramID"] = substr($data["ProgramID"],0,100);
...
if($pageObject->isShowDetailTables && $inlineedit!=EDIT_INLINE && !isMobile())
{
	...
	for($d=0;$d<count($dpParams['ids']);$d++)
	{
		...
		$options['masterTable'] = "student_course_dca";
		$options['firstTime'] = 1;
...
$xt->assign("viewlink_attrs","id=\"viewButton".$id."\" name=\"viewButton".$id."\" onclick=\"window.location.href='student_course_dca_view.php?".$viewlink."'\"");
