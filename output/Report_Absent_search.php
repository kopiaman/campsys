...
$layout->blocks["top"][] = "search";$page_layouts["Report_Absent_search"] = $layout;
...
$mode = SEARCH_SIMPLE;
$templatefile = "Report_Absent_search.htm";
...
$params['shortTableName'] = 'Report_Absent';
...
if ($mode==SEARCH_SIMPLE)
{
	...
	if($pageObject->pSet->getLookupTable("ABS"))
		$pageObject->settingsMap["globalSettings"]['shortTNames'][$pageObject->pSet->getLookupTable("ABS")] = GetTableURL($pageObject->pSet->getLookupTable("ABS"));
	
	$pageObject->fillFieldToolTips("ABS");	
	
	$srchFields = $pageObject->searchClauseObj->getSearchCtrlParams("ABS");
	$firstFieldParams = array();
	if (count($srchFields))
	{
		$firstFieldParams = $srchFields[0];
	...
	if(isEnableSection508())
		$xt->assign_section("ABS_label","<label for=\"".GetInputElementId("ABS", $id, PAGE_SEARCH)."\">","</label>");
	else 
		$xt->assign("ABS_label", true);
	
	$xt->assign("ABS_fieldblock", true);
	$xt->assignbyref("ABS_editcontrol", $ctrlBlockArr['searchcontrol']);
	$xt->assign("ABS_notbox", $ctrlBlockArr['notbox']);
	// create second control, if need it
	$xt->assignbyref("ABS_editcontrol1", $ctrlBlockArr['searchcontrol1']);
	// create search type select
	$xt->assign("searchtype_ABS", $ctrlBlockArr['searchtype']);
	$isFieldNeedSecCtrl = $searchControlBuilder->isNeedSecondCtrl("ABS");
	$ctrlInd = 0;
	if ($isFieldNeedSecCtrl)
	{
		$pageObject->controlsMap["search"]["searchBlocks"][] = array('fName'=>"ABS", 'recId'=>$id, 'ctrlsMap'=>array(0=>$ctrlInd, 1=>($ctrlInd+1)));
		$ctrlInd+=2;
	}
	else
	{
		$pageObject->controlsMap["search"]["searchBlocks"][] = array('fName'=>"ABS", 'recId'=>$id, 'ctrlsMap'=>array(0=>$ctrlInd));			
		$ctrlInd++;
	}
	// search fields data
	
	if($pageObject->pSet->getLookupTable("total_hour"))
		$pageObject->settingsMap["globalSettings"]['shortTNames'][$pageObject->pSet->getLookupTable("total_hour")] = GetTableURL($pageObject->pSet->getLookupTable("total_hour"));
	
	$pageObject->fillFieldToolTips("total_hour");	
	
	$srchFields = $pageObject->searchClauseObj->getSearchCtrlParams("total_hour");
	$firstFieldParams = array();
	if (count($srchFields))
	{
		$firstFieldParams = $srchFields[0];
	...
	{
		$firstFieldParams['fName'] = "total_hour";
		$firstFieldParams['eType'] = '';
		$firstFieldParams['value1'] = '';
		$firstFieldParams['opt'] = '';
		$firstFieldParams['value2'] = '';
	...
	if(isEnableSection508())
		$xt->assign_section("total_hour_label","<label for=\"".GetInputElementId("total_hour", $id, PAGE_SEARCH)."\">","</label>");
	else 
		$xt->assign("total_hour_label", true);
	
	$xt->assign("total_hour_fieldblock", true);
	$xt->assignbyref("total_hour_editcontrol", $ctrlBlockArr['searchcontrol']);
	$xt->assign("total_hour_notbox", $ctrlBlockArr['notbox']);
	// create second control, if need it
	$xt->assignbyref("total_hour_editcontrol1", $ctrlBlockArr['searchcontrol1']);
	// create search type select
	$xt->assign("searchtype_total_hour", $ctrlBlockArr['searchtype']);
	$isFieldNeedSecCtrl = $searchControlBuilder->isNeedSecondCtrl("total_hour");
	$ctrlInd = 0;
	if ($isFieldNeedSecCtrl)
	{
		$pageObject->controlsMap["search"]["searchBlocks"][] = array('fName'=>"total_hour", 'recId'=>$id, 'ctrlsMap'=>array(0=>$ctrlInd, 1=>($ctrlInd+1)));
		$ctrlInd+=2;
	}
	else
	{
		$pageObject->controlsMap["search"]["searchBlocks"][] = array('fName'=>"total_hour", 'recId'=>$id, 'ctrlsMap'=>array(0=>$ctrlInd));			
		$ctrlInd++;
	}
	// search fields data
