...
$layout->blocks["top"][] = "details";$page_layouts["exam_creator_edit"] = $layout;
...
$templatefile = "exam_creator_edit.htm";
...
//	processing exam_remark - begin
	$condition = $inlineedit!=EDIT_INLINE;//(!$inlineedit) edit simple mode

	if($condition)
	{
		$control_exam_remark = $pageObject->getControl("exam_remark", $id);
		$control_exam_remark->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);
...
if ($IsSaved && no_output_done() && $inlineedit == EDIT_SIMPLE)
{
	...
	// redirect
	header("Location: exam_creator_".$pageObject->getPageType().".php?".$keyGetQ);
...
if(!$data)
{
	...
	{
		header("Location: exam_creator_list.php?a=return");
		exit();
	}
	else
		$data = array();
...
if($readevalues)
{
	...
	$data["exam_remark"] = $evalues["exam_remark"];
}
...
if($inlineedit != EDIT_INLINE)
{
	...
	if(isEnableSection508())
		$xt->assign_section("exam_remark_label","<label for=\"".GetInputElementId("exam_remark", $id, PAGE_EDIT)."\">","</label>");
...
$xt->assign("viewlink_attrs","id=\"viewButton".$id."\" name=\"viewButton".$id."\" onclick=\"window.location.href='exam_creator_view.php?".$viewlink."'\"");
