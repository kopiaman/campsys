...
if($inlineadd == ADD_INLINE)
	$templatefile = "q_assessor_inline_add.htm";
else
	$templatefile = "q_assessor_add.htm";
...
if(isset($_REQUEST['afteradd'])){
		header('Location: q_assessor_add.php');
...
//	processing name - end
//	processing ext - start
	$inlineAddOption = true;
	$inlineAddOption = $inlineadd==ADD_INLINE;
	if($inlineAddOption)
	{
		$control_ext = $pageObject->getControl("ext", $id);
		$control_ext->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing ext - end
...
//	processing ext - start
			...
			if($inlineAddOption)
			{
				$control_ext->afterSuccessfulSave();
			}
//	processing ext - end
//	processing email - start
			$inlineAddOption = true;
			...
			if($inlineadd==ADD_SIMPLE || $inlineadd==ADD_MASTER)
			{
				...
				if (count($keys))
				{
					...
					if($pageObject->pSet->hasEditPage() && $permis['edit'])
						$message .='&nbsp;<a href=\'q_assessor_edit.php?'.$keylink.'\'>'."Edit".'</a>&nbsp;';
					if($pageObject->pSet->hasViewPage() && $permis['search'])
						$message .='&nbsp;<a href=\'q_assessor_view.php?'.$keylink.'\'>'."View".'</a>&nbsp;';
				}
				$mesClass = "mes_ok";	
			}
		}
...
if (no_output_done() && $inlineadd==ADD_SIMPLE && $IsSaved)
{
	...
	// redirect
	header("Location: q_assessor_".$pageObject->getPageType().".php");
...
//	ext
	...
	if($display)
	{	
		$value = $pageObject->showDBValue("ext", $data, $keylink);
		$showValues["ext"] = $value;
		$showFields[] = "ext";
	}	
//	email
	$display = false;
...
//	remark
	...
	{	
		...
		$showRawValues["ext"] = substr($data["ext"],0,100);
...
if($pageObject->isShowDetailTables && ($inlineadd==ADD_SIMPLE || $inlineadd==ADD_POPUP) && !isMobile())
{
	...
	for($d=0;$d<count($dpParams['ids']);$d++)
	{
		...
		$options['masterTable'] = "q_assessor";
		$options['firstTime'] = 1;
...
function GetAddedDataLookupQuery($pageObject, $keys, $forLookup)
{
	...
	if(postvalue("table")=="q_assesment" && postvalue("field")=="as_name") 
	{
		$lookupFieldName = "as_name";
		$lookupTable = "q_assessor";
		$linkFieldName = "id";
		$dispfield = "name";
		// Table which contain lookup field!
		$lookupPSet = new ProjectSettings("q_assesment", PAGE_ADD);
		$nLookupType = $lookupPSet->getLookupType($lookupFieldName);
		if($nLookupType == LT_QUERY)
		{
			if($lookupPSet->getCustomDisplay($lookupFieldName))
				$pageObject->pSet->getSQLQuery()->AddCustomExpression($dispfield, $pageObject->pSet, "q_assesment", "as_name");
