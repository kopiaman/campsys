...
if($inlineadd == ADD_INLINE)
	$templatefile = "student_course_moe_inline_add.htm";
else
	$templatefile = "student_course_moe_add.htm";
...
if(isset($_REQUEST['afteradd'])){
		header('Location: student_course_moe_add.php');
...
//	processing sem - end
//	processing Final_Exam - start
	$inlineAddOption = true;
	$inlineAddOption = $inlineadd==ADD_INLINE;
	if($inlineAddOption)
	{
		$control_Final_Exam = $pageObject->getControl("Final_Exam", $id);
		$control_Final_Exam->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
			...
			if($inlineadd==ADD_SIMPLE || $inlineadd==ADD_MASTER)
			{
				...
				if (count($keys))
				{
					...
					if($pageObject->pSet->hasEditPage() && $permis['edit'])
						$message .='&nbsp;<a href=\'student_course_moe_edit.php?'.$keylink.'\'>'."Edit".'</a>&nbsp;';
					if($pageObject->pSet->hasViewPage() && $permis['search'])
						$message .='&nbsp;<a href=\'student_course_moe_view.php?'.$keylink.'\'>'."View".'</a>&nbsp;';
				}
				$mesClass = "mes_ok";	
			}
		}
...
if (no_output_done() && $inlineadd==ADD_SIMPLE && $IsSaved)
{
	...
	// redirect
	header("Location: student_course_moe_".$pageObject->getPageType().".php");
