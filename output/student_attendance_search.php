...
if ($mode==SEARCH_SIMPLE)
{
	...
	if($pageObject->pSet->getLookupTable("attend"))
		$pageObject->settingsMap["globalSettings"]['shortTNames'][$pageObject->pSet->getLookupTable("attend")] = GetTableURL($pageObject->pSet->getLookupTable("attend"));
	
	$pageObject->fillFieldToolTips("attend");	
	
	$srchFields = $pageObject->searchClauseObj->getSearchCtrlParams("attend");
	$firstFieldParams = array();
	if (count($srchFields))
	{
		$firstFieldParams = $srchFields[0];
	...
	if(isEnableSection508())
		$xt->assign_section("attend_label","<label for=\"".GetInputElementId("attend", $id, PAGE_SEARCH)."\">","</label>");
	else 
		$xt->assign("attend_label", true);
	
	$xt->assign("attend_fieldblock", true);
	$xt->assignbyref("attend_editcontrol", $ctrlBlockArr['searchcontrol']);
	$xt->assign("attend_notbox", $ctrlBlockArr['notbox']);
	// create second control, if need it
	$xt->assignbyref("attend_editcontrol1", $ctrlBlockArr['searchcontrol1']);
	// create search type select
	$xt->assign("searchtype_attend", $ctrlBlockArr['searchtype']);
	$isFieldNeedSecCtrl = $searchControlBuilder->isNeedSecondCtrl("attend");
	$ctrlInd = 0;
	if ($isFieldNeedSecCtrl)
	{
		$pageObject->controlsMap["search"]["searchBlocks"][] = array('fName'=>"attend", 'recId'=>$id, 'ctrlsMap'=>array(0=>$ctrlInd, 1=>($ctrlInd+1)));
		$ctrlInd+=2;
	}
	else
	{
		$pageObject->controlsMap["search"]["searchBlocks"][] = array('fName'=>"attend", 'recId'=>$id, 'ctrlsMap'=>array(0=>$ctrlInd));			
		$ctrlInd++;
	}
