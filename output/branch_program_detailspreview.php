...
$layout->blocks["bare"][] = "detailspreviewgrid";$page_layouts["branch_program_detailspreview"] = $layout;
...
if($mastertable == "branch")
{
	$where = "";
		$where .= GetFullFieldName("branch_id", $strTableName, false)."=".make_db_value("branch_id",$_SESSION[$strTableName."_masterkey1"]);
}
...
if($rowcount) {
	...
	//	branch_id - 
		    ...
		    $value = $viewContainer->showDBValue("branch_id", $data, $keylink);
			$row["branch_id_value"] = $value;
	//	program_id - 
			$viewContainer->recId = $recordsCounter;
		    $value = $viewContainer->showDBValue("program_id", $data, $keylink);
			$row["program_id_value"] = $value;
		$rowinfo[] = $row;
		$data = $cipherer->DecryptFetchedArray($rs);
	}
...
$xt->load_template("branch_program_detailspreview.htm");
