...
if ($mode==SEARCH_SIMPLE)
{
	...
	if($pageObject->pSet->getLookupTable("father"))
		$pageObject->settingsMap["globalSettings"]['shortTNames'][$pageObject->pSet->getLookupTable("father")] = GetTableURL($pageObject->pSet->getLookupTable("father"));
	
	$pageObject->fillFieldToolTips("father");	
	
	$srchFields = $pageObject->searchClauseObj->getSearchCtrlParams("father");
	$firstFieldParams = array();
	if (count($srchFields))
	{
		$firstFieldParams = $srchFields[0];
	...
	if(isEnableSection508())
		$xt->assign_section("father_label","<label for=\"".GetInputElementId("father", $id, PAGE_SEARCH)."\">","</label>");
	else 
		$xt->assign("father_label", true);
	
	$xt->assign("father_fieldblock", true);
	$xt->assignbyref("father_editcontrol", $ctrlBlockArr['searchcontrol']);
	$xt->assign("father_notbox", $ctrlBlockArr['notbox']);
	// create second control, if need it
	$xt->assignbyref("father_editcontrol1", $ctrlBlockArr['searchcontrol1']);
	// create search type select
	$xt->assign("searchtype_father", $ctrlBlockArr['searchtype']);
	$isFieldNeedSecCtrl = $searchControlBuilder->isNeedSecondCtrl("father");
	$ctrlInd = 0;
	if ($isFieldNeedSecCtrl)
	{
		$pageObject->controlsMap["search"]["searchBlocks"][] = array('fName'=>"father", 'recId'=>$id, 'ctrlsMap'=>array(0=>$ctrlInd, 1=>($ctrlInd+1)));
		$ctrlInd+=2;
	}
	else
	{
		$pageObject->controlsMap["search"]["searchBlocks"][] = array('fName'=>"father", 'recId'=>$id, 'ctrlsMap'=>array(0=>$ctrlInd));			
		$ctrlInd++;
	}
	// search fields data
	
	if($pageObject->pSet->getLookupTable("father_tel"))
		$pageObject->settingsMap["globalSettings"]['shortTNames'][$pageObject->pSet->getLookupTable("father_tel")] = GetTableURL($pageObject->pSet->getLookupTable("father_tel"));
	
	$pageObject->fillFieldToolTips("father_tel");	
	
	$srchFields = $pageObject->searchClauseObj->getSearchCtrlParams("father_tel");
	$firstFieldParams = array();
	if (count($srchFields))
	{
		$firstFieldParams = $srchFields[0];
	...
	{
		$firstFieldParams['fName'] = "father_tel";
		$firstFieldParams['eType'] = '';
		$firstFieldParams['value1'] = '';
		$firstFieldParams['opt'] = '';
		$firstFieldParams['value2'] = '';
	...
	if(isEnableSection508())
		$xt->assign_section("father_tel_label","<label for=\"".GetInputElementId("father_tel", $id, PAGE_SEARCH)."\">","</label>");
	else 
		$xt->assign("father_tel_label", true);
	
	$xt->assign("father_tel_fieldblock", true);
	$xt->assignbyref("father_tel_editcontrol", $ctrlBlockArr['searchcontrol']);
	$xt->assign("father_tel_notbox", $ctrlBlockArr['notbox']);
	// create second control, if need it
	$xt->assignbyref("father_tel_editcontrol1", $ctrlBlockArr['searchcontrol1']);
	// create search type select
	$xt->assign("searchtype_father_tel", $ctrlBlockArr['searchtype']);
	$isFieldNeedSecCtrl = $searchControlBuilder->isNeedSecondCtrl("father_tel");
	$ctrlInd = 0;
	if ($isFieldNeedSecCtrl)
	{
		$pageObject->controlsMap["search"]["searchBlocks"][] = array('fName'=>"father_tel", 'recId'=>$id, 'ctrlsMap'=>array(0=>$ctrlInd, 1=>($ctrlInd+1)));
		$ctrlInd+=2;
	}
	else
	{
		$pageObject->controlsMap["search"]["searchBlocks"][] = array('fName'=>"father_tel", 'recId'=>$id, 'ctrlsMap'=>array(0=>$ctrlInd));			
		$ctrlInd++;
	}
	// search fields data
	
	if($pageObject->pSet->getLookupTable("mother"))
		$pageObject->settingsMap["globalSettings"]['shortTNames'][$pageObject->pSet->getLookupTable("mother")] = GetTableURL($pageObject->pSet->getLookupTable("mother"));
	
	$pageObject->fillFieldToolTips("mother");	
	
	$srchFields = $pageObject->searchClauseObj->getSearchCtrlParams("mother");
	$firstFieldParams = array();
	if (count($srchFields))
	{
		$firstFieldParams = $srchFields[0];
	...
	if(isEnableSection508())
		$xt->assign_section("mother_label","<label for=\"".GetInputElementId("mother", $id, PAGE_SEARCH)."\">","</label>");
	else 
		$xt->assign("mother_label", true);
	
	$xt->assign("mother_fieldblock", true);
	$xt->assignbyref("mother_editcontrol", $ctrlBlockArr['searchcontrol']);
	$xt->assign("mother_notbox", $ctrlBlockArr['notbox']);
	// create second control, if need it
	$xt->assignbyref("mother_editcontrol1", $ctrlBlockArr['searchcontrol1']);
	// create search type select
	$xt->assign("searchtype_mother", $ctrlBlockArr['searchtype']);
	$isFieldNeedSecCtrl = $searchControlBuilder->isNeedSecondCtrl("mother");
	$ctrlInd = 0;
	if ($isFieldNeedSecCtrl)
	{
		$pageObject->controlsMap["search"]["searchBlocks"][] = array('fName'=>"mother", 'recId'=>$id, 'ctrlsMap'=>array(0=>$ctrlInd, 1=>($ctrlInd+1)));
		$ctrlInd+=2;
	}
	else
	{
		$pageObject->controlsMap["search"]["searchBlocks"][] = array('fName'=>"mother", 'recId'=>$id, 'ctrlsMap'=>array(0=>$ctrlInd));			
		$ctrlInd++;
	}
	// search fields data
	
	if($pageObject->pSet->getLookupTable("mother_tel"))
		$pageObject->settingsMap["globalSettings"]['shortTNames'][$pageObject->pSet->getLookupTable("mother_tel")] = GetTableURL($pageObject->pSet->getLookupTable("mother_tel"));
	
	$pageObject->fillFieldToolTips("mother_tel");	
	
	$srchFields = $pageObject->searchClauseObj->getSearchCtrlParams("mother_tel");
	$firstFieldParams = array();
	if (count($srchFields))
	{
		$firstFieldParams = $srchFields[0];
	...
	{
		$firstFieldParams['fName'] = "mother_tel";
		$firstFieldParams['eType'] = '';
		$firstFieldParams['value1'] = '';
		$firstFieldParams['opt'] = '';
		$firstFieldParams['value2'] = '';
	...
	if(isEnableSection508())
		$xt->assign_section("mother_tel_label","<label for=\"".GetInputElementId("mother_tel", $id, PAGE_SEARCH)."\">","</label>");
	else 
		$xt->assign("mother_tel_label", true);
	
	$xt->assign("mother_tel_fieldblock", true);
	$xt->assignbyref("mother_tel_editcontrol", $ctrlBlockArr['searchcontrol']);
	$xt->assign("mother_tel_notbox", $ctrlBlockArr['notbox']);
	// create second control, if need it
	$xt->assignbyref("mother_tel_editcontrol1", $ctrlBlockArr['searchcontrol1']);
	// create search type select
	$xt->assign("searchtype_mother_tel", $ctrlBlockArr['searchtype']);
	$isFieldNeedSecCtrl = $searchControlBuilder->isNeedSecondCtrl("mother_tel");
	$ctrlInd = 0;
	if ($isFieldNeedSecCtrl)
	{
		$pageObject->controlsMap["search"]["searchBlocks"][] = array('fName'=>"mother_tel", 'recId'=>$id, 'ctrlsMap'=>array(0=>$ctrlInd, 1=>($ctrlInd+1)));
		$ctrlInd+=2;
	}
	else
	{
		$pageObject->controlsMap["search"]["searchBlocks"][] = array('fName'=>"mother_tel", 'recId'=>$id, 'ctrlsMap'=>array(0=>$ctrlInd));			
		$ctrlInd++;
	}
	// search fields data
