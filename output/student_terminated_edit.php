...
$layout = new TLayout("edit","BoldOrange","MobileOrange");
...
$layout->blocks["top"][] = "details";$page_layouts["student_terminated_edit"] = $layout;
...
$layout->blocks["bare"][] = "1";$page_layouts["student_terminated_edit_Personal1"] = $layout;
...
$layout->skins["1"] = "fields";
$layout->blocks["bare"][] = "1";$page_layouts["student_terminated_edit_Family_Info1"] = $layout;
...
$layout->skins["1"] = "fields";
$layout->blocks["bare"][] = "1";$page_layouts["student_terminated_edit_Next_Of_Kin1"] = $layout;
...
$layout->skins["1"] = "fields";
$layout->blocks["bare"][] = "1";$page_layouts["student_terminated_edit_Campus_Info1"] = $layout;
...
$layout->skins["1"] = "fields";
$layout->blocks["bare"][] = "1";$page_layouts["student_terminated_edit_Medical1"] = $layout;
...
$templatefile = "student_terminated_edit.htm";
...
//	processing f_ic - end
//	processing f_address - begin
	$condition = $inlineedit!=EDIT_INLINE;//(!$inlineedit) edit simple mode

	if($condition)
	{
		$control_f_address = $pageObject->getControl("f_address", $id);
		$control_f_address->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);
...
//	processing m_ic - end
//	processing m_address - begin
	$condition = $inlineedit!=EDIT_INLINE;//(!$inlineedit) edit simple mode

	if($condition)
	{
		$control_m_address = $pageObject->getControl("m_address", $id);
		$control_m_address->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);
...
if ($IsSaved && no_output_done() && $inlineedit == EDIT_SIMPLE)
{
	...
	// redirect
	header("Location: student_terminated_".$pageObject->getPageType().".php?".$keyGetQ);
...
if(!$data)
{
	...
	{
		header("Location: student_terminated_list.php?a=return");
		exit();
	}
	else
		$data = array();
...
//	show readonly fields
	$pageObject->readOnlyFields["Name1"] = htmlspecialchars($pageObject->showDBValue("Name1", $data));
...
if($inlineedit != EDIT_INLINE)
{
	...
	if(!$pageObject->isAppearOnTabs("Name1"))
		$xt->assign("Name1_fieldblock",true);
	else
		$xt->assign("Name1_tabfieldblock",true);
	$xt->assign("Name1_label",true);
	if(isEnableSection508())
		$xt->assign_section("Name1_label","<label for=\"".GetInputElementId("Name1", $id, PAGE_EDIT)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("f_address_label","<label for=\"".GetInputElementId("f_address", $id, PAGE_EDIT)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("m_address_label","<label for=\"".GetInputElementId("m_address", $id, PAGE_EDIT)."\">","</label>");
...
foreach($pageObject->editFields as $fName)
{
	...
	if($inlineedit == EDIT_INLINE)
	{
		...
		$controls["controls"]['mode'] = "inline_edit";
			...
			$controls["controls"]['mode'] = "edit";
							...
							if($pageObject->pSet->isUseRTE("Type") && $fName == "Type")
	{
		$_SESSION[$strTableName."_"."Type"."_rte"] = "Fulltime";
										...
										if($pageObject->pSet->isUseRTE("Nationality") && $fName == "Nationality")
	{
		...
		$control[$gfName]["params"]["mode"] = "add";
				...
				if($pageObject->pSet->isUseRTE("Status") && $fName == "Status")
	{
		$_SESSION[$strTableName."_"."Status"."_rte"] = "Active";
		$control[$gfName]["params"]["mode"] = "add";
	}
...
if($pageObject->isShowDetailTables && $inlineedit!=EDIT_INLINE && !isMobile())
{
	...
	for($d=0;$d<count($dpParams['ids']);$d++)
	{
		...
		$options['masterTable'] = "student_terminated";
		$options['firstTime'] = 1;
...
$xt->assign("viewlink_attrs","id=\"viewButton".$id."\" name=\"viewButton".$id."\" onclick=\"window.location.href='student_terminated_view.php?".$viewlink."'\"");
