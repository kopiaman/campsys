...
if($inlineadd == ADD_INLINE)
	$templatefile = "course_moe2_inline_add.htm";
else
	$templatefile = "course_moe2_add.htm";
...
if(isset($_REQUEST['afteradd'])){
		header('Location: course_moe2_add.php');
			...
			if($inlineadd==ADD_SIMPLE || $inlineadd==ADD_MASTER)
			{
				...
				if (count($keys))
				{
					...
					if($pageObject->pSet->hasEditPage() && $permis['edit'])
						$message .='&nbsp;<a href=\'course_moe2_edit.php?'.$keylink.'\'>'."Edit".'</a>&nbsp;';
					if($pageObject->pSet->hasViewPage() && $permis['search'])
						$message .='&nbsp;<a href=\'course_moe2_view.php?'.$keylink.'\'>'."View".'</a>&nbsp;';
				}
				$mesClass = "mes_ok";	
			}
		}
...
if (no_output_done() && $inlineadd==ADD_SIMPLE && $IsSaved)
{
	...
	// redirect
	header("Location: course_moe2_".$pageObject->getPageType().".php");
