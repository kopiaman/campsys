...
$layout->blocks["top"][] = "details";$page_layouts["group_add"] = $layout;
			...
			if($inlineadd==ADD_SIMPLE || $inlineadd==ADD_MASTER)
			{
				...
				if (count($keys))
				{
					...
					if($pageObject->pSet->hasEditPage() && $permis['edit'])
						$message .='&nbsp;<a href=\'group_edit.php?'.$keylink.'\'>'."Edit".'</a>&nbsp;';
					if($pageObject->pSet->hasViewPage() && $permis['search'])
						$message .='&nbsp;<a href=\'group_view.php?'.$keylink.'\'>'."View".'</a>&nbsp;';
				}
				$mesClass = "mes_ok";	
			}
		}
...
function GetAddedDataLookupQuery($pageObject, $keys, $forLookup)
{
	...
	if(postvalue("table")=="Report_Sponsor" && postvalue("field")=="Class") 
	{
		$lookupFieldName = "Class";
		$lookupTable = "group";
		$linkFieldName = "id";
		$dispfield = "group";
		...
		if($nLookupType == LT_QUERY)
		{
			if($lookupPSet->getCustomDisplay($lookupFieldName))
				$pageObject->pSet->getSQLQuery()->AddCustomExpression($dispfield, $pageObject->pSet, "Report_Sponsor", "Class");
	...
	if(postvalue("table")=="Report_Exam_Slip_Batch" && postvalue("field")=="Class") 
	{
		$lookupFieldName = "Class";
		$lookupTable = "group";
		$linkFieldName = "id";
		...
		{
			if($lookupPSet->getCustomDisplay($lookupFieldName))
				$pageObject->pSet->getSQLQuery()->AddCustomExpression($dispfield, $pageObject->pSet, "Report_Exam_Slip_Batch", "Class");
			$lookupQueryObj = $pageObject->pSet->getSQLQuery()->CloneObject();
			if(!$noBlobReplace)
				$lookupQueryObj->ReplaceFieldsWithDummies($pageObject->pSet->getBinaryFieldsIndices());
		}else{
	...
	if(postvalue("table")=="Report_Exam_Result_Batch" && postvalue("field")=="Class") 
	{
		$lookupFieldName = "Class";
		$lookupTable = "group";
		$linkFieldName = "id";
		...
		{
			if($lookupPSet->getCustomDisplay($lookupFieldName))
				$pageObject->pSet->getSQLQuery()->AddCustomExpression($dispfield, $pageObject->pSet, "Report_Exam_Result_Batch", "Class");
			$lookupQueryObj = $pageObject->pSet->getSQLQuery()->CloneObject();
			if(!$noBlobReplace)
				$lookupQueryObj->ReplaceFieldsWithDummies($pageObject->pSet->getBinaryFieldsIndices());
		}else{
