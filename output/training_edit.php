...
$skeys = "";
$savedKeys = array();
$keys["TrainingID"] = urldecode(postvalue("editid1"));
$savedKeys["TrainingID"] = urldecode(postvalue("editid1"));
...
//	processing TrainingName - begin
	$condition = $inlineedit==EDIT_INLINE;//($inlineedit) inline mode

	if($condition)
	{
		$control_TrainingName = $pageObject->getControl("TrainingName", $id);
		$control_TrainingName->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);
...
if ($IsSaved && no_output_done() && $inlineedit == EDIT_SIMPLE)
{
	...
	// key get query
	$keyGetQ = "";
		$keyGetQ.="editid1=".rawurldecode($keys["TrainingID"])."&";
	// cut last &
...
if($readevalues)
{
	$data["TrainingName"] = $evalues["TrainingName"];
}
...
if($inlineedit != EDIT_INLINE)
{
	...
	$xt->assign("show_key1", htmlspecialchars($pageObject->showDBValue("TrainingID", $data)));
