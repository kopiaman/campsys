...
if ($mode==SEARCH_SIMPLE)
{
	...
	if($pageObject->pSet->getLookupTable("Employee_Type"))
		$pageObject->settingsMap["globalSettings"]['shortTNames'][$pageObject->pSet->getLookupTable("Employee_Type")] = GetTableURL($pageObject->pSet->getLookupTable("Employee_Type"));
	
	$pageObject->fillFieldToolTips("Employee_Type");	
	
	$srchFields = $pageObject->searchClauseObj->getSearchCtrlParams("Employee_Type");
	$firstFieldParams = array();
	if (count($srchFields))
	{
		$firstFieldParams = $srchFields[0];
	...
	{
		$firstFieldParams['fName'] = "Employee_Type";
		$firstFieldParams['eType'] = '';
		$firstFieldParams['value1'] = '';
		$firstFieldParams['opt'] = '';
		$firstFieldParams['value2'] = '';
	...
	if(isEnableSection508())
		$xt->assign_section("Employee_Type_label","<label for=\"".GetInputElementId("Employee_Type", $id, PAGE_SEARCH)."\">","</label>");
	else 
		$xt->assign("Employee_Type_label", true);
	
	$xt->assign("Employee_Type_fieldblock", true);
	$xt->assignbyref("Employee_Type_editcontrol", $ctrlBlockArr['searchcontrol']);
	$xt->assign("Employee_Type_notbox", $ctrlBlockArr['notbox']);
	// create second control, if need it
	$xt->assignbyref("Employee_Type_editcontrol1", $ctrlBlockArr['searchcontrol1']);
	// create search type select
	$xt->assign("searchtype_Employee_Type", $ctrlBlockArr['searchtype']);
	$isFieldNeedSecCtrl = $searchControlBuilder->isNeedSecondCtrl("Employee_Type");
	$ctrlInd = 0;
	if ($isFieldNeedSecCtrl)
	{
		$pageObject->controlsMap["search"]["searchBlocks"][] = array('fName'=>"Employee_Type", 'recId'=>$id, 'ctrlsMap'=>array(0=>$ctrlInd, 1=>($ctrlInd+1)));
		$ctrlInd+=2;
	}
	else
	{
		$pageObject->controlsMap["search"]["searchBlocks"][] = array('fName'=>"Employee_Type", 'recId'=>$id, 'ctrlsMap'=>array(0=>$ctrlInd));			
		$ctrlInd++;
	}
