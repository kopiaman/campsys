...
$layout->blocks["bare"][] = "detailspreviewgrid";$page_layouts["t_eva_assign_detailspreview"] = $layout;
...
if($mastertable == "Evaluation_Batch_List")
{
	$where = "";
		$where .= GetFullFieldName("program", $strTableName, false)."=".make_db_value("program",$_SESSION[$strTableName."_masterkey1"]);
		$where.=" and ";
	$where .= GetFullFieldName("batch", $strTableName, false)."=".make_db_value("batch",$_SESSION[$strTableName."_masterkey2"]);
}
...
if($rowcount) {
	...
	//	program - 
			$viewContainer->recId = $recordsCounter;
		    $value = $viewContainer->showDBValue("program", $data, $keylink);
			$row["program_value"] = $value;
	//	batch - 
			$viewContainer->recId = $recordsCounter;
		    $value = $viewContainer->showDBValue("batch", $data, $keylink);
			$row["batch_value"] = $value;
		$rowinfo[] = $row;
		$data = $cipherer->DecryptFetchedArray($rs);
	}
...
$xt->load_template("t_eva_assign_detailspreview.htm");
