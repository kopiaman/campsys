...
add_nocache_headers();
include("include/w_course_variables.php");
...
$layout->blocks["top"][] = "details";$page_layouts["w_course_add"] = $layout;
...
if($inlineadd == ADD_INLINE)
	$templatefile = "w_course_inline_add.htm";
else
	$templatefile = "w_course_add.htm";
...
if(isset($_REQUEST['afteradd'])){
		header('Location: w_course_add.php');
...
//	processing name - end
//	processing intro - start
	$inlineAddOption = true;
	$inlineAddOption = $inlineadd!=ADD_INLINE;
	if($inlineAddOption)
	{
		$control_intro = $pageObject->getControl("intro", $id);
		$control_intro->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing intro - end
//	processing about - start
	$inlineAddOption = true;
	$inlineAddOption = $inlineadd!=ADD_INLINE;
	if($inlineAddOption)
	{
		$control_about = $pageObject->getControl("about", $id);
		$control_about->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing about - end
//	processing requirement - start
	$inlineAddOption = true;
	$inlineAddOption = $inlineadd!=ADD_INLINE;
	if($inlineAddOption)
	{
		$control_requirement = $pageObject->getControl("requirement", $id);
		$control_requirement->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing requirement - end
...
//	processing intro - start
			...
			if($inlineAddOption)
			{
				$control_intro->afterSuccessfulSave();
			}
//	processing intro - end
//	processing about - start
			$inlineAddOption = true;
			...
			if($inlineAddOption)
			{
				$control_about->afterSuccessfulSave();
			}
//	processing about - end
//	processing requirement - start
			$inlineAddOption = true;
			...
			if($inlineAddOption)
			{
				$control_requirement->afterSuccessfulSave();
			}
//	processing requirement - end
			...
			if($inlineadd==ADD_SIMPLE || $inlineadd==ADD_MASTER)
			{
				...
				if (count($keys))
				{
					...
					if($pageObject->pSet->hasEditPage() && $permis['edit'])
						$message .='&nbsp;<a href=\'w_course_edit.php?'.$keylink.'\'>'."Edit".'</a>&nbsp;';
					if($pageObject->pSet->hasViewPage() && $permis['search'])
						$message .='&nbsp;<a href=\'w_course_view.php?'.$keylink.'\'>'."View".'</a>&nbsp;';
				}
				$mesClass = "mes_ok";	
			}
		}
...
if (no_output_done() && $inlineadd==ADD_SIMPLE && $IsSaved)
{
	...
	// redirect
	header("Location: w_course_".$pageObject->getPageType().".php");
...
if($readavalues)
{
	...
	$defvalues["intro"]=@$avalues["intro"];
	$defvalues["requirement"]=@$avalues["requirement"];
	$defvalues["about"]=@$avalues["about"];
	$defvalues["pos"]=@$avalues["pos"];
}
...
if($inlineadd!=ADD_INLINE)
{
	...
	if(!$pageObject->isAppearOnTabs("intro"))
		$xt->assign("intro_fieldblock",true);
	else
		$xt->assign("intro_tabfieldblock",true);
	$xt->assign("intro_label",true);
	if(isEnableSection508())
		$xt->assign_section("intro_label","<label for=\"".GetInputElementId("intro", $id, PAGE_ADD)."\">","</label>");
	
	if(!$pageObject->isAppearOnTabs("requirement"))
		$xt->assign("requirement_fieldblock",true);
	else
		$xt->assign("requirement_tabfieldblock",true);
	$xt->assign("requirement_label",true);
	if(isEnableSection508())
		$xt->assign_section("requirement_label","<label for=\"".GetInputElementId("requirement", $id, PAGE_ADD)."\">","</label>");
	
	if(!$pageObject->isAppearOnTabs("about"))
		$xt->assign("about_fieldblock",true);
	else
		$xt->assign("about_tabfieldblock",true);
	$xt->assign("about_label",true);
	if(isEnableSection508())
		$xt->assign_section("about_label","<label for=\"".GetInputElementId("about", $id, PAGE_ADD)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("pos_label","<label for=\"".GetInputElementId("pos", $id, PAGE_ADD)."\">","</label>");
...
//	intro
	...
	if($display)
	{	
		$value = $pageObject->showDBValue("intro", $data, $keylink);
		$showValues["intro"] = $value;
		$showFields[] = "intro";
	}	
//	requirement
	$display = false;
	...
	if($display)
	{	
		$value = $pageObject->showDBValue("requirement", $data, $keylink);
		$showValues["requirement"] = $value;
		$showFields[] = "requirement";
	}	
//	about
	$display = false;
	if($inlineadd==ADD_MASTER)
	...
	if($display)
	{	
		$value = $pageObject->showDBValue("about", $data, $keylink);
		$showValues["about"] = $value;
		$showFields[] = "about";
	}	
//	pos
	$display = false;
	...
	{	
		...
		$showRawValues["intro"] = substr($data["intro"],0,100);
		$showRawValues["requirement"] = substr($data["requirement"],0,100);
		$showRawValues["about"] = substr($data["about"],0,100);
