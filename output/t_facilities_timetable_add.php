...
$layout->blocks["top"][] = "details";$page_layouts["t_facilities_timetable_add"] = $layout;
...
$layout->blocks["bare"][] = "1";$page_layouts["t_facilities_timetable_add_Datetime1"] = $layout;
...
if($inlineadd == ADD_INLINE)
	$templatefile = "t_facilities_timetable_inline_add.htm";
else
	$templatefile = "t_facilities_timetable_add.htm";
...
if(isset($_REQUEST['afteradd'])){
		header('Location: t_facilities_timetable_add.php');
...
if(@$_POST["a"]=="added")
{
	...
	$avalues=array();
	$blobfields=array();
//	processing legend - start
	$inlineAddOption = true;
	$inlineAddOption = $inlineadd!=ADD_INLINE;
	if($inlineAddOption)
	{
		$control_legend = $pageObject->getControl("legend", $id);
		$control_legend->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
...
//	processing facility - start
	$inlineAddOption = true;
	$inlineAddOption = $inlineadd!=ADD_INLINE;
	if($inlineAddOption)
	{
		$control_facility = $pageObject->getControl("facility", $id);
		$control_facility->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing facility - end
//	processing courseID - start
	$inlineAddOption = true;
	$inlineAddOption = $inlineadd!=ADD_INLINE;
	if($inlineAddOption)
	{
		$control_courseID = $pageObject->getControl("courseID", $id);
		$control_courseID->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
...
//	processing staffID2 - start
	$inlineAddOption = true;
	$inlineAddOption = $inlineadd!=ADD_INLINE;
	if($inlineAddOption)
	{
		$control_staffID2 = $pageObject->getControl("staffID2", $id);
		$control_staffID2->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
			...
			if($inlineadd==ADD_SIMPLE || $inlineadd==ADD_MASTER)
			{
				...
				if (count($keys))
				{
					...
					if($pageObject->pSet->hasEditPage() && $permis['edit'])
						$message .='&nbsp;<a href=\'t_facilities_timetable_edit.php?'.$keylink.'\'>'."Edit".'</a>&nbsp;';
					if($pageObject->pSet->hasViewPage() && $permis['search'])
						$message .='&nbsp;<a href=\'t_facilities_timetable_view.php?'.$keylink.'\'>'."View".'</a>&nbsp;';
				}
				$mesClass = "mes_ok";	
			}
		}
...
if (no_output_done() && $inlineadd==ADD_SIMPLE && $IsSaved)
{
	...
	// redirect
	header("Location: t_facilities_timetable_".$pageObject->getPageType().".php");
...
if($readavalues)
{
	$defvalues["courseID"]=@$avalues["courseID"];
	...
	$defvalues["facility"]=@$avalues["facility"];
	...
	$defvalues["legend"]=@$avalues["legend"];
	$defvalues["staffID2"]=@$avalues["staffID2"];
}
...
if($inlineadd!=ADD_INLINE)
{
	...
	if(isEnableSection508())
		$xt->assign_section("courseID_label","<label for=\"".GetInputElementId("courseID", $id, PAGE_ADD)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("facility_label","<label for=\"".GetInputElementId("facility", $id, PAGE_ADD)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("legend_label","<label for=\"".GetInputElementId("legend", $id, PAGE_ADD)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("staffID2_label","<label for=\"".GetInputElementId("staffID2", $id, PAGE_ADD)."\">","</label>");
...
//	facility
	...
	if($display)
	{	
		$value = $pageObject->showDBValue("facility", $data, $keylink);
		$showValues["facility"] = $value;
		$showFields[] = "facility";
	}	
//	program
	$display = false;
...
//	legend
	...
	if($display)
	{	
		$value = $pageObject->showDBValue("legend", $data, $keylink);
		$showValues["legend"] = $value;
		$showFields[] = "legend";
	}	
//	staffID2
	$display = false;
	...
	if($display)
	{	
		$value = $pageObject->showDBValue("staffID2", $data, $keylink);
		$showValues["staffID2"] = $value;
		$showFields[] = "staffID2";
	}	
		...
		$showRawValues["facility"] = substr($data["facility"],0,100);
		...
		$showRawValues["legend"] = substr($data["legend"],0,100);
		$showRawValues["room"] = substr($data["room"],0,100);
		$showRawValues["staffID2"] = substr($data["staffID2"],0,100);
