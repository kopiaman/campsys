...
$layout->blocks["top"][] = "search";$page_layouts["exam_result_memo_moe_search"] = $layout;
...
$mode = SEARCH_SIMPLE;
$templatefile = "exam_result_memo_moe_search.htm";
...
$params['shortTableName'] = 'exam_result_memo_moe';
...
if ($mode==SEARCH_SIMPLE)
{
	...
	if($pageObject->pSet->getLookupTable("date_memo2"))
		$pageObject->settingsMap["globalSettings"]['shortTNames'][$pageObject->pSet->getLookupTable("date_memo2")] = GetTableURL($pageObject->pSet->getLookupTable("date_memo2"));
	
	$pageObject->fillFieldToolTips("date_memo2");	
	
	$srchFields = $pageObject->searchClauseObj->getSearchCtrlParams("date_memo2");
	...
	{
		$firstFieldParams['fName'] = "date_memo2";
	...
	if(isEnableSection508())
		$xt->assign_section("date_memo2_label","<label for=\"".GetInputElementId("date_memo2", $id, PAGE_SEARCH)."\">","</label>");
	else 
		$xt->assign("date_memo2_label", true);
	
	$xt->assign("date_memo2_fieldblock", true);
	$xt->assignbyref("date_memo2_editcontrol", $ctrlBlockArr['searchcontrol']);
	$xt->assign("date_memo2_notbox", $ctrlBlockArr['notbox']);
	// create second control, if need it
	$xt->assignbyref("date_memo2_editcontrol1", $ctrlBlockArr['searchcontrol1']);
	// create search type select
	$xt->assign("searchtype_date_memo2", $ctrlBlockArr['searchtype']);
	$isFieldNeedSecCtrl = $searchControlBuilder->isNeedSecondCtrl("date_memo2");
	$ctrlInd = 0;
	if ($isFieldNeedSecCtrl)
	{
		$pageObject->controlsMap["search"]["searchBlocks"][] = array('fName'=>"date_memo2", 'recId'=>$id, 'ctrlsMap'=>array(0=>$ctrlInd, 1=>($ctrlInd+1)));
		$ctrlInd+=2;
	}
	else
	{
		$pageObject->controlsMap["search"]["searchBlocks"][] = array('fName'=>"date_memo2", 'recId'=>$id, 'ctrlsMap'=>array(0=>$ctrlInd));			
		$ctrlInd++;
	}
	// search fields data
	
	if($pageObject->pSet->getLookupTable("ref2"))
		$pageObject->settingsMap["globalSettings"]['shortTNames'][$pageObject->pSet->getLookupTable("ref2")] = GetTableURL($pageObject->pSet->getLookupTable("ref2"));
	
	$pageObject->fillFieldToolTips("ref2");	
	
	$srchFields = $pageObject->searchClauseObj->getSearchCtrlParams("ref2");
	$firstFieldParams = array();
	if (count($srchFields))
	{
		$firstFieldParams = $srchFields[0];
	...
	if(isEnableSection508())
		$xt->assign_section("ref2_label","<label for=\"".GetInputElementId("ref2", $id, PAGE_SEARCH)."\">","</label>");
	else 
		$xt->assign("ref2_label", true);
	
	$xt->assign("ref2_fieldblock", true);
	$xt->assignbyref("ref2_editcontrol", $ctrlBlockArr['searchcontrol']);
	$xt->assign("ref2_notbox", $ctrlBlockArr['notbox']);
	// create second control, if need it
	$xt->assignbyref("ref2_editcontrol1", $ctrlBlockArr['searchcontrol1']);
	// create search type select
	$xt->assign("searchtype_ref2", $ctrlBlockArr['searchtype']);
	$isFieldNeedSecCtrl = $searchControlBuilder->isNeedSecondCtrl("ref2");
	$ctrlInd = 0;
	if ($isFieldNeedSecCtrl)
	{
		$pageObject->controlsMap["search"]["searchBlocks"][] = array('fName'=>"ref2", 'recId'=>$id, 'ctrlsMap'=>array(0=>$ctrlInd, 1=>($ctrlInd+1)));
		$ctrlInd+=2;
	}
	else
	{
		$pageObject->controlsMap["search"]["searchBlocks"][] = array('fName'=>"ref2", 'recId'=>$id, 'ctrlsMap'=>array(0=>$ctrlInd));			
		$ctrlInd++;
	}
	// search fields data
